	...
	struct fragment *fragment, struct directory *dir_in, long long sparse)
{
	...
	ra_target_attr(subpath, strlen(subpath), &attr);

	xattr = read_xattrs(dir_ent, attr);
	...
	if ((type == SQUASHFS_LSYMLINK_TYPE) || (type == SQUASHFS_SYMLINK_TYPE))
		base->mode = SQUASHFS_MODE(st_mode);
	else
...
#define VERSION() \
	printf("mksquashfs version 4.3 (2014/05/12), add rule-attr by czyong (2015/02/28)\n");\
...
/* ANDROID CHANGES END */
			...
			root_name = argv[i];
		} else if((strcmp(argv[i], "--rulefile") == 0) || (strcmp(argv[i], "-T") == 0)) {
			if(++i == argc) {
				ERROR("%s: --rulefile(-T) missing rulefile name.\n",
					argv[0]);
				exit(1);
			}

			ra_rulefile = argv[i];
		} else if((strcmp(argv[i], "--ruledebug") == 0) || (strcmp(argv[i], "-G") == 0)) {
			if(++i == argc) {
				ERROR("%s: --ruledebug(-G) missing debug level\n",
					argv[0]);
				exit(1);
			}

			ra_debuglevel = strtoll(argv[i], &b, 10);
...
/* ANDROID CHANGES END */
			...
			ERROR("\t\t\tOptionally a suffix of K, M or G can be"
				" given to specify\n\t\t\tKbytes, Mbytes or"
				" Gbytes respectively\n");
			ERROR("--rulefile(-T) <file>\t attrubite rule file\n");
			ERROR("--ruledebug(-G) <level>\t attrubite debug level\n");
