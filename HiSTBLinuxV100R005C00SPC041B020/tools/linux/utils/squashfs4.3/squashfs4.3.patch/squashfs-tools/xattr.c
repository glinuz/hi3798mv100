...
static int read_xattrs_from_rule(void *attr, struct xattr_list **xattrs)
{
	int vsize;
	struct xattr_list *xl;

	vsize = ra_attr_cap(attr, NULL);
	if (vsize < 0)
		return 0;

	xl = (struct xattr_list *)malloc(sizeof(struct xattr_list));
	if (!xl) {
		ERROR("Out of memory in read_attrs\n");
		return 0;
	}

	xl->type = get_prefix(xl, "security.capability");
	if (xl->type == -1) {
		free(xl);
		return 0;
	}
					...
					void *attr)
{
	...
	i = read_xattrs_from_rule(attr, xattrs);
	if (i)
		return i;
...
int read_xattrs(void *d, void *attr)
{
...
/* ANDROID CHANGES START*/
	...
	if (context_file) {
		...
		xattrs = read_xattrs_from_system(filename, &xattr_list, attr);
	}
#else
	xattrs = read_xattrs_from_system(filename, &xattr_list, attr);
#endif
