...
#define GRID 100  
  ...
  {"rulefile", 'T', N_("RULEFILE"), 0,
   N_("rule list filename"), GRID+1 },
  {"ruledebug", 'G', N_("RULEDEBUG"), 0,
   N_("rule list debuglevel"), GRID+1 },
  {NULL, 'B', NULL, 0,
...
crc newc odc bin ustar tar (all-caps also recognized)"), arg));
      break;
    case 'T': 	  /* Input archive file name.  */
	ra_rulefile = arg;
	break;
    case 'G':     /* Input archive file name.  */
	ra_debuglevel = strtol(arg, NULL, 10);
	break;
    case 'i':		/* Copy-in mode.  */
      if (copy_function != 0)
