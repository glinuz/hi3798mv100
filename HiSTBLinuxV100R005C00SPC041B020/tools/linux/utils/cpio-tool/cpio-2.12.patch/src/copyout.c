...
static char *target_path(char *pathname, int *sz_tgpath)
{
	int sz_path;
	char *ptr = pathname;
	if (!pathname)
		return NULL;
	sz_path = strlen(pathname);
	if ((*ptr == '.') && (*(ptr + 1) == '/')) {
		ptr += 1;
		sz_path -= 1;
	}
...
process_copy_out ()
{
  ...
  while (ds_fgetstr (stdin, &input_name, name_end) != NULL)
	...
	{
	  extern char *ra_rulefile;
	  if (ra_rulefile) {
		void *attr;
		char *tgpath;
		int sz_tgpath;
		tgpath = target_path(input_name.ds_string, &sz_tgpath);
	  	mode_t st_mode = file_stat.st_mode;
	  	ra_target_stat(tgpath, sz_tgpath, &file_stat);
		if (S_ISLNK(file_stat.st_mode))
			file_stat.st_mode = st_mode;
	  }
