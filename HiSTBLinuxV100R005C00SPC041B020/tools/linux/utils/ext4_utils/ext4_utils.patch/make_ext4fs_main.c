...
static void usage(char *path)
{
	...
	fprintf(stderr, "    [-T <rulefile>] [-G <debuglevel>]\n");
	fprintf(stderr, "    <filename> [<directory>]\n");
}

/******************************************************************************/

void ra_ext4fs_config(const char *path, int dir, unsigned *uid, unsigned *gid,
		      unsigned *mode, uint64_t *capabilities)
{
	void *attr;
	unsigned int permitted[2] = {0};
	short _gid = 0;
	short _uid = 0;
	unsigned short _mode = 0;

	if (!path || !uid || !gid || !mode || !capabilities) {
		fprintf(stderr, "ext4fs_config invalid param\n");
		return;
	}

	if (ra_target_attr(path, strlen(path), &attr))
		return;

	if (ra_attr_stat_cap(attr, &_gid, &_uid, &_mode, permitted))
		return;

	*uid = _uid;
	*gid = _gid;
	*mode = _mode;
	*capabilities = permitted[1];
	*capabilities <<= 32;
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "l:j:b:g:i:I:L:a:S:T:G:fwzJsctv")) != -1) {
		switch (opt) {
		case 'l':
		...
		case 'T':
			...
			break;
		case 'G':
			ra_debuglevel = parse_num(optarg);
			break;
		default: /* '?' */
			usage(argv[0]);
			exit(EXIT_FAILURE);
	...
	if (!fs_config_func && ra_rulefile) {
		fs_config_func = ra_ext4fs_config;
