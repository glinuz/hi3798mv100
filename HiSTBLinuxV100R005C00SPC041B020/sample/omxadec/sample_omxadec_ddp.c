#ifdef ANDROID
#undef  LOG_TAG
#define LOG_TAG    "HIOMX_DDP_SAMPLE"
#endif
...
FILE* fIn, *fPcmOut, *fBitOut;

#define INPUT_PORT 0
#define OUTPUT_PORT 1
#define MAX_NUM_OF_BUFS_AUDIODEC 16

#define COMPONENT_NAME_BASE "OMX.hisi.audio.decode"
...
int fill_data (OMX_BUFFERHEADERTYPE* pBuf, FILE* fIn)
{
    ...
    //sample_printf( "\n*****************************************************\n");
    //sample_printf( "%d :: APP:: Buffer No : %d  pBuf->pBuffer = %p pBuf->nAllocLen = * %ld, nRead = %ld\n",
...
OMX_STRING strDdpDecoder = "OMX.hisi.audio.ddpdec";
                             ...
                             OMX_OUT OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if (u32PcmBytesPerFrame > 0)
    {
        fwrite(pBuffer->pBuffer, 1, u32PcmBytesPerFrame, fPcmOut);
        WvH.dwDataLen += u32PcmBytesPerFrame;
		
		if (pHAData->stAdec.u32BitstreamOffset != 0)
		{
			pBitsBuffer = pBuffer->pBuffer + u32PcmBytesPerFrame;
			if(pBuffer->nFilledLen > u32PcmBytesPerFrame)
			{
			   u32BitsSize = pBuffer->nFilledLen - u32PcmBytesPerFrame;
			   fwrite(pBitsBuffer, 1, u32BitsSize, fBitOut);
			}
		}		
    }
...
OMX_ERRORTYPE test_OpenClose(OMX_STRING component_name)
{
    ...
    sample_printf("GENERAL TEST %s\n", __func__);
    err = OMX_GetHandle(&audiodechandle, strDdpDecoder, &sAppPrivData, &audiodeccallbacks);
    if (err != OMX_ErrorNone)
    {
...
int main(int argc, char** argv)
{
    ...
    if (argc < 4)
    {
        sample_printf(" usage: ./sample_omxadec_ddp infile outpcmfile outbitfile\n"); 
        return -1;
    }
    ...
    if (fPcmOut)
    {
        ...
        fseek(fPcmOut, 0, SEEK_SET);
        fwrite(&WvH, sizeof(WAVEHEADER), 1, fPcmOut);
    }
    ...
    GT_START();
    error = OMX_GetHandle(&audiodechandle, strDdpDecoder, &sAppPrivData, &audiodeccallbacks);
    ...
    if (error != OMX_ErrorNone)
    {
        ...
        sample_printf("%d :: OMX_ErrorBadParameter\n", __LINE__);
        goto EXIT;
    }
    ...
    if (error == OMX_ErrorNone)
    {
        ...
        g_channels = sPcmParam.nChannels;
		g_pcmSize  = sPcmParam.nSize;
		printf("samplerate %d,chanel %d,nsize %d\n", g_samplerate,g_channels,g_pcmSize);
    }
	else
	{
        sample_printf( "Error:  OMX_GetParameter reports an error %X\n", error);
