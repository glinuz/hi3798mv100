#ifdef ANDROID
#undef  LOG_TAG
#define LOG_TAG    "HIOMX_MP3_SAMPLE"
#endif
...
OMX_STRING strMp3Decoder = "OMX.hisi.audio.mp3dec";
...
OMX_ERRORTYPE test_OpenClose(OMX_STRING component_name)
{
    ...
    sample_printf("GENERAL TEST %s\n", __func__);
    err = OMX_GetHandle(&audiodechandle, strMp3Decoder, &sAppPrivData, &audiodeccallbacks);
    if (err != OMX_ErrorNone)
    {
...
int main(int argc, char** argv)
{
  ...
  //  sample_printf("------------------------------------------------------\n");
 //   sample_printf("This is Main Thread In MP3 DECODER Test Application:\n");
    ...
    GT_START();
    error = OMX_GetHandle(&audiodechandle, strMp3Decoder, &sAppPrivData, &audiodeccallbacks);
