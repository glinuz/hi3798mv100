...
/* directfb type and hisi encode format map. */
typedef struct {
...
dfb_res_map map[] = {
	{HI_DRV_DISP_FMT_1080P_60, DSETV_NTSC, DSEF_60HZ, DSOR_1920_1080, DSESM_PROGRESSIVE}, /* 1080P 60Hz */
	{HI_DRV_DISP_FMT_1080P_50, DSETV_PAL, DSEF_50HZ, DSOR_1920_1080, DSESM_PROGRESSIVE}, /* 1080P 50Hz */
	{HI_DRV_DISP_FMT_1080i_60, DSETV_NTSC, DSEF_60HZ, DSOR_1920_1080, DSESM_INTERLACED}, /* 1080I 60Hz */
	{HI_DRV_DISP_FMT_1080i_50, DSETV_PAL, DSEF_50HZ, DSOR_1920_1080, DSESM_INTERLACED},	/* 1080I 50Hz */

	{HI_DRV_DISP_FMT_720P_60, DSETV_NTSC, DSEF_60HZ, DSOR_1280_720, DSESM_PROGRESSIVE},	/* 720P 60Hz */
	{HI_DRV_DISP_FMT_720P_50, DSETV_PAL, DSEF_50HZ, DSOR_1280_720, DSESM_PROGRESSIVE},	/* 720P 50Hz */
...
static DFBResult hisi_init_encoder( CoreScreen                   *screen,
		void                         *driver_data,
		void                         *screen_data,
		int                           encoder,
...
static DFBResult hisi_test_encoder_config( CoreScreen                   *screen,
		void                         *driver_data,
		void                         *screen_data,
		int                           encoder,
...
static DFBResult hisi_config_encoder ( CoreScreen                   *screen,
		void                         *driver_data,
		void                         *screen_data,
		int                           encoder,
		const DFBScreenEncoderConfig *config );

static DFBResult hisi_init_mixer ( CoreScreen                   *screen,
		void                         *driver_data,
		void                         *screen_data,
		int                           mixer,
...
static DFBResult hisi_config_mixer( CoreScreen                  *screen,
		void                        *driver_data,
		void                        *screen_data,
		int                          mixer,
		const DFBScreenMixerConfig  *config);

static DFBResult hisi_get_screen_size   ( CoreScreen                  *screen,
		void                        *driver_data,
		void                        *screen_data,
		int                         *ret_width,
		int                         *ret_height );
...
/* Find hisi encode format frome tv stand/scanmode/frequency/resolution. */
static HI_UNF_ENC_FMT_E find_fmt(DFBScreenEncoderTVStandards stand, 
		DFBScreenEncoderScanMode ip, DFBScreenEncoderFrequency freq, 
		DFBScreenOutputResolution res)
{
...
HI_S32 O5SDK_Disp_Init(HI_DRV_DISP_FMT_E enFormat)
{
	HI_S32 ret;
//	HI_UNF_DISP_INTF_S  stDispIntf;
//	HI_UNF_DISP_BG_COLOR_S  BgColor;
	// 1 
	HI_DRV_DISP_INTF_S stInt = {0};

	ret = HI_MPI_DISP_Init();
	if (ret != HI_SUCCESS)
	{
		D_ERROR("call HI_MPI_DISP_Init failed.\n");
		goto err0;
	}
	
	stInt.eID = HI_DRV_DISP_INTF_HDMI0;
	ret = HI_MPI_DISP_AddIntf(HI_DRV_DISPLAY_1, &stInt);
	if (ret)
	{
		D_ERROR("call HI_MPI_DISP_AddIntf failed.\n");
		goto err0_1;
	}
	...
	ret = HI_MPI_DISP_SetFormat(HI_DRV_DISPLAY_1 , HI_DRV_DISP_STEREO_NONE, enFormat);
	if (ret)
	{
		D_ERROR("call HI_MPI_DISP_SetFormat failed.\n");
		goto err0_1;
	}

	ret = HI_MPI_DISP_SetFormat(HI_DRV_DISPLAY_0 , HI_DRV_DISP_STEREO_NONE,  HI_DRV_DISP_FMT_PAL);
	if (ret)
	{
		D_ERROR("call HI_MPI_DISP_SetFormat failed.\n");
		goto err0_1;
	...
	HI_MPI_DISP_Attach(HI_DRV_DISPLAY_0 , HI_DRV_DISPLAY_1);
	if (ret)
	{
		D_ERROR("call HI_MPI_DISP_Attach failed.\n");
		goto err0_1;
	}
	// 6.1
	ret = HI_MPI_DISP_Open(HI_DRV_DISPLAY_1);
	if (ret != HI_SUCCESS)
	{
		D_ERROR("call HI_MPI_DISP_Open failed.\n");
		goto err2;
	}
	// 6.2   
	ret = HI_MPI_DISP_Open(HI_DRV_DISPLAY_0);
	if (ret != HI_SUCCESS)
	{
		D_ERROR("call HI_MPI_DISP_Open SD failed.\n");
 		HI_MPI_DISP_Close(HI_DRV_DISPLAY_1);
 		goto err2;	
    }
#if 0
	{
		...
		for (i = HI_UNF_DISPLAY0; i <HI_UNF_DISPLAY_BUTT; i++) {
			for (j = HI_UNF_DISP_LAYER_GFX0; j < HI_UNF_DISP_LAYER_BUTT; j++) {
				printf("%s->%d, i:%d, j:%d\n", __func__,__LINE__, i, j);
				ret = HI_UNF_DISP_SetLayerZorder( (HI_UNF_DISP_E)i, (HI_UNF_DISP_LAYER_E)j, HI_LAYER_ZORDER_MOVETOP );
				if (HI_SUCCESS != ret){
					D_ERROR("HI_UNF_DISP_SetLayerZorder Fail,ret:0x%x!\n",ret);
				}
				getchar();
			}
		}
...
#else
				printf("%s->%d, i:%d, j:%d\n", __func__,__LINE__, HI_UNF_DISPLAY1, HI_UNF_DISP_LAYER_GFX);
				ret = HI_UNF_DISP_SetLayerZorder( HI_DRV_DISPLAY_1, HI_UNF_DISP_LAYER_GFX, HI_LAYER_ZORDER_MOVETOP );
				if (HI_SUCCESS != ret){
					D_ERROR("HI_UNF_DISP_SetLayerZorder Fail,ret:0x%x!\n",ret);
				}
		...
		DFBScreenEncoderConfig       *config )
{
	...
	/* default 1080i_50 */
	HI_DRV_DISP_FMT_E enFormat = HI_DRV_DISP_FMT_1080i_50;
	...
	description->caps = DSECAPS_TV_STANDARDS | DSECAPS_RESOLUTION | DSECAPS_FREQUENCY | DSECAPS_SCANMODE
		| DSECAPS_BRIGHTNESS|DSECAPS_CONTRAST|DSECAPS_HUE|DSECAPS_SATURATION;
	...
	config->flags		= DSECONF_TV_STANDARD | DSECONF_RESOLUTION | DSECONF_FREQUENCY | DSECONF_SCANMODE
		| DSECONF_ADJUSTMENT;
	...
	(HI_VOID)HI_MPI_DISP_GetBrightness(HI_UNF_DISPLAY0, &val);
	config->adjustment.brightness = (u16)val;

	(HI_VOID)HI_MPI_DISP_GetContrast(HI_UNF_DISPLAY0, &val);
	config->adjustment.contrast = (u16)val;

	(HI_VOID)HI_MPI_DISP_GetHuePlus(HI_UNF_DISPLAY0, &val);
	config->adjustment.hue = (u16)val;

	(HI_VOID)HI_MPI_DISP_GetSaturation(HI_UNF_DISPLAY0, &val);
		...
		const DFBScreenEncoderConfig *config )
{
	...
	if (config->flags & (DSECAPS_TV_STANDARDS | DSECAPS_RESOLUTION | DSECAPS_FREQUENCY | DSECAPS_SCANMODE)) {
		...
		/* refer to HI_UNF_DISP_SetFormat. set hdmi first.. */
		//(HI_VOID)HI_MPI_HDMI_Pre_SetFormat(enFormat);

		ret = HI_MPI_DISP_SetFormat(HI_UNF_DISPLAY1, HI_DRV_DISP_STEREO_NONE,  enFormat);
		if (HI_SUCCESS != ret)
		{
			D_ERROR("call HI_MPI_DISP_SetFormat failed.\n");
			return DFB_FAILURE;
		...
		{
			ret = HI_MPI_DISP_SetFormat(HI_UNF_DISPLAY0, HI_DRV_DISP_STEREO_NONE, HI_UNF_ENC_FMT_NTSC);
			if (HI_SUCCESS != ret)
			{
				D_ERROR("call HI_MPI_DISP_SetFormat failed.\n");
				return DFB_FAILURE;
		...
		{
			ret = HI_MPI_DISP_SetFormat(HI_UNF_DISPLAY0, HI_DRV_DISP_STEREO_NONE,  HI_UNF_ENC_FMT_PAL);
			if (HI_SUCCESS != ret)
			{
				D_ERROR("call HI_MPI_DISP_SetFormat failed.\n");
				return DFB_FAILURE;
		...
		/* refer to HI_UNF_DISP_SetFormat. set hdmi .. */
		//(HI_VOID)HI_MPI_HDMI_SetFormat(enFormat);
	}

	/* adjustment */
	if (config->flags & DSECONF_ADJUSTMENT) {
		if (config->adjustment.flags & DCAF_BRIGHTNESS) {
			if (HI_SUCCESS != HI_MPI_DISP_SetBrightness(HI_UNF_DISPLAY0, (HI_U32)config->adjustment.brightness)) {
				return DFB_FAILURE;
			}
		}

		if (config->adjustment.flags & DCAF_CONTRAST) {
			if (HI_SUCCESS != HI_MPI_DISP_SetContrast(HI_UNF_DISPLAY0, (HI_U32)config->adjustment.contrast)) {
				return DFB_FAILURE;
			}
		}

		if (config->adjustment.flags & DCAF_HUE) {
			if (HI_SUCCESS != HI_MPI_DISP_SetHuePlus(HI_UNF_DISPLAY0, (HI_U32)config->adjustment.hue)) {
				return DFB_FAILURE;
			}
		}

		if (config->adjustment.flags & DCAF_SATURATION) {
			if (HI_SUCCESS != HI_MPI_DISP_SetSaturation(HI_UNF_DISPLAY0, (HI_U32)config->adjustment.saturation)) {
				return DFB_FAILURE; 
			}
		}
	}
		...
		int                         *ret_height )
{
	...
	HI_DRV_DISP_STEREO_MODE_E stereo;
	int i, cnt = sizeof(map) / sizeof(map[0]);


	if(HI_MPI_DISP_GetFormat(HI_UNF_DISPLAY1, &stereo, &eFormat)) {
		D_ERROR("%s->%d, fail to call HI_MPI_DISP_GetFormat\n",__func__,__LINE__);
		return DFB_FAILURE;
	}
	D_ERROR("%s->%d, call HI_MPI_DISP_GetFormat, fmt:%d\n",__func__,__LINE__,eFormat);
	...
	switch (map[i].res) {
		case DSOR_1920_1080:
			*ret_width = 1920;
			*ret_height = 1080;
			D_ERROR("%s->%d, 1920 * 1080!\n",__func__,__LINE__);

			break;
		case DSOR_1280_720:
			*ret_width = 1280;
			*ret_height = 720;
			D_ERROR("%s->%d, 1280 * 720!\n",__func__,__LINE__);
