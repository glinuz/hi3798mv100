...
)
{
    ...
    COM_CHECK_PARAM(HI_NULL_PTR == pstSection);
    COM_CHECK_PARAM(0 == u32SectionDataLen);

    pu8ReadPos = pu8SectionData;

    HI_READ_08BIT(pu8ReadPos, pstSection->u8TableID, u32TotalRead);
    HI_READ_12BIT(pu8ReadPos, pstSection->u16SectionLength, u32TotalRead);

    if (pstSection->u16SectionLength + u32TotalRead != u32SectionDataLen)
    {
        COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
        return COM_ERR_IMAGE_ILLEGAL;
    }

    HI_READ_16BIT(pu8ReadPos, pstSection->u16TableIDExtension, u32TotalRead);
    HI_READ_08BIT(pu8ReadPos, pstSection->u8SoftwareVersion,   u32TotalRead);
    HI_READ_08BIT(pu8ReadPos, pstSection->u8SectionNumber,     u32TotalRead);
    HI_READ_08BIT(pu8ReadPos, pstSection->u8LastSectionNumber, u32TotalRead);

    pstSection->u32PrivateDataLen = u32SectionDataLen - (u32TotalRead + 4);
    pstSection->pu8PrivateData    = pu8ReadPos;

    HI_SKIP_BYTES(pu8ReadPos, pstSection->u32PrivateDataLen, u32TotalRead);
    HI_READ_32BIT(pu8ReadPos, pstSection->u32CRC32, u32TotalRead);
    ...
    u32CRC32 = COM_CRC32(0xffffffffL, pu8SectionData, u32SectionDataLen - 4);
    if (pstSection->u32CRC32 != u32CRC32)
    {
        COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
        return COM_ERR_IMAGE_ILLEGAL;
    }
