...
#obj_prefix :=$(TARGET_OUT_OBJ_DIR)/$(LOCAL_MODULE).SHARED
obj_prefix :=$(TARGET_OUT_OBJ_DIR)/$(patsubst $(mediaos_dir)/%,%,$(LOCAL_PATH))
#lib_prefix :=$(TARGET_OUT_LIB_DIR)/share
ifeq ($(LOCAL_MODULE_PATH),)
lib_prefix :=$(TARGET_OUT_LIB_DIR)
else
lib_prefix :=$(LOCAL_MODULE_PATH)
endif
...
LOCAL_INTERMEDIATE_TARGETS := $(target_all_objs) $(target_all_source) \
                              ...
                              $(lib_prefix)/$(LOCAL_MODULE)$(LOCAL_MODULE_SUFFIX)-deps \
...
$(LOCAL_MODULE)-deps:$(lib_prefix)/$(LOCAL_MODULE)$(LOCAL_MODULE_SUFFIX)-deps
...
$(LOCAL_MODULE)-cfg-clean:$(LOCAL_MODULE)-t-shlib-clean
$(LOCAL_MODULE)-prepare-clean:$(LOCAL_MODULE)-t-shlib-clean

$(lib_prefix)/$(LOCAL_MODULE)$(LOCAL_MODULE_SUFFIX)-deps:$(all_required_modules) $(target_all_objs)
	$(NS)echo "<<< compile shared lib $(PRIVATE_LIB_DIR)/$(PRIVATE_LOCAL_MODULE)$(PRIVATE_MODULE_SUFFIX):"
	$(NS)mkdir -p $(dir $(PRIVATE_LIB_DIR)/$(PRIVATE_LOCAL_MODULE)$(PRIVATE_MODULE_SUFFIX))
	$(NS)$(PRIVATE_CXX) -shared \
	-L$(PRIVATE_TARGET_LD_STATIC_DIRS) \
	-o $(PRIVATE_LIB_DIR)/$(PRIVATE_LOCAL_MODULE)$(PRIVATE_MODULE_SUFFIX) \
	$(PRIVATE_ALL_OBJS) \
        -Wl,--whole-archive \
	...
	$(PRIVATE_LDFLAGS) \
	$(PRIVATE_LDLIBS) $(PRIVATE_STL_LDFLAGS)

$(lib_prefix)/$(LOCAL_MODULE)$(LOCAL_MODULE_SUFFIX):$(target_all_objs)
	$(NS)echo "<<< compile shared lib $@:"
	$(NS)mkdir -p $(dir $@)
	$(NS)$(PRIVATE_CXX) -shared \
	-L$(PRIVATE_TARGET_LD_STATIC_DIRS) \
	-o $@ \
