    ...
    const gchar * elementname, GError ** error)
{
  ...
  GST_DEBUG ("type:%d, elementname:%s", type, elementname);
  ...
  if (!possibilities) {
    //GST_DEBUG ("No %s for URI '%s'", type == GST_URI_SINK ? "sink" : "source",
        //uri);
    GST_DEBUG ("No %s for URI", type == GST_URI_SINK ? "sink" : "source");
  ...
  while (walk) {
    ...
    if (ret != NULL) {
      ...
      if(uri_err){
        ...
        GST_WARNING ("%s didn't accept URI: %s", GST_OBJECT_NAME (ret),
            uri_err->message ? uri_err->message : "unknow msg");
  ...
  GST_LOG_OBJECT (ret, "created %s for URL",
      type == GST_URI_SINK ? "sink" : "source");
