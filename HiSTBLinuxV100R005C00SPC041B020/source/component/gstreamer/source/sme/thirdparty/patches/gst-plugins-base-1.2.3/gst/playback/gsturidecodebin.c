...
struct _GstURIDecodeBin
{
  ...
  gint high_percent;
#endif
  gint is_subtile;//0:means this uridecoder is not for subtile, others means it is for subtile.
...
#ifndef __LINUX__
 g_object_class_install_property (gobject_class, PROP_DECOUT_INTERVAL,
  ...
  g_object_class_install_property (gobject_class, PROP_DECIN_BUF_DURATION,
  ...
  /**
   * GstURIDecodeBin::bitstream-pad
   *
   * Get the peer-pad of bitstream pad
   *
   */
  g_object_class_install_property (gobject_class, PROP_ACTIVE_BITSTREAM_PAD,
      g_param_spec_object ("active-bitstream-pad", "Active bitstream pad",
          "The currently active bitstream src pad", GST_TYPE_PAD,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  ...
  gst_uri_decode_bin_signals[SIGNAL_MANIFEST_PARSE_COMPLETE] =
...
gst_uri_decode_bin_init (GstURIDecodeBin * dec)
{
  ...
  dec->active_audio_pad = NULL;

  dec->byte_range = TRUE;
  dec->cookies = NULL;
  dec->referer = NULL;
  dec->user_agent = NULL;
  dec->demux_proc_info = NULL;
  dec->vdec_proc_info = NULL;
  dec->adec_proc_info = NULL;
...
gst_uri_decode_bin_finalize (GObject * obj)
{
  ...
  if (dec->active_audio_pad) {
    gst_object_unref (dec->active_audio_pad);
    dec->active_audio_pad = NULL;
  }

  g_free (dec->user_agent);
  dec->user_agent = NULL;
  g_free (dec->referer);
  dec->referer = NULL;
  if (dec->cookies) {
    g_strfreev (dec->cookies);
    dec->cookies = NULL;
  }
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_CONNECTION_SPEED:
      ...
      GST_WARNING_OBJECT(dec,"playbin->connection_speed:%lld",dec->connection_speed);
      GST_OBJECT_UNLOCK (dec);
      if(dec->decodebins){
        GList* walk;
        for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
          GObject *decodebin = G_OBJECT (walk->data);
          g_object_set(decodebin, "connection-speed", g_value_get_uint64 (value), NULL);
        }
      }
      break;
    case PROP_CAPS:
    ...
    case PROP_EXIT_BLOCK:{
      ...
      if(dec->decodebins){
        ...
        for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
          GObject *decodebin = G_OBJECT (walk->data);
          g_object_set(decodebin, "gst-sme-prop-key-exit-block", (gint)exit_block, NULL);
        }
      }
      break;
    }
    ...
    case PROP_SURFACE:{
      ...
      dec->vdisp_surface = surface_handle;
      if(dec->decodebins){
        GList* walk;
        for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
          GObject *decodebin = G_OBJECT (walk->data);
    ...
    case PROP_USE_BUFFERING:
      ...
      GST_INFO_OBJECT(dec, "gsturidecbin set property use_buffering=%d", dec->use_buffering);
      break;
    ...
    case PROP_ACTIVE_AUDIO_PAD:{
      ...
      GST_INFO_OBJECT (dec, "set active pad %s", GST_PAD_NAME(pad));
      if (pad) {
        if (dec->active_audio_pad) {
          gst_object_unref(dec->active_audio_pad);
          dec->active_audio_pad = NULL;
        ...
        /* store active audio pad in order to get active bitstream pad */
        dec->active_audio_pad = gst_object_ref(pad);

        target = gst_ghost_pad_get_target(GST_GHOST_PAD_CAST(pad));
        GST_INFO_OBJECT (dec, "get target pad %s", GST_PAD_NAME(target));
        if (target) {
          decodebin = gst_pad_get_parent (target);
          GST_INFO_OBJECT (dec, "get parent element %s",
              GST_ELEMENT_NAME(decodebin));
          if (decodebin) {
            g_object_set (decodebin, "active-audio-pad", target, NULL);
    ...
    case PROP_USER_AGENT:
      if (dec->user_agent)
          g_free (dec->user_agent);
      dec->user_agent = g_value_dup_string (value);
      break;
    case PROP_REFERER:
      if (dec->referer)
          g_free (dec->referer);
      dec->referer = g_value_dup_string (value);
      break;
    case PROP_COOKIES:
      g_strfreev (dec->cookies);
      dec->cookies = g_strdupv (g_value_get_boxed (value));
      break;
    case PROP_BYTE_RANGE:
      dec->byte_range = g_value_get_boolean (value);
      break;
    case PROP_SME_DEMUX_INFO:
      dec->demux_proc_info = g_value_get_pointer (value);
      if(dec->decodebins){
        GList* walk;
        for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
          GObject *decodebin = G_OBJECT (walk->data);
          g_object_set (decodebin, "sme-demux-info", dec->demux_proc_info, NULL);
        }
      }
      break;
    case PROP_VDEC_INFO:
      dec->vdec_proc_info = g_value_get_pointer (value);
      if(dec->decodebins){
        GList* walk;
        for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
          GObject *decodebin = G_OBJECT (walk->data);
          g_object_set (decodebin, "video-decoder-info", dec->vdec_proc_info, NULL);
        }
      }
      break;
    case PROP_ADEC_INFO:
      dec->adec_proc_info = g_value_get_pointer (value);
      if(dec->decodebins){
        GList* walk;
        for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
          GObject *decodebin = G_OBJECT (walk->data);
          g_object_set (decodebin, "audio-decoder-info", dec->adec_proc_info, NULL);
        }
      }
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_ACTIVE_BITSTREAM_PAD: {
      GstPad *pad, *target, *dbin_bitstream_pad, *uridbin_bitstream_pad;
      GstObject *decodebin;

      pad = dec->active_audio_pad;
      GST_INFO_OBJECT (dec, "get active bitstream pad %s", GST_PAD_NAME(pad));
      if (pad) {
        target = gst_ghost_pad_get_target(GST_GHOST_PAD_CAST(pad));
        GST_INFO_OBJECT (dec, "get target pad %s", GST_PAD_NAME(target));
        if (target) {
          ...
          if (decodebin) {
            g_object_get (decodebin,
                "active-bitstream-pad", &dbin_bitstream_pad, NULL);
            if (dbin_bitstream_pad) {
              GstObject *uridbin;
              GstPad *internal_pad;

              internal_pad = gst_proxy_pad_get_internal(dbin_bitstream_pad);
              uridbin_bitstream_pad = gst_pad_get_peer(internal_pad);

              uridbin = gst_pad_get_parent (uridbin_bitstream_pad);
              GST_INFO_OBJECT (dec, "get bitstream pad %s, parent %s",
                  GST_PAD_NAME(uridbin_bitstream_pad),
                  GST_ELEMENT_NAME(uridbin));

              g_value_set_object (value, uridbin_bitstream_pad);

              gst_object_unref (uridbin);
              gst_object_unref (uridbin_bitstream_pad);
        ...
        GST_WARNING_OBJECT (dec, "no active audio pad");
      }
      break;
    }
    case PROP_USER_AGENT:
      g_value_set_string (value, dec->user_agent);
      break;
    case PROP_REFERER:
      g_value_set_string (value, dec->referer);
      break;
    case PROP_COOKIES:
      g_value_set_boxed (value, dec->cookies);
      break;
    case PROP_BYTE_RANGE:
      g_value_set_boolean (value, dec->byte_range);
      break;
#ifndef __LINUX__
case PROP_DECIN_BUF_DURATION:{
      ...
      if(dec->decodebins){
        ...
        for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
          GObject *decodebin = G_OBJECT (walk->data);
          g_object_get(decodebin, "gst-sme-prop-key-hivpipe-decin-duration",
            &decin_duration, NULL);
          break;
        }
      }
    ...
    case PROP_DECOUT_INTERVAL:{
      ...
      if(dec->decodebins){
        ...
        for (walk = dec->decodebins; walk; walk = g_slist_next (walk)) {
          GObject *decodebin = G_OBJECT (walk->data);
          g_object_get(decodebin, "gst-sme-prop-key-hivpipe-dec-interval",
            &u_cur_interval, NULL);
          break;
        }
      }
...
{
   g_signal_emit(decoder,gst_uri_decode_bin_signals[SIGNAL_MANIFEST_PARSE_COMPLETE],0,input,result,NULL);
}
...
gen_source_element (GstURIDecodeBin * decoder)
{
  ...
  GstElement *source;
  GParamSpec *pspec, *puser, *pcookies, *preferer, *pbyterange;
  GstQuery *query;
  ...
  GST_LOG_OBJECT (decoder, "finding source...");
  ...
  /** BEGIN: Added for DTS2015092408805  by baihaili(b00311320), 2015/9/25 */
  source_class = G_OBJECT_GET_CLASS (source);
  protocol = gst_uri_get_protocol(decoder->uri);
  if (protocol && (!strncmp (protocol, "http", 4) ||
                   !strncmp (protocol, "https", 5)))  {
    pspec = g_object_class_find_property (source_class, "timeout");
    puser = g_object_class_find_property (source_class, "user-agent");
    pcookies = g_object_class_find_property (source_class, "cookies");
    preferer = g_object_class_find_property (source_class, "referer");
    pbyterange = g_object_class_find_property (source_class, "byte-range");
    if (pspec) {
      GST_INFO_OBJECT (decoder, "configuring timeout");
      g_object_set (source, "timeout", (guint)16, NULL);
    }

    if (puser) {
      GST_INFO_OBJECT (decoder, "configuring user-agent");
      g_object_set (source, "user-agent", decoder->user_agent, NULL);
    }

    if (pcookies) {
      GST_INFO_OBJECT (decoder, "configuring cookies");
      if (decoder->cookies)
        g_object_set (source, "cookies", decoder->cookies, NULL);
    }

    if (preferer) {
      GST_INFO_OBJECT (decoder, "configuring referer");
      g_object_set (source, "referer", decoder->referer, NULL);
    }

    if (pbyterange) {
      GST_INFO_OBJECT (decoder, "configuring byte-range");
      g_object_set (source, "byte-range", decoder->byte_range, NULL);
    }
  }
  if (protocol)
      g_free(protocol);
  /** END: Added for DTS2015092408805 by baihaili(b00311320), 2015/9/25 */
...
make_decoder (GstURIDecodeBin * decoder)
{
  ...
  if (decoder->pending_decodebins) {
    ...
    g_object_set(decodebin, "user-agent", decoder->user_agent, NULL);
    if (decoder->cookies)
      g_object_set(decodebin, "cookies", decoder->cookies, NULL);
    g_object_set(decodebin, "referer", decoder->referer, NULL);
    ...
    g_signal_connect (decodebin,
        "manifest-parse-complete", G_CALLBACK (manifest_parsing_complete_cb), decoder);
  }
...
#else
  GST_INFO_OBJECT(decoder, "make decoder,is_stream=%d,usebuffering=%d,max-size=%u,buf-dur=%llu",
    decoder->is_stream, decoder->use_buffering, decoder->buffer_size,
    decoder->buffer_duration);
  ...
  g_object_set (decodebin, "use-buffering", decoder->use_buffering, NULL);
  {
    guint max_bytes = decoder->buffer_size;
    guint64 max_time = decoder->buffer_duration;
    ...
    g_object_set (decodebin,
        "low-percent", decoder->low_percent,
        "high-percent", decoder->high_percent, NULL);
  }
  ...
  g_object_set(decodebin,
               "gst-sme-prop-key-vdisp-surface",
               (gint)decoder->vdisp_surface, NULL);
#endif
    ...
    GstCaps * caps, GstURIDecodeBin * decoder)
{
  ...
  GST_INFO_OBJECT (decoder, "typefind found caps %, media_type:%s, decoder->is_subtile=%d" GST_PTR_FORMAT,
    caps, media_type, decoder->is_subtile);

  /* BEGIN: Added for DTS2015092309761 by liurongliang(l00180035), 2015/9/28 */
  if(decoder->is_subtile && media_type){
    if(strncmp(media_type, "text/", 5)
      && strncmp(media_type, "application/x-subtitle", strlen("application/x-subtitle"))
      && strncmp(media_type, "application/x-subtitle-tmplayer", strlen("application/x-subtitle-tmplayer"))
      && strncmp(media_type, "application/x-subtitle-sami", strlen("application/x-subtitle-sami"))
      && strncmp(media_type, "application/x-subtitle-mpl2", strlen("application/x-subtitle-mpl2"))
      && strncmp(media_type, "application/x-subtitle-dks", strlen("application/x-subtitle-dks"))
      && strncmp(media_type, "application/x-subtitle-qttext", strlen("application/x-subtitle-qttext"))){
      GST_ELEMENT_ERROR (decoder, STREAM, SUBTILE,
        ("subtile but typed for:%s", media_type), (NULL));
      return;
    }
  }
  /* END:   Added for DTS2015092309761 by liurongliang(l00180035), 2015/9/28 */
...
#else
  g_object_set (queue, "max-size-bytes", 3*1024*1024, NULL);
#endif
...
setup_source (GstURIDecodeBin * decoder)
{
  ...
  if (is_dynamic) {
    ...
    if (decoder->is_stream) {
      ...
      GST_INFO_OBJECT (decoder, "Plugging decodebin to source, is_subtile=%d",
        decoder->is_subtile);
      ...
      /* BEGIN: Added for DTS2015092309761 by liurongliang(l00180035), 2015/9/28 */
      if(decoder->is_subtile){
        g_object_set (dec_elem, "gst-sme-prop-key-is-subtitle", 1, NULL);
      }
      /* END:   Added for DTS2015092309761 by liurongliang(l00180035), 2015/9/28 */
