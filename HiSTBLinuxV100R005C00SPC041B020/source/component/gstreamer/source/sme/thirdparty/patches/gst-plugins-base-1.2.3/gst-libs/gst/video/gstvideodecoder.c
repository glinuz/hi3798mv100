    ...
    GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    {
      GstFlowReturn flow_ret = GST_FLOW_OK;
      GST_INFO("videodecoder receive EOS!");
    ...
    //some decoder api of input is block , we should flush the decoder so that it cann unblock.
    //so we should do fulsh deocder in flush-start.
#if 1
    case GST_EVENT_FLUSH_START:
    {
      GstVideoDecoderClass *klass = GST_VIDEO_DECODER_GET_CLASS (decoder);
      /*
        first we push flush start to downstream.
        then we should flush decoder,so chainfunc of sinkpad will unblock.
      */
      GST_WARNING_OBJECT(decoder, "%s:flush-start v-dec begin...",
                SMEPLAYER_STAT_TAG_TIME);
      GST_WARNING_OBJECT(decoder, "%s:GST_EVENT_FLUSH_START v-dec begin...",
          SMEPLAYER_STAT_TAG_TIME);
      ret = gst_video_decoder_push_event (decoder, event);
      GST_WARNING_OBJECT(decoder, "%s:GST_EVENT_FLUSH_START v-dec end(%d)",
          SMEPLAYER_STAT_TAG_TIME, ret);
      event = NULL;

      GST_VIDEO_DECODER_STREAM_LOCK (decoder);
      /* well, this is kind of worse than a DISCONT */
      GST_WARNING_OBJECT(decoder, "%s:reset v-dec begin...",
          SMEPLAYER_STAT_TAG_TIME);
      /* Inform subclass */
      if (klass->reset) {
        GST_FIXME_OBJECT (decoder, "GstVideoDecoder::reset() is deprecated");
        klass->reset (decoder, TRUE);
      }
      GST_WARNING_OBJECT(decoder, "%s:reset v-dec end",
          SMEPLAYER_STAT_TAG_TIME);

      GST_WARNING_OBJECT(decoder, "%s:flush v-dec begin...",
          SMEPLAYER_STAT_TAG_TIME);

      if (klass->flush)
        klass->flush (decoder);

      GST_WARNING_OBJECT(decoder, "%s:flush v-dec end",
          SMEPLAYER_STAT_TAG_TIME);

      GST_VIDEO_DECODER_STREAM_UNLOCK (decoder);
      GST_WARNING_OBJECT(decoder, "%s:flush-start v-dec end",
          SMEPLAYER_STAT_TAG_TIME);
      break;
    }
    case GST_EVENT_FLUSH_STOP:
    {
      GST_WARNING_OBJECT(decoder, "%s:flush-stop v-dec begin...",
          SMEPLAYER_STAT_TAG_TIME);
      /* and get (re)set for the sequel */
      gst_video_decoder_reset (decoder, FALSE, TRUE);
       ...
       */
      forward_immediate = TRUE;
      GST_WARNING_OBJECT(decoder, "%s:flush-stop v-dec end",
          SMEPLAYER_STAT_TAG_TIME);
      break;
    }
#else
    ...
    GstEvent * event)
{
  ...
  GST_INFO_OBJECT (decoder, "received event %d, %s", GST_EVENT_TYPE (event),
...
not_negotiated:
  {
    GST_ELEMENT_WARNING(decoder, CORE, NEGOTIATION, (NULL),
...
gst_video_decoder_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      ...
      GST_WARNING_OBJECT(decoder, "%s:v-dec open begin...", SMEPLAYER_STAT_TAG_TIME);
      if (decoder_class->open && !decoder_class->open (decoder))
        goto open_failed;
      GST_WARNING_OBJECT(decoder, "%s:v-dec open end", SMEPLAYER_STAT_TAG_TIME);
      break;
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      /* Initialize device/library if needed */
      GST_WARNING_OBJECT(decoder, "%s:v-dec start begin...", SMEPLAYER_STAT_TAG_TIME);
      if (decoder_class->start && !decoder_class->start (decoder))
        goto start_failed;
      GST_WARNING_OBJECT(decoder, "%s:v-dec start end", SMEPLAYER_STAT_TAG_TIME);
      GST_VIDEO_DECODER_STREAM_LOCK (decoder);
      GST_WARNING_OBJECT(decoder, "%s:v-dec reset begin...", SMEPLAYER_STAT_TAG_TIME);
      gst_video_decoder_reset (decoder, TRUE, TRUE);
      GST_WARNING_OBJECT(decoder, "%s:v-dec reset end", SMEPLAYER_STAT_TAG_TIME);
      GST_VIDEO_DECODER_STREAM_UNLOCK (decoder);
      break;
    default:
      break;
  ...
  switch (transition) {
    case GST_STATE_CHANGE_PAUSED_TO_READY:{
      ...
      if (decoder_class->stop) {
        GST_WARNING_OBJECT(decoder, "%s:v-dec stop begin...", SMEPLAYER_STAT_TAG_TIME);
        stopped = decoder_class->stop (decoder);
        GST_WARNING_OBJECT(decoder, "%s:v-dec stop end", SMEPLAYER_STAT_TAG_TIME);
    ...
    case GST_STATE_CHANGE_READY_TO_NULL:
      ...
      GST_WARNING_OBJECT(decoder, "%s:v-dec close begin...", SMEPLAYER_STAT_TAG_TIME);
      if (decoder_class->close && !decoder_class->close (decoder))
        goto close_failed;
      GST_WARNING_OBJECT(decoder, "%s:v-dec close end", SMEPLAYER_STAT_TAG_TIME);
      break;
    default:
      break;
  }
    ...
    decoder, GstVideoCodecFrame * frame, gboolean dropping)
{
  ...
  if (dropping || !decoder->priv->output_state) {
    ...
    for (l = g_list_last (decoder->priv->pending_events); l;
        l = g_list_previous (l)) {
      GST_INFO_OBJECT (decoder, "pushing %s event",
          GST_EVENT_TYPE_NAME (l->data));
      /* BEGIN: Added by liurongliang(l00180035), 2015/1/8 */
      //push segment by dec out time.
      if((GST_EVENT_TYPE(l->data) == GST_EVENT_SEGMENT) && (NULL != frame->output_buffer)){
        GstSegment segment;
        GstSegment segment_in;
        gst_event_copy_segment ((GstEvent *)(l->data), &segment_in);
        gst_segment_init(&segment, GST_FORMAT_TIME);
        if(segment_in.start < GST_BUFFER_PTS(frame->output_buffer)){
          if(decoder->priv->input_state && !decoder->priv->input_state->info.is_frame
             && GST_BUFFER_PTS_IS_VALID(frame->output_buffer)){
            segment.position   = GST_BUFFER_PTS(frame->output_buffer);
            segment.time       = 0;
            segment.start      = GST_BUFFER_PTS(frame->output_buffer);
            GST_WARNING_OBJECT(decoder, "video segment start use dec out pts");
          } else {
            segment.position   = segment_in.start;
            segment.time       = 0;
            segment.start      = segment_in.start;
          }
        }else{
          segment.position   = GST_BUFFER_PTS(frame->output_buffer);
          segment.time       = GST_BUFFER_PTS(frame->output_buffer);
          segment.start      = GST_BUFFER_PTS(frame->output_buffer);
        }
        segment.format     = GST_FORMAT_TIME;
        GST_WARNING_OBJECT(decoder, "%s:v-dec-out segment[%lld:%lld];[%lld]",
            SMEPLAYER_STAT_TAG_TIME, segment.start, segment.stop, segment_in.start);
        gst_video_decoder_push_event (decoder, gst_event_new_segment(&segment));
        gst_event_unref(l->data);
        l->data = NULL;
      }else{
        gst_video_decoder_push_event (decoder, l->data);
      }
      /* END:   Added by liurongliang(l00180035), 2015/1/8 */
    }
...
gst_video_decoder_drop_frame (GstVideoDecoder * dec, GstVideoCodecFrame * frame)
{
  ...
  GstSegment *segment;
#if 0   // fix maybe cause memleak
  GstMessage *qos_msg;
#endif
  gdouble proportion;

  GST_INFO_OBJECT (dec, "drop frame %p,pts=%lld", frame, frame->pts);
    ...
    GstVideoCodecFrame * frame)
{
  ...
  GST_DEBUG_OBJECT (decoder, "finish frame %p", frame);
  ...
  /* BEGIN: Modified by liurongliang(l00180035), 2014/12/27 */
  if(!GST_CLOCK_TIME_IS_VALID(GST_BUFFER_PTS (output_buffer))){
    GST_BUFFER_PTS (output_buffer) = frame->pts;
  }
  if(!GST_CLOCK_TIME_IS_VALID(GST_BUFFER_DTS (output_buffer))){
    GST_BUFFER_DTS (output_buffer) = frame->dts;
  }
  /* END:   Modified by liurongliang(l00180035), 2014/12/27 */

  if(!GST_CLOCK_TIME_IS_VALID(GST_BUFFER_DURATION (output_buffer))
    && GST_CLOCK_TIME_IS_VALID(GST_BUFFER_DURATION (frame->duration))){
    GST_BUFFER_DURATION (output_buffer) = frame->duration;
  }else if(!GST_CLOCK_TIME_IS_VALID(GST_BUFFER_DURATION (output_buffer))){
    GST_WARNING_OBJECT(decoder, "buffer:pts:%lld,duration is invalid",
...
#else
  //we do not clip here, and it will clip in sink.
#endif
    ...
    GstQuery * query)
{
  ...
  if (pool == NULL) {
    ...
    GST_INFO_OBJECT (decoder, "no pool, making new pool");
...
gst_video_decoder_negotiate_default (GstVideoDecoder * decoder)
{
  ...
  GST_INFO_OBJECT (decoder, "output_state par %d/%d fps %d/%d",
      state->info.par_n, state->info.par_d,
      state->info.fps_n, state->info.fps_d);
  ...
  GST_INFO_OBJECT (decoder, "setting caps %" GST_PTR_FORMAT, state->caps);
  ...
  if (!gst_pad_peer_query (decoder->srcpad, query)) {
    GST_INFO_OBJECT (decoder, "didn't get downstream ALLOCATION hints");
  }

  g_assert (klass->decide_allocation != NULL);
  ret = klass->decide_allocation (decoder, query);

  GST_INFO_OBJECT (decoder, "ALLOCATION (%d) params: %" GST_PTR_FORMAT, ret,
      query);
  ...
  if (gst_query_get_n_allocation_params (query) > 0) {
    gst_query_parse_nth_allocation_param (query, 0, &allocator, &params);
    GST_INFO_OBJECT (decoder, "allocation_param,allcator(%p)", allocator);
  } else {
    allocator = NULL;
    gst_allocation_params_init (&params);
    GST_INFO_OBJECT (decoder, "allocation_param none,allocator = NULL !");
  }
