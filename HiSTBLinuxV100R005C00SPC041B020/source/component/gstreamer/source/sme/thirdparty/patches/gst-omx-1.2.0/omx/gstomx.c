    ...
    const gchar * component_name, const gchar * component_role, guint64 hacks)
{
  ...
  /* BEGIN: Added by liurongliang(l00180035), 2015/3/26 */
  if(NULL != component_role){
    if ((dot = g_strrstr (component_role, ".")))
        comp->name = g_strdup (dot + 1);
    else
      comp->name = g_strdup (component_role);
  }else{
  ...
  /* END:   Added by liurongliang(l00180035), 2015/3/26 */
    ...
    OMX_PARAM_PORTDEFINITIONTYPE * port_def)
{
  ...
  GST_INFO_OBJECT (comp->parent, "Updated %s port %u definition: %s (0x%08x),"
      "portdir=%d[%d:%d],format=%d,setformat=%d,enable=%d",
      ...
      port_def?port_def->format.video.eCompressionFormat:-1,
...
gst_omx_port_release_buffer (GstOMXPort * port, GstOMXBuffer * buf)
{
  ...
  if (port->port_def.eDir == OMX_DirInput) {
  if(0){
        FILE* fp = fopen("/mnt/nfs/omx.es", "ab");
        fwrite(buf->omx_buf->pBuffer, 1 , buf->omx_buf->nFilledLen, fp);
        fclose(fp);
    }
...
plugin_init (GstPlugin * plugin)
{
  ...
  GST_ERROR("gst-omx-plugin,plugin_init in");
  g_printf("gst-omx-plugin,plugin_init in\n");
  ...
  if (!g_key_file_load_from_dirs (config, *config_name,
    ...
    g_free (paths);
    g_error_free (err);
    GST_ERROR("gst-omx-plugin,load conf file failed");
    goto done;
  }
  ...
  for (i = 0; i < n_elements; i++) {
    ...
    GST_INFO("gst-omx-plugin load,type-name=%s", type_name);
