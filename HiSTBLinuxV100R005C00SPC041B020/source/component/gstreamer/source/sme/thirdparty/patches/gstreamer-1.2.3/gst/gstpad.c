...
gst_pad_init (GstPad * pad)
{
  ...
  GST_INFO("=== pad init:%s,pad:%p", GST_OBJECT_NAME(pad)?GST_OBJECT_NAME(pad):"unkown_pad", pad);
...
gst_pad_finalize (GObject * object)
{
  ...
  GST_INFO("=== pad finalize :%s, pad:%p", GST_OBJECT_NAME(pad)?GST_OBJECT_NAME(pad):"unkown_pad", pad);
...
gst_pad_activate_mode (GstPad * pad, GstPadMode mode, gboolean active)
{
  ...
  if (active && old != mode && old != GST_PAD_MODE_NONE) {
    ...
    /* BEGIN: Added by liurongliang(l00180035), 2015/5/27 */
    //we should update the last pad mode here.
    old = GST_PAD_MODE(pad);
    /* END:   Added by liurongliang(l00180035), 2015/5/27 */
...
push_sticky (GstPad * pad, PadEvent * ev, gpointer user_data)
{
  ...
  switch (data->ret) {
    ...
    case GST_FLOW_NOT_LINKED:
      ...
      GST_INFO_OBJECT (pad, "pad was not linked, mark pending");
      if (GST_EVENT_TYPE (event) != GST_EVENT_EOS)
        data->ret = GST_FLOW_OK;
      GST_OBJECT_FLAG_SET (pad, GST_PAD_FLAG_PENDING_EVENTS);
      break;
    default:
      GST_INFO_OBJECT (pad, "result %s, mark pending events",
...
flushed:
  {
    GST_INFO_OBJECT (pad, "We're flushing");
    GST_OBJECT_UNLOCK (pad);
    gst_event_unref (event);
    return FALSE;
  }
