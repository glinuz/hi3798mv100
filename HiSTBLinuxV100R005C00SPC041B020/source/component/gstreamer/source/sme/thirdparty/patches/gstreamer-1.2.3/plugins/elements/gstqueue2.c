...
gst_queue2_have_data (GstQueue2 * queue, guint64 offset, guint length)
{
  ...
  if ((range = find_range (queue, offset))) {
    ...
    GST_DEBUG_OBJECT (queue, "cur_level.bytes %u (max %" G_GUINT64_FORMAT ")",
...
gst_queue2_create_write (GstQueue2 * queue, GstBuffer * buffer)
{
  ...
  while (size > 0) {
    ...
    if (new_writing_pos > writing_pos) {
      ...
      if (block_one > 0) {
        GST_DEBUG_OBJECT (queue, "writing %u bytes", block_one);
      ...
      if (block_two > 0) {
        GST_DEBUG_OBJECT (queue, "writing %u bytes", block_two);
        if (QUEUE_IS_USING_TEMP_FILE (queue)) {
