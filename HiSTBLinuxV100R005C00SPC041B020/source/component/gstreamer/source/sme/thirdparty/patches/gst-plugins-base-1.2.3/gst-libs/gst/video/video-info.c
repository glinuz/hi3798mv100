...
gst_video_info_from_caps (GstVideoInfo * info, const GstCaps * caps)
{
  ...
  // get rotation from caps
  if (gst_structure_get_uint(structure, "rotate", &rotation)) {
    info->rotation = rotation;
    GST_INFO("demux pass rotation: %d", info->rotation);
  }else {
    info->rotation = 0;
  }
  /* END:  Added by lidanjing(l00346954),  2015/9/1 */

  /*parse is_frame flag from caps*/
  if (gst_structure_get_boolean (structure, "is_frame", &is_frame)) {
    info->is_frame = is_frame;
    GST_INFO("demux pass is_frame: %d", info->is_frame);
  }else {
    GST_WARNING("demux pass is_frame error,maybe decoder can not work normal,we set default: is_frame=true");
  }
