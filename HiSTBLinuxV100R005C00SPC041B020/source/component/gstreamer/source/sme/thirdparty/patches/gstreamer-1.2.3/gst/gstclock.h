...
/**
 * GST_VALID_TIME_AS_SECONDS:
 * @time: the time
 *
 * Convert a #GstClockTime to seconds, if valid div GST_SECOND
 */
#define GST_VALID_TIME_AS_SECONDS(time)  (GST_CLOCK_TIME_IS_VALID(time) ? (time) / GST_SECOND : time)
/**
 * GST_VALID_TIME_AS_MSECONDS:
 * @time: the time
 *
 * Convert a #GstClockTime to milliseconds (1/1000 of a second),if valid div GST_MSECOND.
 */
#define GST_VALID_TIME_AS_MSECONDS(time) (GST_CLOCK_TIME_IS_VALID(time) ? (time) / GST_MSECOND : time)
/**
 * GST_VALID_TIME_AS_USECONDS:
 * @time: the time
 *
 * Convert a #GstClockTime to microseconds (1/1000000 of a second),if valid div GST_USECOND.
 */
#define GST_VALID_TIME_AS_USECONDS(time) (GST_CLOCK_TIME_IS_VALID(time) ? (time) / GST_USECOND : time)
...
/*
 * gst_clock_clear_last_time:
 * @clock: a #GstClock to use
 *
 * clear the last time to Zero.for some case, we use the same clock obj to play
 * other stream. example:for DVB Channel change, the clock will not destroy(for
 * speed up CC time-cost), but we should clear the clock.
 *
 *
 * Returns: void
*/
void gst_clock_clear_last_time(GstClock * clock);
