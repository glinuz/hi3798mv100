...
#if defined (ALSA_DUMP)
  alsa->dump_file = fopen("/mnt/dump/alsa.pcm", "wb");
  if (alsa->dump_file == NULL) {
    GST_ERROR("fail to open file /mnt/dump/alsa.pcm");
  }
#endif
...
#if defined (ALSA_DUMP)
  if (alsa->dump_file) {
    fwrite(data, 1, length, alsa->dump_file);
  }
#endif
