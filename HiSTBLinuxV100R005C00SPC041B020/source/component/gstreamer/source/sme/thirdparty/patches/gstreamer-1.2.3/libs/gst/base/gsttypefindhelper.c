...
helper_find_peek (gpointer data, gint64 offset, guint size)
{
  ...
  GstBuffer *buffer;
/*start 20140522 z00175754 modify to use block IO read typefind probe data*/
  GstBuffer *buffer2 = NULL;
/*end 20140522 z00175754 modify to use block IO read typefind probe data*/
  GstFlowReturn ret;
...
/*start 20140522 z00175754 fix a bug to avoid getting same probe data repeatedly */
...
/*end 20140522 z00175754 fix a bug to avoid getting same probe data repeatedly */
  buffer = NULL;
...
/*start 20140522 z00175754 modify to use block IO read typefind probe data*/
  ...
  size = (helper->size == G_MAXUINT64) ? size : MAX(helper->size, size);
  /** END: Modified for DTS2015072304117  by baihaili(b00311320), 2015/7/25 */
  while (1)
  {
      buffer2 = NULL;
      ret =
          helper->func (helper->obj, helper->parent, buf_size, MAX (size, 4096)- buf_size,
          &buffer2);
      if (ret != GST_FLOW_OK){
        if(NULL != buffer2){
      ...
      if(NULL != buffer) {
        //gboolean result = gst_buffer_copy_into (buffer, buffer2, GST_BUFFER_COPY_DEEP|GST_BUFFER_COPY_MEMORY, 0, -1);
          gst_buffer_append (buffer, buffer2);
      }else {
          buffer = buffer2;
      }
      buf_size = gst_buffer_get_size (buffer);
...
error:
  {
    GST_WARNING ("--- typefind function returned: %s", gst_flow_get_name (ret));
    return NULL;
  }
map_failed:
  {
    ...
    GstCaps * caps)
{
  ...
  if (probability > helper->best_probability) {
    ...
    GST_INFO_OBJECT (helper->obj,
      "'%s' SCORE=%u, num=%u, %" GST_PTR_FORMAT ")",
      GST_OBJECT_NAME (helper->factory), probability, helper->num, caps);

  } else if (probability == helper->best_probability){
    helper->num ++;
 ...
 */
 ...
 #define GST_SME_TYPE_FIND_ORI_SIZE_SCORE_SAME 4096 * 2
 #define GST_SME_TYPE_FIND_SCORE_CONTEND 50
    ...
    const gchar * extension, GstTypeFindProbability * prob)
{
  ...
  /* BEGIN: Added for DTS2016042803960  on 05/04th/2016
   * root cause:
   *   For DTS2016042803960:
   *   mp3 demuxer give score 25 and mpegps give score 25, then mp3 demuxer is selected.
   *   but in fact, this stream's container is mpegps.
   */
  if ((size <= GST_SME_TYPE_FIND_ORI_SIZE_SCORE_SAME || size == (guint64) - 1)
      && helper.num >= 2 && helper.best_probability < GST_SME_TYPE_FIND_SCORE_CONTEND ) {
    GST_INFO_OBJECT (obj, "probability = %u, num = %u, continue probe",
      (guint) helper.best_probability, helper.num);
  ...
  /* END: Added for DTS2016042803960  on 05/04th/2016  */
