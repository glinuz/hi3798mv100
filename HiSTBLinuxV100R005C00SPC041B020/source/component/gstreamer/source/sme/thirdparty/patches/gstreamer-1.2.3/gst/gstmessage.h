...
typedef enum
{
  ...
  GST_MESSAGE_NEW_BANDWIDTH     = GST_MESSAGE_EXTENDED + 1,
  GST_MESSAGE_RESOLUTION_CHANGED  = GST_MESSAGE_EXTENDED + (1 << 1),
  GST_MESSAGE_NEW_HTTP_ERR      = GST_MESSAGE_EXTENDED + (1 << 2),
  GST_MESSAGE_HAVE_NO_STREAM      = GST_MESSAGE_EXTENDED + (1 << 3),
  GST_MESSAGE_STREAM_RECOVER      = GST_MESSAGE_EXTENDED + (1 << 4),
  GST_MESSAGE_RENDER_FIRST_AUDIO_FRAME = GST_MESSAGE_EXTENDED + (1 << 5),
  GST_MESSAGE_RENDER_FIRST_VIDEO_FRAME = GST_MESSAGE_EXTENDED + (1 << 6),
...
void            gst_message_set_buffering         (GstMessage *message, gint percent);
...
/* NEW_BANDWIDTH */
GstMessage *    gst_message_new_new_bandwidth (GstObject * src, guint bandwidth) G_GNUC_MALLOC;
void            gst_message_parse_new_bandwidth (GstMessage * message, guint * bandwidth);

GstMessage *    gst_message_new_resolution_changed(GstObject * src, gulong width, gulong height) G_GNUC_MALLOC;
void             gst_message_parse_resolution_changed (GstMessage * message, gulong * width, gulong * height);

/* ERROR REPORTING */
GstMessage *    gst_message_new_http_err (GstObject * src, guint statuscode) G_GNUC_MALLOC;
void            gst_message_parse_new_http_err (GstMessage * message, guint * statuscode);

GstMessage * gst_message_have_no_stream (GstObject * src) G_GNUC_MALLOC;
GstMessage * gst_message_stream_recover (GstObject * src) G_GNUC_MALLOC;
GstMessage * gst_message_new_render_first_audio_frame (GstObject * src) G_GNUC_MALLOC;
GstMessage * gst_message_new_render_first_video_frame (GstObject * src) G_GNUC_MALLOC;
