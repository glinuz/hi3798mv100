...
/*Fix ME: Maybe always wait if clock jump(sub class get time gap)*/
#define BADTIME_THRESHOLD (15 * GST_SECOND )
#define _DEBUG_
...
#define GST_SYSTEM_CLOCK_WAIT_TIME_DEFAULT (50*GST_MSECOND)
...
gst_system_clock_class_init (GstSystemClockClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_MAX_BACKWARD,
      g_param_spec_uint64 ("max-backward", "stream pts > clock time",
          "stream pts > clock time",
          0ULL, G_MAXUINT64, G_MAXUINT64,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_CLOCK_TYPE:
      ...
      break;
    case PROP_MAX_BACKWARD:
      sysclock->priv->max_backward = (GstClockType) g_value_get_uint64(value);
      GST_CAT_WARNING (GST_CAT_CLOCK, "system clock wait:%lld",
          sysclock->priv->max_backward);
      break;
    default:
    ...
    GstClockEntry * entry, GstClockTimeDiff * jitter, gboolean restart)
{
  ...
  GST_CAT_LOG (GST_CAT_CLOCK, "entry %p"
...
#ifdef BADTIME_THRESHOLD // Disabled by xueping for avsync
  if (llabs(diff) >= BADTIME_THRESHOLD) {
    status = GST_CLOCK_BADTIME;
    SET_ENTRY_STATUS (entry, status);
    goto done;
  }
#endif

  if(GST_CLOCK_TIME_IS_VALID(sysclock->priv->max_backward)
    && diff >= (GstClockTimeDiff)sysclock->priv->max_backward){
    GST_INFO("diff:%lld,max_backward:%lld", diff, sysclock->priv->max_backward);
    tmp_diff = diff;
    diff = GST_SYSTEM_CLOCK_WAIT_TIME_DEFAULT;
  }else{
    tmp_diff = -1;
  }
  ...
  if (G_LIKELY (diff > 0)) {
    ...
    while (TRUE) {
      ...
      //GST_INFO("gst_poll_wait begin...diff:%lld", diff);
      pollret = gst_poll_wait (sysclock->priv->timer, diff);
      //GST_INFO("gst_poll_wait end,(%d)", pollret);
      ...
      if (G_UNLIKELY (status == GST_CLOCK_UNSCHEDULED)) {
        ...
        now = gst_clock_get_time (clock);
        diff = GST_CLOCK_DIFF (now, entryt);
        //GST_INFO("gst_clock_get_time now_clock(%lld),diff:%lld", now, diff);
        if(tmp_diff != -1){
          diff = 0;
        }
