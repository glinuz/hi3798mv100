...
#define GST_LATE_MAX (60*GST_SECOND)//60S

#define GST_AUDIO_LATE_MAX (20*GST_MSECOND)//20ms
...
#define GET_SINK_TYPE_NAME(BSINK) \
  ((E_SINK_TYPE_VIDEO == BSINK->e_sink_type) ? "vsink" : \
  ((E_SINK_TYPE_AUDIO == BSINK->e_sink_type) ? "asink" : "tsink"))
...
struct _GstBaseSinkPrivate
{
  ...
  /*for dvb live multiq control */
  GstSmeQueueStatus buffer_status;

  /*for sme audio clock */
  GRecMutex  buffer_time_info_rmutex;//rec mutex for R/W audio time info.
  GstClockTime rstart;//the newest buffer start runnint time.
  GstClockTime sys_time;//the system time when update rstart
};
...
static void gst_base_sink_init_presync_info(GstSlowPreSyncInfo *info)
{
  memset(info, 0, sizeof(GstSlowPreSyncInfo));
...
static void gst_base_sink_update_buffer_time_info(GstBaseSink * basesink,
  gboolean is_update_rstart)
{
  GstClockTime last_rstart;
  GstClockTime last_system;
  GstClockTime cur_system;
  GstClockTime new_rstart;

  gst_base_sink_get_buffer_time_info(basesink, &last_rstart, &last_system);
  cur_system = (GstClockTime)GST_USECOND * (guint64)g_get_monotonic_time();

  if(GST_CLOCK_TIME_IS_VALID(last_rstart)
    && GST_CLOCK_TIME_IS_VALID(last_system)){
    if(!is_update_rstart){
      new_rstart = last_rstart;
    }else if(cur_system >= last_system){
      new_rstart = last_rstart + (cur_system - last_system);
    }else{
      new_rstart = last_rstart;
      GST_WARNING_OBJECT(basesink, "update_buffer_time_info, cur_system < last_system,%lld<%lld",
        cur_system, last_system);
    }
    GST_WARNING_OBJECT(basesink, "update_buffer_time_info, is_update_rstart:%d,%lld->%lld",
      is_update_rstart, last_rstart, new_rstart);
    gst_base_sink_set_buffer_time_info(basesink, new_rstart);
  }
...
/**
   * GstBaseSink:buffer status:
   *
   * Set buffer status to sink
   *
   * Since: 1.1.1
   */
  g_object_class_install_property (gobject_class, PROP_BUFFER_STATUS,
      g_param_spec_uint ("buffer-status", "Buffer status",
          "buffer status of pipeline queue", 0, G_MAXUINT,
          GST_SME_NORMAL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_class, PROP_FORCE_ASYNC_DONE,
      g_param_spec_boolean ("force-async-done", "force async_done",
          "force async_done property", FALSE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
gst_base_sink_init (GstBaseSink * basesink, gpointer g_class)
{
  ...
  priv->buffer_status = GST_SME_NORMAL;
  priv->rstart = GST_CLOCK_TIME_NONE;
  priv->sys_time = GST_CLOCK_TIME_NONE;
  GST_OBJECT_FLAG_SET (basesink, GST_ELEMENT_FLAG_SINK);

  /* BEGIN: Added for DTS2015071706676 by liurongliang(l00180035), 2015/7/22 */
  basesink->render_num_after_async_done = 0;
  pszCaps = gst_caps_to_string(GST_PAD_TEMPLATE_CAPS(pad_template));
  GST_INFO_OBJECT(basesink, ("gstcaps=%s", pszCaps));

  if(strstr(pszCaps, "video/")){
    basesink->e_sink_type = E_SINK_TYPE_VIDEO;
  } else if (strstr(pszCaps, "audio/")) {
    basesink->e_sink_type = E_SINK_TYPE_AUDIO;
  } else {
    basesink->e_sink_type = E_SINK_TYPE_TEXT;
  }
  basesink->sync_info.DroppedVideoFrameCntAtSync = 0;
  basesink->sync_info.VideoFrameIsDelay = FALSE;
  basesink->sync_info.IsSync = TRUE;
  if (GST_ELEMENT_CLOCK((basesink)))
    basesink->sync_info.ClockForSync = gst_clock_get_time(GST_ELEMENT_CLOCK(basesink));
  g_free(pszCaps);
  /* END:   Added for DTS2015071706676 by liurongliang(l00180035), 2015/7/22 */
  g_rec_mutex_init(&basesink->priv->buffer_time_info_rmutex);
#ifdef GST_DDP_PASSTHROUGH_ONLY_SUPPORT
  basesink->first_render_flag = 1;
#endif
  gst_base_sink_init_presync_info(&basesink->slow_presync_info);
}
...
gst_base_sink_finalize (GObject * object)
{
  ...
  g_rec_mutex_clear(&basesink->priv->buffer_time_info_rmutex);
...
gst_base_sink_set_async_enabled (GstBaseSink * sink, gboolean enabled)
{
  ...
  GST_INFO_OBJECT (sink, "set async enabled to %d", enabled);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_MAX_BITRATE:
      ...
      break;
    case PROP_BUFFER_STATUS:{
      GstSmeQueueStatus status = g_value_get_uint (value);
      g_return_if_fail (status >= GST_SME_OVERRUN);
      g_return_if_fail (status < GST_SME_LAST);
      gst_base_sink_set_buffer_status (sink, status);
    }
    break;
    case PROP_FORCE_ASYNC_DONE:
      sink->is_force_async_done = g_value_get_boolean (value);
      GST_WARNING_OBJECT(sink, "force async_done:%d", sink->is_force_async_done);
      if(sink->is_force_async_done && sink->is_send_async_start){
        GST_WARNING_OBJECT(sink, "force async_done, send async_done begin...");
        GST_BASE_SINK_PREROLL_LOCK (sink);
        gst_base_sink_commit_state (sink);
        GST_BASE_SINK_PREROLL_UNLOCK (sink);
        GST_WARNING_OBJECT(sink, "force async_done, send async_done end");
        sink->is_send_async_start = FALSE;
      }else if(!sink->is_force_async_done && !sink->is_send_async_start){
        //fix issue:the video will always late too many, caused lost too many
        //video frame,when DVBLive play audio back to video.
        GST_BASE_SINK_PREROLL_LOCK (sink);
        sink->playing_async = TRUE;
        sink->priv->commited = FALSE;
        sink->priv->call_preroll = TRUE;
        sink->need_preroll = TRUE;
        sink->have_preroll = FALSE;
        if (sink->priv->async_enabled) {
          GST_INFO_OBJECT (sink, "%s:doing lost_state(%s) when force 0 ",
            GET_SINK_TYPE_NAME(sink), gst_element_state_get_name(GST_STATE(sink)));
          gst_element_lost_state (GST_ELEMENT_CAST (sink));
          sink->is_send_async_start = TRUE;
        }
        GST_BASE_SINK_PREROLL_UNLOCK (sink);
      }
      break;
...
gst_base_sink_commit_state (GstBaseSink * basesink)
{
  ...
  switch (pending) {
    ...
    {
      GST_INFO_OBJECT (basesink, "commiting state to PLAYING");
    ...
    case GST_STATE_PAUSED:
      GST_INFO_OBJECT (basesink, "commiting state to PAUSED");
      post_paused = TRUE;
      post_async_done = TRUE;
      basesink->priv->commited = TRUE;
  ...
  if (post_paused) {
    GST_INFO_OBJECT (basesink, "posting PAUSED state change message");
    gst_element_post_message (GST_ELEMENT_CAST (basesink),
        gst_message_new_state_changed (GST_OBJECT_CAST (basesink),
            current, next, post_pending));
  }
  if (post_async_done) {
    GST_WARNING_OBJECT (basesink, "%s:%s Async_Done.",
        SMEPLAYER_STAT_TAG_TIME, GET_SINK_TYPE_NAME(basesink));
    gst_element_post_message (GST_ELEMENT_CAST (basesink),
        gst_message_new_async_done (GST_OBJECT_CAST (basesink),
            GST_CLOCK_TIME_NONE));
    basesink->is_send_async_start = FALSE;
  }
  if (post_playing) {
    if (post_paused) {
...
again:
  ...
  if (G_UNLIKELY (!gst_segment_clip (segment, format,
    ...
    if(basesink->slow_presync_info.is_free_run){
      *rsstart = GST_CLOCK_TIME_NONE;
      *rsstop = GST_CLOCK_TIME_NONE;
      *rrstart = GST_CLOCK_TIME_NONE;
      *rrstop = GST_CLOCK_TIME_NONE;
      *rrnext = GST_CLOCK_TIME_NONE;
...
do_times:
  if(0 != basesink->slow_presync_info.slow_presync_state){
    gdouble org_rate = segment->rate;
    if(1 == basesink->slow_presync_info.slow_presync_state){
      segment->rate = basesink->slow_presync_info.gdrate;
      rstart = gst_segment_to_running_time (segment, format, cstart);
      rstop = gst_segment_to_running_time (segment, format, cstop);
      segment->rate = org_rate;

      GST_INFO_OBJECT(basesink, "pre-sync:%s:cstart:%lld, rstart:%lld",
        GET_SINK_TYPE_NAME(basesink), cstart,rstart);
    }else{
      GstClockTime org_seg_start = segment->start;
      if(GST_CLOCK_TIME_NONE == basesink->slow_presync_info.last_presync_pts){
        basesink->slow_presync_info.last_presync_pts = cstart;
        segment->rate = basesink->slow_presync_info.gdrate;
        basesink->slow_presync_info.slow_presync_stime
          = gst_segment_to_running_time (segment, format, cstart);
        GST_WARNING_OBJECT(basesink, "pre-sync:%s:last slow-pre-sync-pts:%lld, stime:%lld",
          GET_SINK_TYPE_NAME(basesink),
          basesink->slow_presync_info.last_presync_pts,
          basesink->slow_presync_info.slow_presync_stime);
        segment->rate = org_rate;
      }
      segment->start = basesink->slow_presync_info.last_presync_pts;
      rstart = gst_segment_to_running_time (segment, format, cstart);
      rstop = gst_segment_to_running_time (segment, format, cstop);
      if(GST_CLOCK_TIME_IS_VALID(basesink->slow_presync_info.slow_presync_stime)){
        if(GST_CLOCK_TIME_IS_VALID(rstart)){
          rstart += basesink->slow_presync_info.slow_presync_stime;
        }
        if(GST_CLOCK_TIME_IS_VALID(rstop)){
          rstop += basesink->slow_presync_info.slow_presync_stime;
        }
      }else{
        GST_WARNING_OBJECT(basesink, "pre-sync:slow_presync_stime is invalid why?");
      }
      segment->start = org_seg_start;

      GST_DEBUG_OBJECT(basesink, "pre-sync:1 %s:cstart:%lld, rstart:%lld",
        GET_SINK_TYPE_NAME(basesink), cstart,rstart);
    }
  }else{
    rstart = gst_segment_to_running_time (segment, format, cstart);
...
out_of_segment:
  {
     ...
     * to sync on it. */
    GST_WARNING_OBJECT (basesink, "buffer skipped, not in segment");
    return FALSE;
  }
}
...
gst_base_sink_adjust_time (GstBaseSink * basesink, GstClockTime time)
{
  ...
  if (G_UNLIKELY (GST_CLOCK_TIME_IS_VALID (basesink->priv->render_delay)))
  {
  ...
  GST_DEBUG_OBJECT_AVSYNC (basesink, "%s:%s adjust_time latency: %lld,"
    "ts_offset %lld, render_delay %lld", AVSYNC_BY_AUDIO_TAG,
    GET_SINK_TYPE_NAME(basesink),
    basesink->priv->latency, basesink->priv->ts_offset,
    basesink->priv->render_delay);
...
gst_base_sink_wait_preroll (GstBaseSink * sink)
{
  ...
  GST_INFO_OBJECT (sink, "waiting in preroll for flush or PLAYING,sinktype=%d,render_num=%d",
    sink->e_sink_type,sink->render_num_after_async_done);

  /* block until the state changes, or we get a flush, or something */
  /* BEGIN: Modified for DTS2015071706676 by liurongliang(l00180035), 2015/7/22 */
  /*
     if this is video sink, we donot wait preroll signal on the first frame.
     by contrast, we will render it.
     by doing this we will see the picture when seek in pause state.
  */
  if(E_SINK_TYPE_VIDEO != sink->e_sink_type || sink->render_num_after_async_done){
    GST_BASE_SINK_PREROLL_WAIT (sink);
    GST_INFO_OBJECT (sink, "waiting in preroll for flush or PLAYING");
    sink->have_preroll = FALSE;
  }
  /* END:   Modified for DTS2015071706676 by liurongliang(l00180035), 2015/7/22 */
...
gst_base_sink_do_preroll (GstBaseSink * sink, GstMiniObject * obj)
{
  ...
  while (G_UNLIKELY (sink->need_preroll)) {
    ...
    if (G_LIKELY (sink->need_preroll)) {
      ...
      */
      if(E_SINK_TYPE_VIDEO == sink->e_sink_type && !sink->render_num_after_async_done){
        GST_INFO_OBJECT(sink, "video sink, render num is 0");
        break;
      }
      /* END: Modified for DTS2015071706676 by liurongliang(l00180035), 2015/7/22 */
    }else{
...
again:
  ...
  if(basesink->slow_presync_info.is_free_run && GST_CLOCK_TIME_IS_VALID(stime)){
    GstClock* tmp_clock = GST_ELEMENT_CLOCK(basesink);
    GstClockTime cur_cc_time = GST_CLOCK_TIME_NONE;
    if(tmp_clock){
      cur_cc_time = gst_clock_get_time(tmp_clock);
    }
    if(GST_CLOCK_TIME_IS_VALID(cur_cc_time)){
      if(stime +  GST_ELEMENT_CAST (basesink)->base_time + GST_AUDIO_LATE_MAX >= cur_cc_time){
        GST_WARNING_OBJECT(basesink, "pre-sync:%s now audio --> video,stime:%lld,"
          "base_time:%lld, cur_cc_time:%lld",
          GET_SINK_TYPE_NAME(basesink), stime, GST_ELEMENT_CAST (basesink)->base_time,
          cur_cc_time);
        basesink->slow_presync_info.is_free_run = FALSE;
      }
    }
  }
  ...
  GST_DEBUG_OBJECT_AVSYNC(basesink, "%s::sinktype:%d, PTS:%lld,rstart:%lld rstop:%lld"
    ", stime:%lld, clock_time:%lld, base_time:%lld, wait begin...",
    AVSYNC_BY_AUDIO_TAG, basesink->e_sink_type, GST_BUFFER_PTS(obj), rstart, rstop, stime,
    GST_ELEMENT_CLOCK(basesink) ? gst_clock_get_time(GST_ELEMENT_CLOCK(basesink)) : GST_CLOCK_TIME_NONE,
    gst_element_get_base_time(basesink));
  ...
  /*
     ...
     by doing this we will see the picture when seek in pause state.
  */
  if(E_SINK_TYPE_VIDEO != basesink->e_sink_type || basesink->render_num_after_async_done){
    status = gst_base_sink_wait_clock (basesink, stime, &jitter);
  }else{
    status = GST_CLOCK_OK;
    jitter = 0;
    GST_INFO_OBJECT(basesink, "render first frame if video");
  }
  /* END: Modified for DTS2015071706676 by liurongliang(l00180035), 2015/7/22 */

  GST_DEBUG_OBJECT_AVSYNC(basesink, "%s::sinktype:%d, PTS:%lld,rstart:%lld rstop:%lld"
    ", stime:%lld, late:%lld, status:%d,lateness=%lld, wait end", AVSYNC_BY_AUDIO_TAG,
    basesink->e_sink_type, GST_BUFFER_PTS(obj), rstart, rstop, stime, jitter,
    status, basesink->max_lateness);
  ...
  /* save running time & system time for sync */
  if(!(*late) && !basesink->slow_presync_info.is_free_run){
    gst_base_sink_set_buffer_time_info(basesink, stime);
  }else if (*late > 0){
    GST_DEBUG_OBJECT_AVSYNC(basesink, "%s:sinktype:%d,late:%d", AVSYNC_BY_AUDIO_TAG,
      basesink->e_sink_type, *late);
  }
    ...
    GstClockReturn status, GstClockTimeDiff jitter, gboolean render)
{
  ...
  if ((late = rstart + jitter > max_lateness)) {
    ...
    if (GST_CLOCK_TIME_IS_VALID (priv->last_render_time) &&
        rstart - priv->last_render_time > GST_LATE_MAX) {
      late = FALSE;
...
{
  GST_WARNING_OBJECT(basesink, "%s:%s flush_start in...",
    SMEPLAYER_STAT_TAG_TIME, GET_SINK_TYPE_NAME(basesink));
   ...
   * anymore */
  GST_WARNING_OBJECT(basesink, "%s:%s flush_start get lock begin...",
    SMEPLAYER_STAT_TAG_TIME, GET_SINK_TYPE_NAME(basesink));
  GST_PAD_STREAM_LOCK (pad);
  GST_WARNING_OBJECT(basesink, "%s:%s flush_start get lock end",
    SMEPLAYER_STAT_TAG_TIME, GET_SINK_TYPE_NAME(basesink));
  gst_base_sink_reset_qos (basesink);
  /* and we need to commit our state again on the next
   * prerolled buffer */
  basesink->playing_async = TRUE;
  GST_INFO("flush start gst_element_lost_state,en=%d", basesink->priv->async_enabled);
  if (basesink->priv->async_enabled) {
    /* BEGIN: Add for DTS2015071706676 by liurongliang(l00180035), 2015/7/22 */
    /*
       if this is video sink, we donot wait preroll signal on the first frame.
       by contrast, we will render it.
       by doing this we will see the picture when seek in pause state.
    */
    basesink->render_num_after_async_done = 0;
    /* END: Add for DTS2015071706676 by liurongliang(l00180035), 2015/7/22 */
    GST_WARNING_OBJECT(basesink, "%s:%s flush_start lost state begin...",
      SMEPLAYER_STAT_TAG_TIME, GET_SINK_TYPE_NAME(basesink));
    gst_element_lost_state (GST_ELEMENT_CAST (basesink));
    if (GST_STATE_CHANGE_FAILURE != GST_STATE_RETURN (GST_ELEMENT_CAST(basesink))){
      basesink->is_send_async_start = TRUE;
    }
    GST_WARNING_OBJECT(basesink, "%s:%s flush_start lost state end",
      SMEPLAYER_STAT_TAG_TIME, GET_SINK_TYPE_NAME(basesink));
    } else {
    ...
    gboolean reset_time)
{
  ...
  if (basesink->pad_mode == GST_PAD_MODE_PUSH) {
     ...
     * But, if reset_time is false, don't reset new segment info;
     * else it will cause below problem:
     *   after seek, do multi audio track switch, video will be freezed.
     *   Because have_newsegment is assigned to false wrongly,
     *   audio start timestamp will be assigned to 0 in gst_base_sink_chain_unlocked().
     *   It will cause audio segment info is different with video segment info.
     */
    ...
    if (reset_time) {
      ...
      //clear rstart, rstop
      gst_base_sink_set_buffer_time_info(basesink, GST_CLOCK_TIME_NONE);
    }
  }
  GST_OBJECT_UNLOCK (basesink);
...
gst_base_sink_default_event (GstBaseSink * basesink, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_SEGMENT:
      ...
      GST_WARNING_OBJECT (basesink, "%s:%s:configured segment %p, [%lld,%lld]",
          AVSYNC_BY_AUDIO_TAG, GET_SINK_TYPE_NAME(basesink),
          &basesink->segment,
          basesink->segment.start,
          basesink->segment.stop);
...
#ifdef GST_DDP_PASSTHROUGH_ONLY_SUPPORT
static void
gst_base_sink_post_render_event(GstBaseSink * basesink, gint flag)
{
    GstStructure *structTmp = NULL;

    if (E_SINK_TYPE_AUDIO != basesink->e_sink_type)
        return ;

    structTmp = gst_structure_new("render_flag","render_flag",G_TYPE_INT,flag,NULL);
    gst_pad_push_event(basesink->sinkpad,gst_event_new_custom (GST_EVENT_CUSTOM_UPSTREAM, structTmp));

    GST_INFO_OBJECT (basesink, "after gst_base_sink_post_render_event %d", flag);
}
#endif
    ...
    gpointer obj, gboolean is_list)
{
  ...
  if(GST_IS_BUFFER(obj) && GST_BUFFER_FLAG_IS_SET(obj, GST_BUFFER_FLAG_DECODE_ONLY)){
    GST_INFO("%s::got a GST_BUFFER_FLAG_DECODE_ONLY buffer,just commit state,"
      "playing_async=%d, need_preroll=%d", GET_SINK_TYPE_NAME(basesink),
      basesink->playing_async, basesink->need_preroll);

    /* commit state */
    if (G_LIKELY (basesink->playing_async)) {
      gst_base_sink_commit_state (basesink);
    }
    ...
    if (G_LIKELY (basesink->need_preroll)) {
      /* block until the state changes, or we get a flush, or something */
      gst_base_sink_wait_preroll (basesink);
    }
    goto out_of_segment;
  }
  ...
  bclass = GST_BASE_SINK_GET_CLASS (basesink);
  if(bclass->calibrate_pts){
    gboolean bcalibrate;
    bclass->calibrate_pts(basesink, sync_buf, &bcalibrate);
    if(bcalibrate){
      GST_ERROR_OBJECT(basesink, "pts is calibrated !");
    }
  }
  ...
  GST_INFO_OBJECT (basesink, "%s:got times start: %" GST_TIME_FORMAT
      ", end: %" GST_TIME_FORMAT, GET_SINK_TYPE_NAME(basesink),
  ...
  if (GST_CLOCK_TIME_IS_VALID (start) && (segment->format == GST_FORMAT_TIME)) {
    if (G_UNLIKELY (!gst_segment_clip (segment,
      ...
      //if is audio let it commit state;
      if(E_SINK_TYPE_AUDIO == basesink->e_sink_type
        && 0 != basesink->slow_presync_info.slow_presync_state){
        if(GST_CLOCK_TIME_NONE != basesink->slow_presync_info.start_wait_presync_systime){
          guint64 cur_systime = GST_USECOND * (guint64)g_get_monotonic_time();
          if(basesink->slow_presync_info.start_wait_presync_systime
            + basesink->slow_presync_info.wait_presync_max_time
            <= cur_systime){
            /* commit state */
            GST_WARNING_OBJECT(basesink, "pre-sync:clip audio by segment, but time too "
              "long, send async done!");
            if (G_LIKELY (basesink->playing_async)) {
              gst_base_sink_commit_state (basesink);
            }
            ...
            if (G_LIKELY (basesink->need_preroll)) {
              /* block until the state changes, or we get a flush, or something */
              ret = gst_base_sink_wait_preroll (basesink);
            }
            basesink->slow_presync_info.start_wait_presync_systime = GST_CLOCK_TIME_NONE;
            basesink->slow_presync_info.is_free_run = TRUE;
          }
        }
      }
      if(!basesink->slow_presync_info.is_free_run){
        ...
        GST_WARNING_OBJECT(basesink, "pre-sync:%s, out_of_segment free run",
          GET_SINK_TYPE_NAME(basesink));
      }
    }
  }
  if (bclass->prepare || bclass->prepare_list) {
    ...
    if(!syncable)
        basesink->sync_info.IsSync = FALSE;
...
again:
  ...
  step_end = FALSE;
  if (GST_ELEMENT_CLOCK((basesink)))
      basesink->sync_info.ClockForSync = gst_clock_get_time(GST_ELEMENT_CLOCK(GST_ELEMENT(basesink)));
  ...
  GST_DEBUG_OBJECT_AVSYNC(basesink, "%s::sinktype:%d, PTS:%lld,duration:%lld"
    ",render begin...", AVSYNC_BY_AUDIO_TAG,
    basesink->e_sink_type, GST_BUFFER_PTS(obj), GST_BUFFER_DURATION(obj));
  /* record rendering time for QoS and stats */
  if (do_qos)
    gst_base_sink_do_render_stats (basesink, TRUE);
  if(!basesink->slow_presync_info.is_free_run){
    basesink->render_num_after_async_done++;
  }
  if (!is_list) {
    /* For buffer lists do not set last buffer for now. */
...
#ifdef GST_DDP_PASSTHROUGH_ONLY_SUPPORT
    if (basesink->first_render_flag) {
        gst_base_sink_post_render_event(basesink,basesink->first_render_flag);
        basesink->first_render_flag = 0;
    }
#endif
    if(1 == basesink->render_num_after_async_done){
      if(E_SINK_TYPE_AUDIO == basesink->e_sink_type){
        GstMessage *render_1st_frame = gst_message_new_render_first_audio_frame ((GstObject*)basesink);
        if(render_1st_frame){
          GST_WARNING_OBJECT(basesink, "pre-sync:post render_first_audio_frame");
          gst_element_post_message((GstElement *)basesink, render_1st_frame);
        }
      }else if(E_SINK_TYPE_VIDEO == basesink->e_sink_type){
        GstMessage *render_1st_frame = gst_message_new_render_first_video_frame ((GstObject*)basesink);
        if(render_1st_frame){
          GST_WARNING_OBJECT(basesink, "pre-sync:post render_first_video_frame");
          gst_element_post_message((GstElement *)basesink, render_1st_frame);
        }
      }
    }
  ...
  GST_DEBUG_OBJECT_AVSYNC(basesink, "%s::sinktype:%d, PTS:%lld,duration:%lld"
    ",render end ...", AVSYNC_BY_AUDIO_TAG,
    basesink->e_sink_type, GST_BUFFER_PTS(obj), GST_BUFFER_DURATION(obj));
...
#endif
    GST_INFO_OBJECT (basesink, "dropping buffer, out of clipping segment");
    gst_mini_object_unref (GST_MINI_OBJECT_CAST (obj));
    return GST_FLOW_OK;
  }
prepare_failed:
...
#endif
    priv->dropped++;
    basesink->sync_info.DroppedVideoFrameCntAtSync = priv->dropped;
    basesink->sync_info.VideoFrameIsDelay = TRUE;
    ...
    gboolean is_list)
{
  ...
  if(GST_IS_BUFFER(obj)){
    GST_DEBUG_OBJECT_AVSYNC(basesink, "%s::sinktype:%d, PTS:%lld,duration:%lld begin",
      AVSYNC_BY_AUDIO_TAG, basesink->e_sink_type, GST_BUFFER_PTS(obj),
      GST_BUFFER_DURATION(obj));
  }
  GST_BASE_SINK_PREROLL_LOCK (basesink);
  result = gst_base_sink_chain_unlocked (basesink, pad, obj, is_list);
  GST_BASE_SINK_PREROLL_UNLOCK (basesink);

  GST_DEBUG_OBJECT_AVSYNC(basesink, "%s::sinktype:%d, end",
    AVSYNC_BY_AUDIO_TAG, basesink->e_sink_type);
...
default_element_query (GstElement * element, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_POSITION:
    {
      ...
      /* BEGIN: Added by qihuafei(q00307195), 2015/7/25 */
      /**
       * Doesn't provide position query if it is text sink,
       * since text sink's postion accuracy is bad.
       */
      if (E_SINK_TYPE_TEXT == basesink->e_sink_type) {
          return res;
      }
      /* END: Added by qihuafei(q00307195), 2015/7/25 */
...
gst_base_sink_default_query (GstBaseSink * basesink, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    case GST_QUERY_DRAIN:{
      if(basesink->priv->last_buffer){
         gst_buffer_unref(basesink->priv->last_buffer);
         basesink->priv->last_buffer = NULL;
      }
      res = TRUE;
      break;
    }
...
gst_base_sink_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    ...
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      ...
      GST_BASE_SINK_PREROLL_LOCK (basesink);
      GST_INFO_OBJECT (basesink, "READY to PAUSED");
      basesink->have_newsegment = FALSE;
      gst_segment_init (&basesink->segment, GST_FORMAT_UNDEFINED);
      basesink->offset = 0;
      basesink->have_preroll = FALSE;
      ...
      if (priv->async_enabled) {
        GST_INFO_OBJECT (basesink, "doing async state change");
        ...
        */
        ...
        if(basesink->is_force_async_done){
          gst_base_sink_commit_state (basesink);
        }else{
          basesink->is_send_async_start = TRUE;
        }
    ...
    case GST_STATE_CHANGE_PAUSED_TO_PLAYING:
      ...
      if (!gst_base_sink_needs_preroll (basesink)) {
        GST_INFO_OBJECT (basesink, "PAUSED to PLAYING, don't need preroll");
        ...
        if (basesink->eos) {
          ...
          GST_INFO_OBJECT (basesink, "Now posting EOS");
          message = gst_message_new_eos (GST_OBJECT_CAST (basesink));
          gst_message_set_seqnum (message, basesink->priv->seqnum);
          gst_element_post_message (GST_ELEMENT_CAST (basesink), message);
        } else {
          GST_INFO_OBJECT (basesink, "signal preroll");
          GST_BASE_SINK_PREROLL_SIGNAL (basesink);
        }
      } else {
        GST_INFO_OBJECT (basesink, "PAUSED to PLAYING, we are not prerolled");
        basesink->need_preroll = TRUE;
        basesink->playing_async = TRUE;
        priv->call_preroll = TRUE;
        priv->commited = FALSE;
      ...
      //update systime when paused->playing
      gst_base_sink_update_buffer_time_info(basesink, FALSE);
      GST_BASE_SINK_PREROLL_UNLOCK (basesink);
      break;
    default:
      break;
  ...
  switch (transition) {
    case GST_STATE_CHANGE_PAUSED_TO_PLAYING:
      ...
      GST_WARNING_OBJECT (basesink, "%s:%s changed to playing",
          SMEPLAYER_STAT_TAG_TIME, GET_SINK_TYPE_NAME(basesink));
      break;
    case GST_STATE_CHANGE_PLAYING_TO_PAUSED:
      g_atomic_int_set (&basesink->priv->to_playing, FALSE);
      GST_INFO_OBJECT (basesink, "PLAYING to PAUSED");
       ...
       * return ASYNC. */
      if (!gst_base_sink_needs_preroll (basesink)) {
        GST_INFO_OBJECT (basesink, "PLAYING to PAUSED, we are prerolled");
        basesink->playing_async = FALSE;
      } else {
        if (GST_STATE_TARGET (GST_ELEMENT (basesink)) <= GST_STATE_READY) {
          GST_INFO_OBJECT (basesink, "element is <= READY");
      ...
      //update rstart when paused
      gst_base_sink_update_buffer_time_info(basesink, TRUE);
      gst_base_sink_reset_qos (basesink);
      GST_BASE_SINK_PREROLL_UNLOCK (basesink);
      break;
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      ...
      if (!priv->commited) {
        if (priv->async_enabled) {
          GST_INFO_OBJECT (basesink, "PAUSED to READY, posting async-done");
...
/**
 * gst_base_sink_set_running_time:
 * @sink: a #GstBaseSink
 * @rstart: audio buffer start running time
 * set audio buffer time info.
 */
void gst_base_sink_set_buffer_time_info(GstBaseSink * sink, GstClockTime rstart)
{
  g_return_if_fail (GST_IS_BASE_SINK (sink));
  g_return_if_fail (sink->priv);
  g_rec_mutex_lock(&sink->priv->buffer_time_info_rmutex);

  sink->priv->rstart = rstart;
  if(GST_CLOCK_TIME_IS_VALID(rstart)){
    sink->priv->sys_time = GST_USECOND * (guint64)g_get_monotonic_time();
  }

  g_rec_mutex_unlock(&sink->priv->buffer_time_info_rmutex);
 ...
 * @sink: a #GstBaseSink
 * @rstart: audio buffer start running time
 * @systime: system_time when update rstart
 * get audio buffer time info.
 */
void gst_base_sink_get_buffer_time_info(GstBaseSink * sink, GstClockTime* rstart,
  GstClockTime* systime)
{
  if(rstart){
    *rstart = GST_CLOCK_TIME_NONE;
  }

  if(systime){
    *systime = GST_CLOCK_TIME_NONE;
  }
  ...
  g_rec_mutex_lock(&sink->priv->buffer_time_info_rmutex);
  if(rstart){
    *rstart = sink->priv->rstart;
  }
  if(systime){
    *systime = sink->priv->sys_time;
  }
  g_rec_mutex_unlock(&sink->priv->buffer_time_info_rmutex);
...
/**
 * gst_base_sink_get_buffer_status:
 * @sink: a #GstBaseSink
 *
 */
guint32
gst_base_sink_get_buffer_status(GstBaseSink * sink)
{
  g_return_if_fail (GST_IS_BASE_SINK (sink));
  g_return_if_fail (sink->priv);

  return sink->priv->buffer_status;
}

void
gst_base_sink_set_buffer_status (GstBaseSink * sink, guint32 status )
{
  g_return_if_fail (GST_IS_BASE_SINK (sink));
  g_return_if_fail (sink->priv);

  GST_OBJECT_LOCK(sink);
  sink->priv->buffer_status = status;
  GST_OBJECT_UNLOCK (sink);
}
