    ...
    GstEvent * event)
{
  ...
  if (typefind->segment.format != GST_FORMAT_UNDEFINED &&
    ...
    /** BEGIN: Added for DTS2015110203863 by baihaili(b00311320), 2015/11/3 */
    result = gst_type_find_element_seek (typefind, event);
    gst_event_unref (event);
    return result;
    /* END: Added for DTS2015110203863 by baihaili(b00311320), 2015/11/3 */
  } else {
...
gst_type_find_get_extension (GstTypeFindElement * typefind, GstPad * pad)
{
  ...
  GST_DEBUG_OBJECT (typefind, "finding extension of uri");
...
#define GST_SME_TYPE_FIND_ORI_SIZE 4096
#define GST_SME_TYPE_FIND_MIN_SCORE 5
  ...
  if (typefind->mode == MODE_TYPEFIND) {
    ...
    if (!found_caps) {
      ...
      if (peer) {
        ...
        /*modified begin, 初始探测数据为4096B，若未探测到，则增大typefind_size，继续探测*/
        max_size = MIN(size, GST_SME_TYPE_FIND_MAX_SIZE);
        for(typefind_size = MIN(GST_SME_TYPE_FIND_ORI_SIZE,size);
          typefind_size <= max_size; typefind_size = MIN(typefind_size << 1,max_size)){
          GST_INFO("min_probability=%d, typefind_size is %"G_GINT64_FORMAT,
              typefind->min_probability, typefind_size);
          found_caps =
            ...
            (guint64) typefind_size, ext, &probability);

          if(found_caps && (probability >= GST_SME_TYPE_FIND_MIN_SCORE)){
              GST_DEBUG ("have found the type, and typefind_size is %"G_GINT64_FORMAT, typefind_size);
              break;
          }
          /** BEGIN: Modified for DTS2015072304117  by baihaili(b00311320), 2015/7/25 */
          if (typefind_size <= 0) {
            if (!gst_pad_query_duration (peer, GST_FORMAT_BYTES, &size)) {
                GST_WARNING_OBJECT (typefind, "Could not query upstream length!");
            } else {
                /** BEGIN: Modified for DTS2016042305660, 2016/5/9 */
                // typefind loop should not continue when we're resetting the pipeline, for scenario
                // such as src element constantly return size -1, caused by http 500 error
                if (g_atomic_int_get (&typefind->shutdown)) {
                  GST_INFO ("typefind loop get shutdown 1");
                  g_free (ext);
                  gst_object_unref (peer);
                  ret = GST_FLOW_ERROR;
                  goto pause;
                }
                /** END: Modified for DTS2016042305660, 2016/5/9 */
                max_size = MIN(size, GST_SME_TYPE_FIND_MAX_SIZE);
                GST_WARNING_OBJECT (typefind, "query size:%"G_GINT64_FORMAT,size);
                typefind_size = MIN(GST_SME_TYPE_FIND_ORI_SIZE,size);
                GST_WARNING_OBJECT (typefind, "Update typefind size to:%"G_GINT64_FORMAT,typefind_size);
                //fix issue:http file < 4K, it will typefind error!
                continue;
            }
          }
          /** END: Modified for DTS2015072304117  by baihaili(b00311320), 2015/7/25 */

          if(typefind_size == max_size)
          {
            break;
          }
        }
        ...
        gchar* caps_tostring = gst_caps_to_string(found_caps);
        GST_INFO("Found caps=%s", caps_tostring);
    ...
    GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      g_atomic_int_set (&typefind->shutdown, 0);
      GST_INFO ("gsttypefind set shutdown to 0");
      break;
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      g_atomic_int_set (&typefind->shutdown, 1);
      GST_INFO ("gsttypefind set shutdown to 1");
      break;
    default:
      break;
  }
