...
post_start:
  if (amessage) {
    ...
    GST_INFO_OBJECT (bin, "posting ASYNC_START to parent");
...
had_error:
  {
    GST_CAT_INFO_OBJECT (GST_CAT_STATES, bin, "we had an error");
    return;
  }
was_busy:
  {
    GST_CAT_INFO_OBJECT (GST_CAT_STATES, bin, "state change busy");
    GST_OBJECT_UNLOCK (bin);
    goto post_start;
  }
was_no_preroll:
  {
    GST_CAT_INFO_OBJECT (GST_CAT_STATES, bin, "ignoring, we are NO_PREROLL");
    GST_OBJECT_UNLOCK (bin);
    goto post_start;
  }
}
    ...
    gboolean flag_pending, GstClockTime running_time)
{
  ...
  GST_INFO("bin_handle_async_done, %s", GST_OBJECT_NAME(bin)?GST_OBJECT_NAME(bin):"NULL bin");
  if (GST_STATE_RETURN (bin) == GST_STATE_CHANGE_FAILURE)
    goto had_error;
  ...
  if (amessage) {
    ...
    GST_INFO_OBJECT (bin, "posting ASYNC_DONE to parent,%s",  GST_OBJECT_NAME(bin)?GST_OBJECT_NAME(bin):"NULL bin");
    gst_element_post_message (GST_ELEMENT_CAST (bin), amessage);
  }
  ...
  if (cont) {
    ...
    GST_INFO_OBJECT (bin, "all async-done, starting state continue,%s",  GST_OBJECT_NAME(bin)?GST_OBJECT_NAME(bin):"NULL bin");
    bin_push_state_continue (cont);
  } else {
    GST_INFO_OBJECT (bin, "state change complete %s",  GST_OBJECT_NAME(bin)?GST_OBJECT_NAME(bin):"NULL bin");
...
had_error:
  {
    GST_CAT_INFO_OBJECT (GST_CAT_STATES, bin, "we had an error %s",  GST_OBJECT_NAME(bin)?GST_OBJECT_NAME(bin):"NULL bin");
    return;
  }
was_busy:
  {
    GST_CAT_INFO_OBJECT (GST_CAT_STATES, bin, "state change busy %s",  GST_OBJECT_NAME(bin)?GST_OBJECT_NAME(bin):"NULL bin");
...
nothing_pending:
  {
    GST_CAT_INFO_OBJECT (GST_CAT_STATES, bin, "nothing pending %s",  GST_OBJECT_NAME(bin)?GST_OBJECT_NAME(bin):"NULL bin");
    return;
  }
}
...
gst_bin_handle_message_func (GstBin * bin, GstMessage * message)
{
  ...
  switch (type) {
    ...
    case GST_MESSAGE_ASYNC_START:
    {
      ...
      GST_INFO_OBJECT (bin, "ASYNC_START message %p, %s,binname=%s", message,
          src ? GST_OBJECT_NAME (src) : "(NULL)", GST_OBJECT_NAME(bin)?GST_OBJECT_NAME(bin):"NULL bin");
    ...
    case GST_MESSAGE_ASYNC_DONE:
    {
      ...
      GST_INFO_OBJECT (bin, "ASYNC_DONE message %p, %s,binname=%s", message,
          src ? GST_OBJECT_NAME (src) : "(NULL)",GST_OBJECT_NAME(bin)?GST_OBJECT_NAME(bin):"NULL bin");
