...
#if defined (SME_AUDIO_BITSTREAM)
#define PENDING_AUDIO_BITSTREAM_BLOCK(playsink) \
  ((playsink->pending_blocked_pads) & (1 << GST_PLAY_SINK_TYPE_AUDIO_BITSTREAM))
#endif
...
struct _GstPlaySink
{
...
#if defined (SME_AUDIO_BITSTREAM)
  GstPlayAudioBitstreamChain *audiobitstreamchain;
#endif
  ...
  /* audio bitstream */
#if defined (SME_AUDIO_BITSTREAM)
  GstPad *audio_bitstream_pad;
  gboolean audio_bitstream_pad_blocked;
  GstPad *audio_bitstream_srcpad_stream_synchronizer;
  GstPad *audio_bitstream_sinkpad_stream_synchronizer;
  gulong audio_bitstream_block_id;
  gulong audio_bitstream_notify_caps_id;
#endif
...
GST_STATIC_PAD_TEMPLATE ("audio_sink",
    ...
    GST_STATIC_CAPS_ANY);
#if defined (SME_AUDIO_BITSTREAM)
static GstStaticPadTemplate audiobitstreamtemplate =
GST_STATIC_PAD_TEMPLATE ("audio_bitstream_sink",
    GST_PAD_SINK,
    GST_PAD_REQUEST,
    GST_STATIC_CAPS_ANY);
#endif
...
#if defined (SME_AUDIO_BITSTREAM)
  /**
   * GstPlaySink:audio-bitstream-sink:
   *
   * Set the used audio bitstream sink element. NULL will use the default sink.
   * playsink must be in %GST_STATE_NULL
   *
   */
  g_object_class_install_property (gobject_klass, PROP_AUDIO_BITSTREAM_SINK,
      g_param_spec_object ("audio-bitstream-sink", "Audio Bitstream Sink",
          "the audio bitstream output element to use (NULL = default sink)",
          GST_TYPE_ELEMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
...
#if defined (SME_AUDIO_BITSTREAM)
  gst_element_class_add_pad_template (gstelement_klass,
      gst_static_pad_template_get (&audiobitstreamtemplate));
#endif
  gst_element_class_add_pad_template (gstelement_klass,
...
gst_play_sink_init (GstPlaySink * playsink)
{
  ...
  playsink->vsink_proc_info = NULL;
  playsink->asink_proc_info = NULL;
#if defined (SME_AUDIO_BITSTREAM)
  playsink->audio_bitstream_sink = NULL;
#endif
...
#if defined (SME_AUDIO_BITSTREAM)
  if (playsink->audio_bitstream_sink != NULL) {
    gst_element_set_state (playsink->audio_bitstream_sink, GST_STATE_NULL);
    gst_object_unref (playsink->audio_bitstream_sink);
    playsink->audio_bitstream_sink = NULL;
  }
#endif
...
#if defined (SME_AUDIO_BITSTREAM)
  free_chain ((GstPlayChain *) playsink->audiobitstreamchain);
  playsink->audiobitstreamchain = NULL;
#endif
    ...
    GstElement * sink)
{
  ...
  switch (type) {
    ...
    case GST_PLAY_SINK_TYPE_AUDIO_BITSTREAM:
      elem = &playsink->audio_bitstream_sink;
      break;
#endif
...
gst_play_sink_get_sink (GstPlaySink * playsink, GstPlaySinkType type)
{
  ...
  switch (type) {
    ...
    case GST_PLAY_SINK_TYPE_AUDIO_BITSTREAM:
    {
      GstPlayAudioBitstreamChain *chain;
      if (chain = playsink->audiobitstreamchain)
        chainp = chain->sink;
      elem = playsink->audio_bitstream_sink;
      break;
    }
...
#if defined (SME_AUDIO_BITSTREAM)
/* make the chain that contains the elements needed to perform
 * audio bitstream playback.
 *
 *  +------------------------------------------------+
 *  | abbin                                          |
 *  |      +---------+  +----------------------+     |
 *  |      |  queue  |  | audio-bitstream-sink |     |
 *  |   +-srck      src-sink                   |     |
 *  |   |  +---------+  +----------------------+     |
 * sink-+                                            |
 *  +------------------------------------------------+
 */
static GstPlayAudioBitstreamChain *
gen_audio_bitstream_chain (GstPlaySink * playsink)
{
  GstPlayAudioBitstreamChain *chain;
  GstBin *bin;
  GstPad *pad;
  ...
  chain = g_new0 (GstPlayAudioBitstreamChain, 1);
  chain->chain.playsink = playsink;

  GST_DEBUG_OBJECT (playsink, "making audio bitstream chain %p", chain);

  if (playsink->audio_bitstream_sink) {
    GST_DEBUG_OBJECT (playsink, "trying configured audiobitstreamsink"
                      " %" GST_PTR_FORMAT,
                      playsink->audio_bitstream_sink);
    chain->sink = try_element (playsink, playsink->audio_bitstream_sink, FALSE);
  } else {
    /* only try fallback if no specific sink was chosen */
    if (chain->sink == NULL) {
      GST_DEBUG_OBJECT (playsink, "bitstream chain trying autoaudiosink");
      elem = gst_element_factory_make ("autoaudiosink", "audiosink");
      chain->sink = try_element (playsink, elem, TRUE);
    }
    if (chain->sink == NULL) {
      /* if default sink from config.h is different then try it too */
      if (strcmp (DEFAULT_AUDIOSINK, "autoaudiosink")) {
        GST_DEBUG_OBJECT (playsink, "bitstream trying " DEFAULT_AUDIOSINK);
        elem = gst_element_factory_make (DEFAULT_AUDIOSINK, "audiosink");
        chain->sink = try_element (playsink, elem, TRUE);
      }
    }
    if (chain->sink)
      playsink->audio_bitstream_sink = gst_object_ref (chain->sink);
  }
  if (chain->sink == NULL)
    goto no_sinks;

  chain->chain.bin = gst_bin_new ("abbin");
  bin = GST_BIN_CAST (chain->chain.bin);
  gst_object_ref_sink (bin);
  gst_bin_add (bin, chain->sink);

  /* we have to add a queue when we need to decouple
   * for the audio bitstream sink and for streamsynchronizer */
  GST_DEBUG_OBJECT (playsink, "adding audio bitstream queue");
  chain->queue = gst_element_factory_make ("queue", "aqueue");
  if (chain->queue == NULL) {
    post_missing_element_message (playsink, "queue");
    GST_ELEMENT_WARNING (playsink, CORE, MISSING_PLUGIN,
        (_("Missing element '%s' - check your GStreamer installation."),
            "queue"), ("audio bitstream playback might not work"));
    head = chain->sink;
    prev = NULL;
  } else {
    g_object_set (chain->queue, "silent", TRUE, NULL);
...
no_sinks:
  {
    if (!elem && !playsink->audio_bitstream_sink) {
      post_missing_element_message (playsink, "autoaudiosink");
      if (strcmp (DEFAULT_AUDIOSINK, "autoaudiosink")) {
        post_missing_element_message (playsink, DEFAULT_AUDIOSINK);
        GST_ELEMENT_ERROR (playsink, CORE, MISSING_PLUGIN,
      ...
      if (playsink->audio_bitstream_sink) {
        GST_ELEMENT_ERROR (playsink, CORE, STATE_CHANGE,
            (_("Configured audiosink %s is not working."),
                GST_ELEMENT_NAME (playsink->audio_bitstream_sink)), (NULL));
      } else if (strcmp (DEFAULT_AUDIOSINK, "autoaudiosink")) {
        GST_ELEMENT_ERROR (playsink, CORE, STATE_CHANGE,
            (_("Both autoaudiosink and %s elements are not working."),
                DEFAULT_AUDIOSINK), (NULL));
...
static gboolean
setup_audio_bitstream_chain (GstPlaySink * playsink)
{
  GstElement *elem;
  GstPlayAudioBitstreamChain *chain;
  GstStateChangeReturn ret;

  chain = playsink->audiobitstreamchain;
...
#if defined (SME_AUDIO_BITSTREAM)
  gboolean need_audio_bitstream = FALSE;
#endif
...
#if defined (SME_AUDIO_BITSTREAM)
  if (playsink->audio_bitstream_pad) {
    /** TODO: do we need to add a new flag for bitstream? */
    if ((flags & GST_PLAY_FLAG_AUDIO) || (flags & GST_PLAY_FLAG_NATIVE_AUDIO)) {
      need_audio_bitstream = TRUE;
    }
  }
#endif
...
#if defined (SME_AUDIO_BITSTREAM)
  if (need_audio_bitstream) {
    GST_DEBUG_OBJECT (playsink, "adding audio bitstream");

    if (playsink->audiobitstreamchain) {
      /* try to reactivate the chain */
      if ((playsink->audio_bitstream_sink
           && playsink->audio_bitstream_sink !=
           playsink->audiobitstreamchain->sink)
          || !setup_audio_bitstream_chain (playsink)) {
        GST_DEBUG_OBJECT (playsink, "removing current audio bitstream chain");

        if (playsink->audio_bitstream_sinkpad_stream_synchronizer) {
          gst_element_release_request_pad (GST_ELEMENT_CAST
              (playsink->stream_synchronizer),
              playsink->audio_bitstream_sinkpad_stream_synchronizer);
          GstPad *srcpad = NULL, *sinkpad = NULL;

          sinkpad = playsink->audio_bitstream_sinkpad_stream_synchronizer;
          gst_object_unref (sinkpad);
          playsink->audio_bitstream_sinkpad_stream_synchronizer = NULL;
          srcpad = playsink->audio_bitstream_srcpad_stream_synchronizer;
          gst_object_unref (srcpad);
          playsink->audio_bitstream_srcpad_stream_synchronizer = NULL;
        }

        add_chain (GST_PLAY_CHAIN (playsink->audiobitstreamchain), FALSE);

        /* Remove the sink from the bin to keep its state
         * and unparent it to allow reuse */
        if (playsink->audiobitstreamchain->sink) {
          if (playsink->audiobitstreamchain->sink !=
              playsink->audio_bitstream_sink) {
            gst_element_set_state (playsink->audiobitstreamchain->sink,
                                   GST_STATE_NULL);
          }

          gst_bin_remove (
              GST_BIN_CAST (playsink->audiobitstreamchain->chain.bin),
              playsink->audiobitstreamchain->sink);
        }

        activate_chain (GST_PLAY_CHAIN (playsink->audiobitstreamchain), FALSE);
        if (playsink->audiobitstreamchain->ts_offset)
          gst_object_unref (playsink->audiobitstreamchain->ts_offset);
        playsink->audiobitstreamchain->ts_offset = NULL;
        free_chain ((GstPlayChain *) playsink->audiobitstreamchain);
        playsink->audiobitstreamchain = NULL;
      }
    }
    ...
    if (!playsink->audiobitstreamchain) {
      GST_DEBUG_OBJECT (playsink, "creating new audio bitstream chain");
      playsink->audiobitstreamchain = gen_audio_bitstream_chain (playsink);
    }
    ...
    if (!playsink->audio_bitstream_sinkpad_stream_synchronizer) {
      ...
      g_assert (it);
      gst_iterator_next (it, &item);
      playsink->audio_bitstream_srcpad_stream_synchronizer =
          g_value_dup_object (&item);
      g_value_unset (&item);
      g_assert (playsink->audio_bitstream_srcpad_stream_synchronizer);
      gst_iterator_free (it);
    }

    if (playsink->audio_bitstream_pad)
      gst_ghost_pad_set_target (
          GST_GHOST_PAD_CAST (playsink->audio_bitstream_pad),
          playsink->audio_bitstream_sinkpad_stream_synchronizer);

    if (playsink->audiobitstreamchain) {
      GST_DEBUG_OBJECT (playsink, "adding audio bitstream chain");
      add_chain (GST_PLAY_CHAIN (playsink->audiobitstreamchain), TRUE);
      activate_chain (GST_PLAY_CHAIN (playsink->audiobitstreamchain), TRUE);
      gst_pad_link_full (playsink->audio_bitstream_srcpad_stream_synchronizer,
          playsink->audiobitstreamchain->sinkpad, GST_PAD_LINK_CHECK_NOTHING);
    }
  } else {
    GST_DEBUG_OBJECT (playsink, "no audio bitstream needed");
    /* we have no audio bitstream or
     * we are requested to not play audio bitstream
     */
    if (playsink->audiobitstreamchain) {
      GST_DEBUG_OBJECT (playsink, "removing audio bitstream chain");
      if (playsink->audio_bitstream_sinkpad_stream_synchronizer) {
        gst_element_release_request_pad (GST_ELEMENT_CAST
            (playsink->stream_synchronizer),
            playsink->audio_bitstream_sinkpad_stream_synchronizer);
        gst_object_unref(playsink->audio_bitstream_sinkpad_stream_synchronizer);
        playsink->audio_bitstream_sinkpad_stream_synchronizer = NULL;
        gst_object_unref (playsink->audio_bitstream_srcpad_stream_synchronizer);
        playsink->audio_bitstream_srcpad_stream_synchronizer = NULL;
      }
...
#if defined (SME_AUDIO_BITSTREAM)
  GstPlayAudioBitstreamChain *abchain;
#endif
...
#if defined (SME_AUDIO_BITSTREAM)
  abchain = (GstPlayAudioBitstreamChain *)playsink->audiobitstreamchain;
#endif

  if (achain && vchain && achain->ts_offset && vchain->ts_offset
#if defined (SME_AUDIO_BITSTREAM)
      && abchain && abchain->ts_offset
#endif
      ) {
    ...
    g_object_set (vchain->ts_offset,
        "ts-offset", MAX (G_GINT64_CONSTANT (0), av_offset), NULL);
#if defined (SME_AUDIO_BITSTREAM)
    g_object_set (abchain->ts_offset,
        "ts-offset", MAX (G_GINT64_CONSTANT (0), -av_offset), NULL);
#endif
  } else {
...
#if defined (SME_AUDIO_BITSTREAM)
static void
audio_bitstream_set_blocked (GstPlaySink * playsink, gboolean blocked)
{
  if (playsink->audio_bitstream_pad) {
    GstPad *opad =
        GST_PAD_CAST (gst_proxy_pad_get_internal (GST_PROXY_PAD
            (playsink->audio_bitstream_pad)));
    if (blocked && playsink->audio_bitstream_block_id == 0) {

      playsink->audio_bitstream_block_id =
          gst_pad_add_probe (opad, GST_PAD_PROBE_TYPE_BLOCK_DOWNSTREAM,
          sinkpad_blocked_cb, playsink, NULL);
    } else if (!blocked && playsink->audio_bitstream_block_id) {
      gst_pad_remove_probe (opad, playsink->audio_bitstream_block_id);
      PENDING_FLAG_UNSET (playsink, GST_PLAY_SINK_TYPE_AUDIO_BITSTREAM);
      playsink->audio_bitstream_block_id = 0;
      playsink->audio_bitstream_pad_blocked = FALSE;
    }
    gst_object_unref (opad);
  }
}
...
#if defined (SME_AUDIO_BITSTREAM)
  audio_bitstream_set_blocked(playsink, TRUE);
#endif
  GST_PLAY_SINK_UNLOCK (playsink);
...
#if defined (SME_AUDIO_BITSTREAM)
  else if (pad == playsink->audio_bitstream_pad) {
    playsink->audio_bitstream_pad_blocked = TRUE;
    GST_DEBUG_OBJECT (pad, "Audio bitstream pad blocked");
  }
#endif
...
#if defined (SME_AUDIO_BITSTREAM)
      && (!playsink->audio_bitstream_pad
          || playsink->audio_bitstream_pad_blocked
          || !PENDING_AUDIO_BITSTREAM_BLOCK(playsink))
#endif
      ) {
...
#if defined (SME_AUDIO_BITSTREAM)
    audio_bitstream_set_blocked(playsink, FALSE);
#endif
...
caps_notify_cb (GstPad * pad, GParamSpec * unused, GstPlaySink * playsink)
{
  ...
  if (pad == playsink->audio_pad) {
    ...
    //reconfigure = playsink->audiobitstreamchain;
    GST_DEBUG_OBJECT (pad,
        "Audio bitstream caps changed: raw %d reconfigure %d "
        "caps %" GST_PTR_FORMAT,
        raw, reconfigure, caps);
  }
#endif
    ...
    GstPlaySinkType type)
{
  ...
  if (pad == playsink->video_pad) {
    ...
    if (type != GST_PLAY_SINK_TYPE_AUDIO_BITSTREAM)
      goto wrong_type;
    block_id = &playsink->audio_bitstream_block_id;
  }
#endif
...
#if defined (SME_AUDIO_BITSTREAM)
    case GST_PLAY_SINK_TYPE_AUDIO_BITSTREAM:
      pad_name = "audio_bitstream_sink";
      if (!playsink->audio_bitstream_pad) {
        GST_LOG_OBJECT (playsink, "ghosting audio bitstream sink");
        playsink->audio_bitstream_pad =
            gst_ghost_pad_new_no_target (pad_name, GST_PAD_SINK);
        playsink->audio_bitstream_notify_caps_id =
            g_signal_connect (G_OBJECT (playsink->audio_bitstream_pad),
                              "notify::caps",
                              G_CALLBACK (caps_notify_cb), playsink);
        created = TRUE;
      }
      res = playsink->audio_bitstream_pad;
      block_id = &playsink->audio_bitstream_block_id;
      break;
#endif
    case GST_PLAY_SINK_TYPE_VIDEO_RAW:
...
#if defined (SME_AUDIO_BITSTREAM)
  else if (!strcmp (tplname, "audio_bitstream_sink"))
...
gst_play_sink_release_pad (GstPlaySink * playsink, GstPad * pad)
{
  ...
  if (pad == playsink->video_pad) {
    ...
    res = &playsink->audio_bitstream_pad;
    g_signal_handler_disconnect (playsink->audio_bitstream_pad,
        playsink->audio_bitstream_notify_caps_id);
    audio_bitstream_set_blocked (playsink, FALSE);
#endif
  } else {
...
gst_play_sink_handle_message (GstBin * bin, GstMessage * message)
{
  ...
  switch (GST_MESSAGE_TYPE (message)) {
    case GST_MESSAGE_STEP_DONE:
    {
      ...
      /** TODO: do we need to send the same event to bitstream chain? */
...
gst_play_sink_send_event_to_sink (GstPlaySink * playsink, GstEvent * event)
{
  ...
  if (playsink->send_event_mode == MODE_FIRST) {
    /* BEGIN: Modified for DTS2015072205123 by liurongliang(l00180035), 2015/7/26 */
    /*we should first text than video or audio*/
    gst_stream_synchronizer_refresh_all_segment(playsink->stream_synchronizer);
    if (playsink->textchain && playsink->textchain->sink) {
      gst_event_ref (event);
      if ((res =
              gst_element_send_event (playsink->textchain->chain.bin, event))) {
        GST_INFO_OBJECT (playsink, "Sent event successfully to text sink");
      } else {
        GST_WARNING_OBJECT (playsink, "Event failed when sent to text sink");
      }
    }
    ...
    if (playsink->videochain) {
      ...
      if ((res =
              gst_element_send_event (playsink->videochain->chain.bin,
                  event))) {
        GST_INFO_OBJECT (playsink, "Sent event successfully to video sink");
        goto done;
      }
      GST_WARNING_OBJECT (playsink, "Event failed when sent to video sink");
    }
    if (playsink->audiochain) {
      gst_event_ref (event);
      if ((res =
              gst_element_send_event (playsink->audiochain->chain.bin,
                  event))) {
        GST_INFO_OBJECT (playsink, "Sent event successfully to audio sink");
        goto done;
      }
      GST_WARNING_OBJECT (playsink, "Event failed when sent to audio sink");
    }
#if defined (SME_AUDIO_BITSTREAM)
    if (playsink->audiobitstreamchain) {
      gst_event_ref (event);
      if ((res =
              gst_element_send_event (playsink->audiobitstreamchain->chain.bin,
                  event))) {
        GST_INFO_OBJECT (playsink, "Sent event successfully to "
                         "audio bitstream sink");
        goto done;
      }
      GST_WARNING_OBJECT (playsink, "Event failed when sent to "
                          "audio bitstream sink");
    }
#endif
    /* END:   Modified for DTS2015072205123 by liurongliang(l00180035), 2015/7/26 */
  } else {
    return
...
#if defined (SME_AUDIO_BITSTREAM)
      if (playsink->audiobitstreamchain &&
          playsink->audiobitstreamchain->ts_offset) {
        gst_object_unref (playsink->audiobitstreamchain->ts_offset);
        playsink->audiobitstreamchain->ts_offset = NULL;
      }
#endif
    ...
    const GValue * value, GParamSpec * spec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_AUDIO_SINK:
      ...
      break;
#if defined (SME_AUDIO_BITSTREAM)
    case PROP_AUDIO_BITSTREAM_SINK:
      gst_play_sink_set_sink (playsink, GST_PLAY_SINK_TYPE_AUDIO_BITSTREAM,
          g_value_get_object (value));
      break;
#endif
      case PROP_TEXT_SINK:
    ...
    case PROP_VSINK_INFO:{
      playsink->vsink_proc_info = g_value_get_pointer(value);
      if (playsink->video_sink) {
        g_object_set(playsink->video_sink, "vsink-proc-info",
          playsink->vsink_proc_info, NULL);
      }
      break;
    }
    case PROP_ASINK_INFO:{
      playsink->asink_proc_info = g_value_get_pointer(value);
      if (playsink->audio_sink) {
        g_object_set(playsink->audio_sink, "asink-proc-info",
          playsink->asink_proc_info, NULL);
      }
      break;
    }
