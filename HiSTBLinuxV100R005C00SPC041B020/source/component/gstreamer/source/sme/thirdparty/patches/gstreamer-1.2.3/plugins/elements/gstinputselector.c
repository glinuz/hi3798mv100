...
gst_selector_pad_cache_buffer (GstSelectorPad * selpad, GstBuffer * buffer)
{
  if (selpad->segment.format != GST_FORMAT_TIME) {
    GST_DEBUG_OBJECT (selpad, "Buffer %p with segment not in time format, "
        "not caching", buffer);
    /*Fix buffer leak in sync_streams & cache_buffers mode,merge from gstreamer 1.6.1*/
...
gst_selector_pad_event (GstPad * pad, GstObject * parent, GstEvent * event)
{
  ...
  if (forward) {
    ...
    /* Send caps event to down stream for multi-audio-track switch scenario.
     * Because in multi-audio-track switch scenario, if sample rate is different,
     * the new active audio decoder would not send caps to downstream properly,
     * so when flush_stop event sent from multi-queue is received, send caps to down stream.
     * Edward Lee.  2015/12/30th.
     */
    if (GST_EVENT_TYPE (event) == GST_EVENT_FLUSH_STOP && sel->is_multi_audio_track_switching) {
      GstCaps * caps = gst_pad_get_current_caps(pad);
      gchar* caps_tostring = gst_caps_to_string(caps);

      GST_INFO("multi_audio_track_switching, send caps to downstream, caps=%s",
        caps_tostring);
      g_free(caps_tostring);

      res= gst_pad_set_caps(sel->srcpad, caps);
      gst_caps_unref (caps);

      sel->is_multi_audio_track_switching = FALSE;
    }
  } else {
...
gst_input_selector_set_active_pad (GstInputSelector * self, GstPad * pad)
{
  ...
  /* Get audio sink pad caps */
  sinkpad_caps = gst_pad_get_current_caps(pad);
  str_caps = gst_caps_to_string(sinkpad_caps);

  if (0 == g_strncasecmp(str_caps, "audio/", strlen("audio/"))) {
    self->is_multi_audio_track_switching = TRUE;

    GST_INFO_OBJECT (self,
      "set multi_audio_track_switching = TRUE, New active pad is %p",
      self->active_sinkpad);
  }
