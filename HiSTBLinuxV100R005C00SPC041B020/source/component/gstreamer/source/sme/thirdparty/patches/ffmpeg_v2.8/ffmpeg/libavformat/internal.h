...
/* changed: */
#define CHECK_NET_INTERRUPT(ret) \
     (  ret == AVERROR(ETIMEDOUT) || ret == AVERROR(ECONNRESET)    \
     || ret == AVERROR(EPROTOTYPE) || ret == AVERROR(ENOPROTOOPT)  \
     || ret == AVERROR(EADDRNOTAVAIL) || ret == AVERROR(ENETDOWN)  \
     || ret == AVERROR(ENETUNREACH) || ret == AVERROR(ENETRESET)   \
     || ret == AVERROR(ECONNABORTED) || ret == AVERROR(ECONNRESET) \
...
/* changed: return AVERROR_EXIT if interrupted. */
int av_usleep_interruptible(unsigned usec, AVIOInterruptCB *cb);
/* changed end */
