    ...
    GstBuffer * buf)
{
  ...
  if (G_UNLIKELY (src->read_position != offset)) {
    ...
    //to use "lseek64", macro "_LARGEFILE64_SOURCE" is defined.
    res = lseek64 (src->fd, offset, SEEK_SET);
    if (G_UNLIKELY (res < 0 || res != offset)) {
        GST_ERROR("lseek64, res=%llu, offset=%llu", res, offset);
        goto seek_failed;
    }
...
gst_file_src_start (GstBaseSrc * basesrc)
{
  ...
  {
    off64_t res = lseek64 (src->fd, 0, SEEK_END);

    if (res < 0) {
      GST_WARNING_OBJECT (src, "seekable=FALSE, disabling seeking, lseek failed: %s",
          g_strerror (errno));
      src->seekable = FALSE;
    } else {
      GST_INFO_OBJECT (src, "seekable=TRUE");
      src->seekable = TRUE;
    }
    lseek64 (src->fd, 0, SEEK_SET);
  }
...
open_failed:
  {
    switch (errno) {
      ...
      default:
        ...
        //    GST_ERROR_SYSTEM);
        GST_ELEMENT_ERROR (src, RESOURCE, OPEN_READ,
            (_("Could not open file for reading.")), GST_ERROR_SYSTEM);
        break;
    }
    goto error_exit;
  }
...
was_directory:
  {
    ...
    GST_ELEMENT_ERROR (src, RESOURCE, OPEN_READ,
        (_("filename is a directory.")), (NULL));
    goto error_close;
  }
was_socket:
  {
    ...
    GError ** err)
{
  ...
  if (!location || (err != NULL && *err != NULL)) {
    ...
    GST_WARNING_OBJECT (src, "Invalid URI for filesrc: %s",
        (err != NULL && *err != NULL) ? (*err)->message : "unknown error");
    goto beach;
  }
