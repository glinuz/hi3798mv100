...
static GstMessageQuarks message_quarks[] = {
  ...
  {GST_MESSAGE_NEW_BANDWIDTH, "new-bandwidth", 0},
  {GST_MESSAGE_RESOLUTION_CHANGED, "resolution-set", 0},
  {GST_MESSAGE_NEW_HTTP_ERR, "new-http-err", 0},
  {GST_MESSAGE_HAVE_NO_STREAM, "have-no-stream", 0},
  {0, NULL, 0}
};
...
GstMessage *
gst_message_new_resolution_changed(GstObject * src, gulong width, gulong height)
{
  GstMessage *message;
  GstStructure *structure;
  structure = gst_structure_new_id (GST_QUARK (RESOLUTION),
      GST_QUARK (WIDTH), G_TYPE_ULONG, width,
      GST_QUARK (HEIGHT), G_TYPE_ULONG, height, NULL);
  message = gst_message_new_custom (GST_MESSAGE_RESOLUTION_CHANGED, src, structure);
  return message;
}
void
gst_message_parse_resolution_changed (GstMessage * message, gulong * width, gulong * height)
{
  g_return_if_fail (GST_IS_MESSAGE (message));
  g_return_if_fail (GST_MESSAGE_TYPE (message) == GST_MESSAGE_RESOLUTION_CHANGED);
  if (width)
    *width =
        g_value_get_ulong (gst_structure_id_get_value (GST_MESSAGE_STRUCTURE
            (message), GST_QUARK (WIDTH)));
  if (height)
    *height =
        g_value_get_ulong (gst_structure_id_get_value (GST_MESSAGE_STRUCTURE
            (message), GST_QUARK (HEIGHT)));
  return ;
}

/**
 * gst_message_have_no_stream:
 * @src: (transfer none): The object originating the message.
 *
 * This message is posted by elements(tsdmx) when there are no stream.
 *
 * Returns: (transfer full): The have_no_stream message.
 *
 * MT safe.
 */
GstMessage *
gst_message_have_no_stream (GstObject * src)
{
  GstMessage *message;

  message = gst_message_new_custom (GST_MESSAGE_HAVE_NO_STREAM, src, NULL);

  return message;
}

/**
 * GST_MESSAGE_STREAM_RECOVER:
 * @src: (transfer none): The object originating the message.
 *
 * This message is posted by elements(tsdmx) when dvb stream recover by stream.
 *
 * Returns: (transfer full): The stream_recover message.
 *
 * MT safe.
 */
GstMessage * gst_message_stream_recover (GstObject * src)
{
    GstMessage *message;

    message = gst_message_new_custom (GST_MESSAGE_STREAM_RECOVER, src, NULL);
...
/**
 * gst_message_set_buffering:
 * @message: A valid #GstMessage of type GST_MESSAGE_BUFFERING.
 * @percent: (in) : new percent set to the message
 *
 * set the buffering percent to the GstMessage. see also
 * gst_message_new_buffering().
 *
 * MT safe.
 */
void
gst_message_set_buffering (GstMessage * message, gint percent)
{
  GstStructure *structure;
...
/**
 * gst_message_new_bandwidth_changed:
 * @src: (transfer none): The object originating the message.
 *
 * Create a new bandwidth changed message. This message is posted by elements
 * that know the bandwidth of a stream when the bandwidth changes. This message
 * is received by bins and is used to calculate the total bandwidth of a
 * pipeline. The new bandwidth can then be retrieved via a query.
 *
 * Returns: (transfer full): The new bandwidth message.
 *
 * MT safe.
 */
GstMessage *
gst_message_new_new_bandwidth (GstObject * src, guint bandwidth)
{

  GstMessage *message;
  GstStructure *structure;

  structure = gst_structure_new_id (GST_QUARK (MESSAGE_NEW_BANDWIDTH),
    GST_QUARK (BANDWIDTH), G_TYPE_UINT, bandwidth, NULL);
  message = gst_message_new_custom (GST_MESSAGE_NEW_BANDWIDTH, src, structure);
...
void
gst_message_parse_new_bandwidth (GstMessage * message, guint * bandwidth)
{
  g_return_if_fail (GST_IS_MESSAGE (message));
  g_return_if_fail (GST_MESSAGE_TYPE (message) == GST_MESSAGE_NEW_BANDWIDTH);

  if (bandwidth)
    *bandwidth = g_value_get_uint (gst_structure_id_get_value (GST_MESSAGE_STRUCTURE(message),
      GST_QUARK (BANDWIDTH)));
}

GstMessage *
gst_message_new_http_err (GstObject * src, guint statuscode)
{
  GstMessage *message;
  GstStructure *structure;

  structure = gst_structure_new_id (GST_QUARK (MESSAGE_NEW_HTTP_ERR),
    GST_QUARK (STATUS_CODE), G_TYPE_UINT, statuscode, NULL);
  message = gst_message_new_custom (GST_MESSAGE_NEW_HTTP_ERR, src, structure);

  return message;
}

void
gst_message_parse_new_http_err (GstMessage * message, guint * statuscode)
{
  g_return_if_fail (GST_IS_MESSAGE (message));
  g_return_if_fail (GST_MESSAGE_TYPE (message) == GST_MESSAGE_NEW_HTTP_ERR);

  if (statuscode)
    *statuscode = g_value_get_uint (gst_structure_id_get_value (GST_MESSAGE_STRUCTURE(message),
      GST_QUARK (STATUS_CODE)));
}

/**
 * GST_MESSAGE_RENDER_FIRST_AUDIO_FRAME:
 * @src: (transfer none): The object originating the message.
 *
 * This message is posted by elements(asink) when first audio frame is begin to render.
 *
 * Returns: (transfer full): The GST_MESSAGE_RENDER_FIRST_AUDIO_FRAME message.
 *
 * MT safe.
 */
GstMessage * gst_message_new_render_first_audio_frame (GstObject * src)
{
    GstMessage *message;

    message = gst_message_new_custom (GST_MESSAGE_RENDER_FIRST_AUDIO_FRAME, src, NULL);

    return message;
}

/**
 * GST_MESSAGE_RENDER_FIRST_VIDEO_FRAME:
 * @src: (transfer none): The object originating the message.
 *
 * This message is posted by elements(vsink) when first audio frame is begin to render.
 *
 * Returns: (transfer full): The GST_MESSAGE_RENDER_FIRST_VIDEO_FRAME message.
 *
 * MT safe.
 */
GstMessage * gst_message_new_render_first_video_frame (GstObject * src)
{
    ...
    message = gst_message_new_custom (GST_MESSAGE_RENDER_FIRST_VIDEO_FRAME, src, NULL);
