...
#define ADEV_MAX_LENGTH (64)
#define ADEC_DUMP_FILE_STORE_PATH_LENGTH 256
...
typedef struct _ProcAdecInfo
{
    struct timeval          CreateAdecTime;
    unsigned int            MaxBufferSize;
    unsigned int            ADecGetFirstIFrameTimeFlag;
    struct timeval          ADecGetFirstIFrameTime;
    struct timeval          ADecFirstPlayTime;
    unsigned int            ADecFirstPlayTimeFlag;
    unsigned int            ADecErrorCnt;
    char                    ADecErrorInfo[ADEV_MAX_LENGTH*8];
}ST_ADEC_SHOW_INFO;
...
typedef struct __AudioDecCmdInfo
{
    ...
    char bDumpSrcStream;
    char cDumpFileStorePath[ADEC_DUMP_FILE_STORE_PATH_LENGTH];
}ST_ADEC_CMD_INFO;

typedef struct __AudioDecProcInifo {
...
#ifdef SMEPLAYER_BENCHMARK_OPEN
static void gst_audio_dump_sink_file(GstAudioDecoder * dec, const char * data, int len)
{
    if (dec->dump_sink_flag) {
        ...
        if (dec->dump_sink_file) {
            fwrite(data, 1, len, dec->dump_sink_file);
        }
    } else {
        if (dec->dump_sink_file) {
...
static void gst_audio_dump_src_file(GstAudioDecoder * dec, const char * data, int len)
{
    if (dec->dump_src_flag) {
        ...
        if (dec->dump_src_file) {
            fwrite(data, 1, len, dec->dump_src_file);
        }
    } else {
        if (dec->dump_src_file) {
...
#ifdef SMEPLAYER_BENCHMARK_OPEN
  memset(&dec->priv->audio_decoder_info,0,sizeof(dec->priv->audio_decoder_info));
  gettimeofday(&dec->priv->audio_decoder_info.CreateAdecTime, NULL);

  dec->dump_sink_file = NULL;
  dec->dump_sink_flag = FALSE;
  g_snprintf(dec->dump_sink_file_name,sizeof(dec->dump_sink_file_name),"/mnt/adec_in.es");

  dec->dump_src_file = NULL;
  dec->dump_src_flag = FALSE;
  g_snprintf(dec->dump_src_file_name,sizeof(dec->dump_src_file_name),"/mnt/adec_out.pcm");

  //GST_WARNING_OBJECT (dec, "dec %p, gst_audio_decoder_init dump_src_flag %s: %p,%s!",dec, dec->dump_src_flag?"true":"false",dec->dump_src_file,dec->dump_src_file_name);
#endif
...
#ifdef SMEPLAYER_BENCHMARK_OPEN
    dec->priv->audio_decoder_info.ADecGetFirstIFrameTimeFlag = 0;
    dec->priv->audio_decoder_info.ADecFirstPlayTimeFlag = 0;
    dec->priv->audio_decoder_info.ADecErrorCnt = 0;
#endif
...
#if defined (GST_AUDIO_DECODER_MULTI_OUTPUT)
  if (dec->bitstream_srcpad)
    dec->priv->bitstream_discont = TRUE;
#endif
...
gst_audio_decoder_push_event (GstAudioDecoder * dec, GstEvent * event)
{
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEGMENT:{
      ...
      GST_WARNING_OBJECT (dec, "%s:a-dec out segment[%lld:%lld]",
        SMEPLAYER_STAT_TAG_TIME, seg.start, seg.stop);
...
#if defined GST_AUDIO_DECODER_MULTI_OUTPUT
  if (dec->bitstream_srcpad) {
      /* ref the event since it needs to be pushed to bitstream src pad too */
      gst_event_ref(event);
  }
  if (!gst_pad_push_event(dec->srcpad, event))
      return FALSE;
  /* push the event to bitstream srcpad too */
  if (dec->bitstream_srcpad) {
      if (!gst_pad_push_event(dec->bitstream_srcpad, event)) {
          return FALSE;
      }
  }
...
gst_audio_decoder_negotiate_default (GstAudioDecoder * dec)
{
  ...
  if (!prevcaps || !gst_caps_is_equal (prevcaps, caps)) {
    ...
    if (!res) {
      ...
      GST_WARNING_OBJECT (dec, "set caps %s fail", caps_str);
      g_free(caps_str);
    }
  }
  if (prevcaps)
...
gst_audio_decoder_push_forward (GstAudioDecoder * dec, GstBuffer * buf)
{
  ...
  GST_DEBUG_OBJECT (dec,
      ...
      ", duration %" GST_TIME_FORMAT",outseg.start=%lld", gst_buffer_get_size (buf),
      GST_TIME_ARGS (GST_BUFFER_TIMESTAMP (buf)),
      GST_TIME_ARGS (GST_BUFFER_DURATION (buf)),
      dec->output_segment.start);
      //we donot clip here, it will clip in basesink.
#if 0
  /* clip buffer */
  buf = gst_audio_buffer_clip (buf, &dec->output_segment, ctx->info.rate,
      ctx->info.bpf);
  if (G_UNLIKELY (!buf)) {
    GST_WARNING_OBJECT (dec, "no data after clipping to segment");
    goto exit;
  }
#endif
  ...
  GST_LOG_OBJECT (dec,
    ...
    //GST_WARNING_OBJECT (dec, "dec %p, gst_audio_decoder_push dump_src_flag %s: %p,%s!",dec, dec->dump_src_flag?"true":"false",dec->dump_src_file,dec->dump_src_file_name);

    if (dec->dump_src_flag) {
        GstMapInfo info;
        gst_buffer_map (buf, &info, GST_MAP_READ);
        gst_audio_dump_src_file(dec,info.data,info.size);
        gst_buffer_unmap (buf, &info);
    } else {
        gst_audio_dump_src_file(dec,NULL,0);
    }

    if (!dec->priv->audio_decoder_info.ADecFirstPlayTimeFlag) {
        dec->priv->audio_decoder_info.ADecFirstPlayTimeFlag = 1;
        gettimeofday(&dec->priv->audio_decoder_info.ADecFirstPlayTime, NULL);
    }
#endif
    ...
    gint frames)
{
  ...
  /** BEGIN: Added for DTS2015071406258 by yaojian(y00342710), 2016/01/08*/
  } else if (G_UNLIKELY (frames > priv->frames.length && priv->frames.length > 0)) {
  /* END: Added for DTS2015071406258 by yaojian(y00342710), 2015/01/08*/
#ifdef FIX_INVALID_PTS
    if (0 && G_LIKELY (!priv->force)) {
#else
  ...
  //g_print("before finish_frame %d/%d\n",frames, priv->frames.length);

#if 1
  if (GST_CLOCK_TIME_IS_VALID(GST_BUFFER_TIMESTAMP(buf))) {
    while (priv->frames.length) {
      /* FIXME:
       * If there is timestamp jumpping within the queue, the queue may still
       * result in overflow error.
       */
      if ((!GST_CLOCK_TIME_IS_VALID(GST_BUFFER_TIMESTAMP(g_queue_peek_head(&priv->frames))) ||
           GST_BUFFER_TIMESTAMP(g_queue_peek_head(&priv->frames)) < GST_BUFFER_TIMESTAMP(buf)) ) {
        gst_buffer_unref (g_queue_pop_head (&priv->frames));
        dec->priv->ctx.delay = dec->priv->frames.length;
               frames --;
         //g_print("finish_frame xxx %d/%d\n",frames, priv->frames.length);
      } else if ((GST_BUFFER_TIMESTAMP(g_queue_peek_head(&priv->frames)) ==
          GST_BUFFER_TIMESTAMP(buf)) && equal_ts_count == 0) {
        /* To avoid an error case in which there are several frames with same
         * timestamp, thus multiple frames are popped and overflow error occurs.
         */
        gst_buffer_unref (g_queue_pop_head (&priv->frames));
        dec->priv->ctx.delay = dec->priv->frames.length;
        equal_ts_count++;
        //g_print("finish_frame yyy %d/%d\n",frames, priv->frames.length);
      } else {
        break;
      }
    }
  } else {
      //g_print("finish_frame: buf_pts invalid\n");
    ...
    while (priv->frames.length) {
       ...
       * result in overflow error.
       */
      if (GST_BUFFER_TIMESTAMP(g_queue_peek_head(&priv->frames)) <
...
overflow:
  {
#if 0
    //for we just post warning for audio dec failed.
    GST_ELEMENT_WARNING (dec, STREAM, ENCODE,
        ("received more decoded frames %d than provided %d", frames,
            priv->frames.length), ("audio_dec: received more decoded frames"));
#else
    GST_WARNING_OBJECT (dec,
        "received more decoded frames %d than provided %d",
        frames, priv->frames.length);
#endif
    if (buf)
      gst_buffer_unref (buf);
    ...
    GstAudioDecoderClass * klass, GstBuffer * buffer)
{
  ...
  ret = klass->handle_frame (dec, buffer);
  /** BEGIN: Added for DTS2015102108817 by qihuafei(q00307195), 2015/10/26*/
  if (ret != GST_FLOW_OK) {
    /* pop the buffer from queue */
    if (G_UNLIKELY(buffer)) {
      gst_buffer_unref (g_queue_pop_head (&dec->priv->frames));
      dec->priv->ctx.delay = dec->priv->frames.length;
    }
  } else {
#ifdef SMEPLAYER_BENCHMARK_OPEN
      if (!dec->priv->audio_decoder_info.ADecGetFirstIFrameTimeFlag) {
          dec->priv->audio_decoder_info.ADecGetFirstIFrameTimeFlag = 1;
          gettimeofday(&dec->priv->audio_decoder_info.ADecGetFirstIFrameTime, NULL);
      }

      if (dec->dump_sink_flag) {
          GstMapInfo info;
          gst_buffer_map (buffer, &info, GST_MAP_READ);
          gst_audio_dump_sink_file(dec,info.data,info.size);
          gst_buffer_unmap (buffer, &info);
      } else {
          gst_audio_dump_sink_file(dec,NULL,0);
      }
#endif

  }
  /** END: Added for DTS2015102108817 by qihuafei(q00307195), 2015/10/26*/
  return ret;
}
...
gst_audio_decoder_chain (GstPad * pad, GstObject * parent, GstBuffer * buffer)
{
  ...
  if (GST_BUFFER_FLAG_IS_SET (buffer, GST_BUFFER_FLAG_DISCONT)) {
    ...
    GST_INFO_OBJECT (dec, "handling discont");
    gst_audio_decoder_flush (dec, FALSE);
    dec->priv->discont = TRUE;
#if defined (GST_AUDIO_DECODER_MULTI_OUTPUT)
    if (dec->bitstream_srcpad)
...
gst_audio_decoder_sink_eventfunc (GstAudioDecoder * dec, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_GAP:
      ...
      break;
#if 1
    /* split the original sequence into two steps, one in flush-start and
     * another in flush-stop
     */
    case GST_EVENT_FLUSH_START:
    {
      ...
      GST_WARNING_OBJECT(dec, "%s:flush-start a-dec begin...",
          SMEPLAYER_STAT_TAG_TIME);
      /* Forward FLUSH_START, it is expected to be forwarded immediately
       * and no buffers are queued anyway. */
      GST_WARNING_OBJECT(dec, "%s:GST_EVENT_FLUSH_START a-dec begin...",
          SMEPLAYER_STAT_TAG_TIME);
      ret = gst_audio_decoder_push_event (dec, event);
      GST_WARNING_OBJECT(dec, "%s:GST_EVENT_FLUSH_START a-dec end(%d)",
          SMEPLAYER_STAT_TAG_TIME, ret);
      event = NULL;

      GST_AUDIO_DECODER_STREAM_LOCK (dec);

      GST_WARNING_OBJECT(dec, "%s:flush a-dec begin...",
          SMEPLAYER_STAT_TAG_TIME);
      gst_audio_decoder_clear_queues (dec);
       ...
       * and kicking out stuff */
      if (klass->flush && dec->priv->samples_out > 0)
        klass->flush (dec, TRUE);

      GST_WARNING_OBJECT(dec, "%s:flush a-dec end",
          SMEPLAYER_STAT_TAG_TIME);
      GST_AUDIO_DECODER_STREAM_UNLOCK (dec);
      GST_WARNING_OBJECT(dec, "%s:flush-start a-dec end",
          SMEPLAYER_STAT_TAG_TIME);
      break;
    }
    case GST_EVENT_FLUSH_STOP:
      GST_WARNING_OBJECT(dec, "%s:flush-stop a-dec begin...",
          SMEPLAYER_STAT_TAG_TIME);
      GST_AUDIO_DECODER_STREAM_LOCK (dec);
      gst_segment_init (&dec->input_segment, GST_FORMAT_TIME);
      gst_segment_init (&dec->output_segment, GST_FORMAT_TIME);
       ...
       * and no buffers are queued anyway. */
      ret = gst_audio_decoder_push_event (dec, event);
      GST_WARNING_OBJECT(dec, "%s:flush-stop a-dec end",
          SMEPLAYER_STAT_TAG_TIME);
      break;
#else
    case GST_EVENT_FLUSH_STOP:
    ...
    case GST_EVENT_TAG:
    {
      ...
      if (gst_tag_list_get_scope (tags) == GST_TAG_SCOPE_STREAM) {
        ...
        /* Send tag event immediately to downstream element,
         * otherwise, in multi audio track case, the tag may not be able to
         * arrive at inputselector.
         * It is conflict with the original logic in GstAudioDecoder. But it is
         * the easiest way to resolve the problem.
         */
        if (dec->priv->taglist && dec->priv->taglist_changed) {
          GST_INFO_OBJECT (dec, "codec tag %" GST_PTR_FORMAT,
              dec->priv->taglist);
  ...
  switch (prop_id) {
    ...
    case PROP_ADEC_INFO:
    {
      ...
      if (pstAdecProcInfo && pstAdecProcInfo->eProcCmdType == E_SME_PROC_CMD_SHOW_INFO) {
        ...
        memcpy(pstAdecShowInfo, &dec->priv->audio_decoder_info, sizeof(dec->priv->audio_decoder_info));
      ...
      else if (pstAdecProcInfo && pstAdecProcInfo->eProcCmdType == E_SME_PROC_CMD_SET_CMD)
      {
          ...
          if (pstAdecCmdInfo->bDumpSinkStream != 0XFF) {
              if (pstAdecCmdInfo->bDumpSinkStream == 1) {
                //begin to start dump sink data;
                struct timeval cur_time;
                gettimeofday(&cur_time, NULL);
                g_snprintf(dec->dump_sink_file_name,sizeof(dec->dump_sink_file_name),
          ...
          if (pstAdecCmdInfo->bDumpSrcStream != 0xFF) {
              if (pstAdecCmdInfo->bDumpSrcStream == 1) {
                //begin to start dump src data;
                struct timeval cur_time;
                gettimeofday(&cur_time, NULL);
                g_snprintf(dec->dump_src_file_name,sizeof(dec->dump_src_file_name),
...
gst_audio_decoder_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      GST_WARNING_OBJECT(codec, "%s:a-dec open begin...", SMEPLAYER_STAT_TAG_TIME);
      if (klass->open) {
        if (!klass->open (codec))
          goto open_failed;
      }
      GST_WARNING_OBJECT(codec, "%s:a-dec open end", SMEPLAYER_STAT_TAG_TIME);
      break;
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      GST_WARNING_OBJECT(codec, "%s:a-dec start begin...", SMEPLAYER_STAT_TAG_TIME);
      if (!gst_audio_decoder_start (codec)) {
        goto start_failed;
      }
      GST_WARNING_OBJECT(codec, "%s:a-dec start end", SMEPLAYER_STAT_TAG_TIME);
      break;
  ...
  switch (transition) {
    ...
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      GST_WARNING_OBJECT(codec, "%s:a-dec stop begin...", SMEPLAYER_STAT_TAG_TIME);
      if (!gst_audio_decoder_stop (codec)) {
        goto stop_failed;
      }
      GST_WARNING_OBJECT(codec, "%s:a-dec stop end", SMEPLAYER_STAT_TAG_TIME);
      break;
    case GST_STATE_CHANGE_READY_TO_NULL:
#ifdef SMEPLAYER_BENCHMARK_OPEN
      if (codec->dump_sink_file) {
          fclose(codec->dump_sink_file);
          codec->dump_sink_file = NULL;
      }
      codec->dump_sink_flag = FALSE;

      if (codec->dump_src_file) {
          fclose(codec->dump_src_file);
          codec->dump_src_file = NULL;
      }
      codec->dump_src_flag = FALSE;
#endif
      GST_WARNING_OBJECT(codec, "%s:a-dec close begin...", SMEPLAYER_STAT_TAG_TIME);
      if (klass->close) {
        if (!klass->close (codec))
          goto close_failed;
      }
      GST_WARNING_OBJECT(codec, "%s:a-dec close end", SMEPLAYER_STAT_TAG_TIME);
      break;
    default:
      break;
  }
...
#ifdef SMEPLAYER_BENCHMARK_OPEN
	dec->priv->audio_decoder_info.ADecErrorCnt = dec->priv->error_count;
    g_snprintf(dec->priv->audio_decoder_info.ADecErrorInfo,sizeof(dec->priv->audio_decoder_info.ADecErrorInfo),"%s",txt);
#endif
  dec->priv->discont = TRUE;
#if defined (GST_AUDIO_DECODER_MULTI_OUTPUT)
  if (dec->bitstream_srcpad)
...
#ifdef SMEPLAYER_BENCHMARK_OPEN
  dec->priv->audio_decoder_info.MaxBufferSize = size;
#endif
...
#if defined (GST_AUDIO_DECODER_MULTI_OUTPUT)
/**
 * gst_audio_decoder_pad_output:
 * @dec: a #GstAudioDecoder
 * @pad: the dedicated pad to push the buf
 * @buf: decoded data
 * @frames: number of decoded frames represented by decoded data
 *
 * Collects decoded data and pushes it downstream.
 * Compared with gst_audio_decoder_finish_frame, this function doesn't handle
 * timestamp, queue of frames and related data fields. Thus this function
 * should be called along with gst_audio_decoder_finish_frame.
 *
 * @buf may be NULL in which case the indicated number of frames
 * are discarded and considered to have produced no output
 * (e.g. lead-in or setup frames).
 ...
 * data in @buf.
 *
 * Note that a frame received in gst_audio_decoder_pad_output() may be
 * invalidated by a call to this function.
 *
 * Returns: a #GstFlowReturn that should be escalated to caller (of caller)
 */
GstFlowReturn
gst_audio_decoder_pad_output (GstAudioDecoder * dec, GstPad *pad,
                              GstBuffer * buf, gint frames)
{
  ...
  /* no dummy calls please */
  g_return_val_if_fail (frames != 0, GST_FLOW_ERROR);
  /* only bitstream pad is allowed now */
  g_return_val_if_fail (pad == dec->bitstream_srcpad, GST_FLOW_ERROR);

  priv = dec->priv;
  size = buf ? gst_buffer_get_size (buf) : 0;

  GST_LOG_OBJECT (dec,
                  "accepting %" G_GSIZE_FORMAT " bytes == %"
                  "%d frames", buf ? size : -1,
                  frames);
  ...
  /* discont */
  if (G_UNLIKELY (priv->bitstream_discont)) {
    GST_LOG_OBJECT (dec, "bitstream pad marking discont");
    GST_BUFFER_FLAG_SET (buf, GST_BUFFER_FLAG_DISCONT);
    priv->bitstream_discont = FALSE;
  }
...
gboolean gst_audio_decoder_default_src_query (GstPad * pad, GstObject * parent,
                                              GstQuery * query)
{
    return gst_audio_decoder_src_query(pad, parent, query);
}

gboolean gst_audio_decoder_default_src_event (GstPad * pad, GstObject * parent,
                                              GstEvent * event)
{
    return gst_audio_decoder_src_event(pad, parent, event);
}
