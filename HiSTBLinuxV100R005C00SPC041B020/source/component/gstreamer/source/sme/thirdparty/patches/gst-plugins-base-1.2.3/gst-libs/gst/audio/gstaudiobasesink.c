...
#if defined (AUDIO_BASE_SINK_DUMP)
  audiobasesink->dump_file = fopen("/mnt/dump/audiobasesink.pcm", "wb");
  if (audiobasesink->dump_file == NULL) {
    GST_ERROR("fail to open file /mnt/dump/audiobasesink.pcm");
  }
#endif
}
...
gst_audio_base_sink_setcaps (GstBaseSink * bsink, GstCaps * caps)
{
  ...
  /* FIXME: remember the current segdone and set it back after release done */
  ringbuf_segdone = g_atomic_int_get(&sink->ringbuffer->segdone);
  ...
  /* FIXME: */
  g_atomic_int_set(&sink->ringbuffer->segdone, ringbuf_segdone);
...
/* BEGIN: Added by qihuafei, 2015/08/04 */
/**
 * Moved from gstreamer 1.5.2
 * Change the procedure of wait event.
 * Code of gstreamer 1.2.3 would result in dead lock if clock is not running.
 */
static void
    ...
    switch (GST_EVENT_TYPE (event)) {
        ...
        case GST_EVENT_GAP:
            ...
            if (G_UNLIKELY (!gst_audio_ring_buffer_is_acquired (sink->ringbuffer))) {
                ...
                /* BEGIN: Modified for DTS2015081807636 by lizimian(l00251625), 2015/10/12 */
...
/* End: Added by qihuafei, 2015/08/04 */
    ...
    GstClockTime sample_offset)
{
  ...
  gint64 samples_done = (gint64)segdone * ringbuf->samples_per_seg;
...
gst_audio_base_sink_render (GstBaseSink * bsink, GstBuffer * buf)
{
  ...
  if (sink->dump_file) {
    ...
    fwrite(info.data, 1, info.size, sink->dump_file);
  }
#endif
