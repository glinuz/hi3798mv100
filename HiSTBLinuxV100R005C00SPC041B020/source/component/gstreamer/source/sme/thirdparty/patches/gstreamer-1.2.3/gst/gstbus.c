 ...
 *FIXME:If the types is an extended one,then it couldn't not exactly
 *filter the message,the one before extended will be popped too.
 *
 * MT safe.
 */
GstMessage *
...
poll_func (GstBus * bus, GstMessage * message, GstBusPollData * poll_data)
{
  ...
  if (type & poll_data->events) {
     ...
     * asked for */
    if ((!GST_MESSAGE_TYPE_IS_EXTENDED (message))
        || (poll_data->events & GST_MESSAGE_EXTENDED)) {
      /* exit the loop, we have a message */
      /* keep ref to message */
