...
gst_fd_src_start (GstBaseSrc * bsrc)
{
  ...
  if (src->seekable_fd){
	  ...
	  if(G_UNLIKELY (res < 0 || res != (guint64)src->start_offset))
		return FALSE;
  }
...
gst_fd_src_create_offset (GstBaseSrc * bsrc, guint64 offset, guint length, GstBuffer ** outbuf)
{
  ...
  GST_LOG_OBJECT (bsrc, "Read buffer of size %" G_GSSIZE_FORMAT, readbytes);
...
poll_error:
  {
    ...
    GST_DEBUG_OBJECT (bsrc, "Error during poll");
    return GST_FLOW_ERROR;
  }
stopped:
  {
    GST_DEBUG_OBJECT (bsrc, "Poll stopped");
...
eos:
  {
    GST_DEBUG_OBJECT (bsrc, "Read 0 bytes. EOS.");
...
read_error:
  {
    ...
    GST_DEBUG_OBJECT (bsrc, "Error reading from fd");
    gst_buffer_unmap (buf, &info);
    gst_buffer_unref (buf);
    return GST_FLOW_ERROR;
  }
...
static GstFlowReturn
gst_fd_src_create_base (GstBaseSrc * bsrc, guint64 offset, guint length,
    GstBuffer ** ret)
{
  GstFlowReturn fret;
  GstFdSrc *src;
  ...
  src = GST_FD_SRC (bsrc);
  if (src->seekable_fd)
    fret = gst_fd_src_create_offset(src, offset, length, ret);
  else
