...
struct _GstDecodeBin
{
  ...
  gint is_subtile;              //0 is not for subtitle, others yes.

  GList *buffering_av_multiqueue; /* multiqueues with buffering enabled which
                                  * queue audio/video es data.
                                  */
  GstPad *active_audio_pad;
  gchar * user_agent;
  gchar ** cookies;
  gchar * referer;
...
#define AUTO_PLAY_SIZE_TIME         1*GST_SECOND
...
static guint get_element_max_src_pads_num (GstElement * srcelement);
#endif

static gboolean
update_overall_buffering (GstDecodeBin *dbin, GstMessage *message);

static void update_buffering_av_multiqueue (GstDecodeBin *dbin,
    GstDecodeChain *chain,
...
#define GET_ELEMENT_NAME(ele) \
gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(GET_ELEMENT_FAC(ele)))

#define IS_ADAPTIVE_DEMUX(ele)  \
    ((GET_ELEMENT_NAME(ele)) \
          && (!strcasecmp((GET_ELEMENT_NAME(ele)), "hlsdemux")\
            || !strcasecmp((GET_ELEMENT_NAME(ele)), "dashdemux")\
            || !strcasecmp((GET_ELEMENT_NAME(ele)), "mssdemux")))
...
struct _GstDecodeChain
{
  ...
  GList *endpads;
  GList *endpadscaps;
  guint totalendpads;           /* total end pads number */
...
gst_decode_bin_class_init (GstDecodeBinClass * klass)
{
  ...
  gst_decode_bin_signals[SIGNAL_MANIFEST_PARSE_COMPLETE] =
  ...
  g_object_class_install_property(gobject_klass, PROP_IS_SUBTITLE,
          g_param_spec_int ("gst-sme-prop-key-is-subtitle", "is subtitle",
          "is this uridecodebin for subtitle", -1, G_MAXINT, 0, G_PARAM_READWRITE
          | G_PARAM_STATIC_STRINGS));
   ...
   *
   */
  g_object_class_install_property (gobject_klass, PROP_ACTIVE_AUDIO_PAD,
      g_param_spec_object ("active-audio-pad", "Active audio pad",
          "The currently active audio src pad", GST_TYPE_PAD,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  /**
   * GstURIDecodeBin::active-bitstream-pad
   *
   * Get the active bitstream pad in the case of multiple tracks
   *
   */
  g_object_class_install_property (gobject_klass, PROP_ACTIVE_BITSTREAM_PAD,
      g_param_spec_object ("active-bitstream-pad", "Active bitstream pad",
          "Get the currently active bitstream src pad", GST_TYPE_PAD,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
gst_decode_bin_init (GstDecodeBin * decode_bin)
{
  ...
  if (!decode_bin->typefind) {
    ...
    GST_INFO_OBJECT (decode_bin, "add:typefind to decbin");
  ...
  decode_bin->active_audio_pad = NULL;
  decode_bin->cookies = NULL;
  decode_bin->referer = NULL;
  decode_bin->user_agent = NULL;
  decode_bin->demux_proc_info = NULL;
  decode_bin->vdec_proc_info = NULL;
  decode_bin->adec_proc_info = NULL;
}
...
gst_decode_bin_dispose (GObject * object)
{
  ...
  if (decode_bin->buffering_av_multiqueue) {
    g_list_foreach (decode_bin->buffering_av_multiqueue,
        (GFunc) gst_object_unref, NULL);
    g_list_free (decode_bin->buffering_av_multiqueue);
    decode_bin->buffering_av_multiqueue = NULL;
  }

  if (decode_bin->active_audio_pad) {
    gst_object_unref (decode_bin->active_audio_pad);
    decode_bin->active_audio_pad = NULL;
  }

  g_free (decode_bin->user_agent);
  decode_bin->user_agent = NULL;
  g_free (decode_bin->referer);
  decode_bin->referer = NULL;
  if (decode_bin->cookies) {
    g_strfreev (decode_bin->cookies);
    decode_bin->cookies = NULL;
  }
...
gst_decode_bin_finalize (GObject * object)
{
  ...
  GST_DEBUG_OBJECT (decode_bin, "gst_decode_bin_finalize");
...
static gboolean
gst_decode_set_active_stream(GstDecodeBin *dbin, GstPad *pad)
{
  GstPad *target, *dec_sinkpad = NULL;
  GstObject *element;
  gboolean ret = FALSE, ismultiqueue = FALSE;
  gboolean done = FALSE;
  GstElementFactory *fac;
  const gchar *elem_longname;
  GstIterator *iter;
  GValue item = { 0, };

  /* find the multiqueue and corresponding pad to select the active stream */
  if (pad) {
    target = gst_ghost_pad_get_target(GST_GHOST_PAD_CAST(pad));
    GST_INFO_OBJECT (dbin, "get target pad %s", GST_PAD_NAME(target));
    if (target) {
      element = gst_pad_get_parent (target);
      GST_INFO_OBJECT (dbin, "get parent element %s",
          GST_ELEMENT_NAME(element));
      if (element) {
        fac = gst_element_get_factory(element);
        elem_longname =
            gst_element_factory_get_metadata (fac,
                GST_ELEMENT_METADATA_LONGNAME);

        GST_INFO_OBJECT (dbin, "element elem_longname %s", elem_longname);

        if (elem_longname && strstr (elem_longname, "MultiQueue")) {
          /* if there is no decoder after mulitqueue */
          GST_WARNING_OBJECT(dbin, "find multiqueue %s",
              GST_ELEMENT_NAME (element));
          ismultiqueue = TRUE;
        } else if (ismultiqueue == FALSE &&
            gst_element_factory_list_is_type (fac,
                GST_ELEMENT_FACTORY_TYPE_DECODER |
                GST_ELEMENT_FACTORY_TYPE_MEDIA_AUDIO)) {
          GST_INFO_OBJECT (dbin, "got an audio decoder");
          /* if it is the audio decoder, find the multiqueue */
          ...
          if (iter) {
            while (!done) {
              switch (gst_iterator_next (iter, &item)) {
                ...
                {
                  if (dec_sinkpad) {
                    GST_WARNING_OBJECT(dbin, "%s has multiple sink pads,"
                        " NOT support", GST_ELEMENT_NAME (element));
                    done = FALSE;
                    break;
                  }

                  dec_sinkpad = g_value_get_object (&item);
                  GST_INFO_OBJECT (dbin, "found sink pad %s",
                      GST_PAD_NAME(dec_sinkpad));
                  g_value_reset (&item);
                  break;
                }
                ...
                case GST_ITERATOR_ERROR:
                  GST_ERROR_OBJECT (dbin, "Could not iterate over"
          ...
          if (dec_sinkpad) {
            GST_INFO_OBJECT (dbin, "found pad %s, trying to find multiqueue",
                GST_PAD_NAME(dec_sinkpad));

            /* unref audio decoder and its src pad */
            gst_object_unref (element);
            gst_object_unref (target);

            target = gst_pad_get_peer (dec_sinkpad);
            element = gst_pad_get_parent (target);
            ...
            if (element) {
              fac = gst_element_get_factory(element);
              elem_longname = gst_element_factory_get_metadata (fac,
                                   GST_ELEMENT_METADATA_LONGNAME);

              GST_INFO_OBJECT (dbin, "element elem_longname %s",
                  elem_longname);
          ...
          GST_ERROR_OBJECT(dbin, "fail to find multiqueue");
        }

        if (ismultiqueue) {
          g_object_set (element, "active-audio-pad", target, NULL);
          ret = TRUE;
        } else {
          GST_ERROR_OBJECT(dbin, "fail to find multiqueue");
        }
...
static gboolean
gst_decode_get_bitstream_pad(GstDecodeBin *dbin,
    GstPad *pad, GstPad **bitstream_pad)
{
  GstPad *target, *dec_srcpad = NULL;
  GstObject *element;
  gboolean ret = FALSE, isbitstream = FALSE;
  gboolean done = FALSE;
  GstElementFactory *fac;
  const gchar *elem_longname;
  GstIterator *iter;
  ...
  /* Find the decoder contains src pad "pad",
   * and then get the bitstream src pad.
   */
  if (pad && bitstream_pad) {
    target = gst_ghost_pad_get_target(GST_GHOST_PAD_CAST(pad));
    GST_INFO_OBJECT (dbin, "get target pad %s", GST_PAD_NAME(target));
    if (target) {
      element = gst_pad_get_parent (target);
      ...
      if (element) {
        ...
        if (gst_element_factory_list_is_type (fac,
            GST_ELEMENT_FACTORY_TYPE_DECODER |
            GST_ELEMENT_FACTORY_TYPE_MEDIA_AUDIO)) {
          GST_INFO_OBJECT (dbin, "got an audio decoder");

          /* if it is the audio decoder, find the bitstream src pad */
          ...
          if (iter) {
            while (!done) {
              switch (gst_iterator_next (iter, &item)) {
                case GST_ITERATOR_OK:
                {
                  ...
                  dec_srcpad = g_value_get_object (&item);
                  GST_INFO_OBJECT (dbin, "found sink pad %s",
                      GST_PAD_NAME(dec_srcpad));
                  g_value_reset (&item);

                  srcpad_caps = gst_pad_get_current_caps(dec_srcpad);
                  str_caps = gst_caps_to_string(srcpad_caps);
                  if (strstr (str_caps, "bitstream")) {
                    isbitstream = TRUE;
                    done = TRUE;
                  }
            ...
            if (isbitstream) {
              *bitstream_pad = gst_pad_get_peer(dec_srcpad);
            } else {
              GST_WARNING_OBJECT (dbin, "can't find bitstream src pad");
              *bitstream_pad = NULL;
            }

          } /* iter end */
        } /* if audio decoder end */
        gst_object_unref (element);
      } else {
        GST_ERROR_OBJECT(dbin, "fail to find element");
      } /* if element end */
      gst_object_unref (target);
    } /* if target end */
  } /* if pad end */
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_CONNECTION_SPEED:{
      GstBin* dgstbin = (GstBin*)dbin;
      GST_OBJECT_LOCK (dbin);
      dbin->connection_speed = g_value_get_uint64 (value) * 1000;
      GST_INFO_OBJECT(dbin,"playbin->connection_speed:%lld",dbin->connection_speed);
      GST_OBJECT_UNLOCK (dbin);
      if(dgstbin->children){
        GList* walk;
        for (walk = dgstbin->children; walk; walk = g_slist_next (walk)) {
          GObject *element = G_OBJECT (walk->data);
          if(element && GST_IS_ELEMENT(element)){
            GstElementFactory *fac = gst_element_get_factory(element);
            if(fac && gst_element_factory_list_is_type (fac, GST_ELEMENT_FACTORY_TYPE_DEMUXER)){
              GObjectClass *theclass = g_type_class_peek(gst_element_factory_get_element_type(fac));
              if(theclass && g_object_class_find_property (theclass, "connection-speed")){
                GST_WARNING("set connection-speed,name=%s",
                  gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)));
                g_object_set(element, "connection-speed", g_value_get_uint64 (value), NULL);
              }
            }
          }
        }
    ...
    case PROP_EXIT_BLOCK:{
      ...
      GST_INFO("PROP_EXIT_BLOCK val = %d, dgstbin->children=%p", exit_block, dgstbin->children);
      if(dgstbin->children){
        GList* walk;
        for (walk = dgstbin->children; walk; walk = g_slist_next (walk)) {
          GObject *element = G_OBJECT (walk->data);
          if(element && GST_IS_ELEMENT(element)){
            ...
            if(fac && gst_element_factory_list_is_type (fac, GST_ELEMENT_FACTORY_TYPE_DEMUXER)){
              GObjectClass *theclass = g_type_class_peek(gst_element_factory_get_element_type(fac));
              if(theclass && g_object_class_find_property (theclass, "gst-sme-prop-key-exit-block")){
                GST_WARNING("set gst-sme-prop-key-exit-block,name=%s",
                  gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)));
                g_object_set(element, "gst-sme-prop-key-exit-block", (gint)exit_block, NULL);
              }else{
                GST_WARNING("not set gst-sme-prop-key-exit-block,name=%s,theclass=%p",
                  gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)), theclass);
              }
            }else{
              GST_WARNING("not a demuxer,name:%s",
                fac?gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)):"not a fac");
            }
          }else{
            GST_WARNING("not an element");
          }
        }
      }
    ...
    case PROP_IS_SUBTITLE:{
      gint is_subtile = g_value_get_int(value);
      GST_INFO("PROP_IS_SUBTITLE val = %d", is_subtile);
      if(dbin){
        dbin->is_subtile = is_subtile;
      }
      break;
    }
    /* END:   Added for DTS2015092309761 by liurongliang(l00180035), 2015/9/28 */
    ...
    case PROP_ACTIVE_AUDIO_PAD:{
      ...
      GST_INFO_OBJECT (dbin, "set active pad %s", GST_PAD_NAME(pad));
      if (pad) {
        if (dbin->active_audio_pad) {
          gst_object_unref (dbin->active_audio_pad);
          dbin->active_audio_pad = NULL;
        }

        dbin->active_audio_pad = gst_object_ref (pad);

        gst_decode_set_active_stream(dbin, pad);
      }
      break;
    }
    case PROP_USER_AGENT:
      if (dbin->user_agent)
          g_free (dbin->user_agent);
      dbin->user_agent = g_value_dup_string (value);
      break;
    case PROP_REFERER:
      if (dbin->referer)
          g_free (dbin->referer);
      dbin->referer = g_value_dup_string (value);
      break;
    case PROP_COOKIES:
      g_strfreev (dbin->cookies);
      dbin->cookies = g_strdupv (g_value_get_boxed (value));
      break;
    case PROP_SME_DEMUX_INFO:{
      GstBin* dgstbin = (GstBin*)dbin;
      GST_OBJECT_LOCK (dbin);
      dbin->demux_proc_info = g_value_get_pointer (value);
      GST_DEBUG("PROP_SME_DEMUX_INFO info pointer=%p", dbin->demux_proc_info);
      GST_OBJECT_UNLOCK (dbin);
      if(dgstbin->children){
        GList* walk;
        for (walk = dgstbin->children; walk; walk = g_slist_next (walk)) {
          ...
          if(element && GST_IS_ELEMENT(element)){
            ...
            if(fac && gst_element_factory_list_is_type (fac, GST_ELEMENT_FACTORY_TYPE_DEMUXER)){
              GObjectClass *theclass = g_type_class_peek(gst_element_factory_get_element_type(fac));
              if(theclass && g_object_class_find_property (theclass, "sme-demux-info")){
                GST_WARNING("set sme-demux-info,name=%s",
                  gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)));
                g_object_set (element, "sme-demux-info", dbin->demux_proc_info, NULL);
                GST_DEBUG("Set property to sme demux");
              }
            }
          }
        }
    ...
    case PROP_VDEC_INFO:{
      GstBin* dgstbin = (GstBin*)dbin;
      GST_OBJECT_LOCK (dbin);
      dbin->vdec_proc_info = g_value_get_pointer (value);
      GST_DEBUG("PROP_VDEC_INFO info, pointer=%p", dbin->vdec_proc_info);
      GST_OBJECT_UNLOCK (dbin);
      if(dgstbin->children){
        GList* walk;
        for (walk = dgstbin->children; walk; walk = g_slist_next (walk)) {
          ...
          if(element && GST_IS_ELEMENT(element)){
            GstElementFactory *fac = gst_element_get_factory(element);
            if(fac && gst_element_factory_list_is_type (fac, GST_ELEMENT_FACTORY_TYPE_DECODER |
            GST_ELEMENT_FACTORY_TYPE_MEDIA_VIDEO)){
              GObjectClass *theclass = g_type_class_peek(gst_element_factory_get_element_type(fac));
              if(theclass && g_object_class_find_property (theclass, "omx-video-decoder-info")){
                GST_WARNING("set omx-video-decoder-info,name=%s",
                  gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)));
                g_object_set (element, "omx-video-decoder-info", dbin->vdec_proc_info, NULL);
                GST_INFO("Set property to omx-vdec");
              }
            }
          }
        }
    ...
    case PROP_ADEC_INFO:{
      GstBin* dgstbin = (GstBin*)dbin;
      GST_OBJECT_LOCK (dbin);
      dbin->adec_proc_info = g_value_get_pointer (value);
      GST_INFO("PROP_ADEC_INFO info, pointer=%p", dbin->adec_proc_info);
      GST_OBJECT_UNLOCK (dbin);
      if(dgstbin->children){
        GList* walk;
        for (walk = dgstbin->children; walk; walk = g_slist_next (walk)) {
          ...
          if(element && GST_IS_ELEMENT(element)){
            ...
            GST_ELEMENT_FACTORY_TYPE_MEDIA_AUDIO)){
              GObjectClass *theclass = g_type_class_peek(gst_element_factory_get_element_type(fac));
              if(theclass && g_object_class_find_property (theclass, "gst-audio-decoder-info")){
                GST_WARNING("set audio-decoder-info,name=%s",
                  gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)));
                g_object_set (element, "gst-audio-decoder-info", dbin->adec_proc_info, NULL);
                GST_INFO("Set pointer to audio decoder");
              }
            }
          }
        }
    ...
    case PROP_SURFACE:{
      ...
      dbin->vdisp_surface = surface_handle;
      break;
    }
#endif
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_ACTIVE_BITSTREAM_PAD:{
      GstPad *pad, *target, *bitstream_pad;

      pad = dbin->active_audio_pad;

      GST_INFO_OBJECT (dbin, "set active pad %s", GST_PAD_NAME(pad));
      if (pad) {
        gst_decode_get_bitstream_pad(dbin, pad, &bitstream_pad);
        g_value_set_object (value, bitstream_pad);
        if (bitstream_pad){
          gst_object_unref (bitstream_pad);
        }
      }
    ...
    case PROP_USER_AGENT:
      g_value_set_string (value, dbin->user_agent);
      break;
    case PROP_REFERER:
      g_value_set_string (value, dbin->referer);
      break;
    case PROP_COOKIES:
      g_value_set_boxed (value, dbin->cookies);
      break;
#ifndef __LINUX__
case PROP_DECIN_BUF_DURATION:{
      ...
      if(dgstbin->children){
        ...
        for (walk = dgstbin->children; walk; walk = g_slist_next (walk)) {
          ...
          if(element && GST_IS_ELEMENT(element)){
            GstElementFactory *fac = gst_element_get_factory(element);
            if(fac && gst_element_factory_list_is_type (fac,
              GST_ELEMENT_FACTORY_TYPE_DECODER | GST_ELEMENT_FACTORY_TYPE_MEDIA_VIDEO)){
              GObjectClass *theclass = g_type_class_peek(gst_element_factory_get_element_type(fac));
              if(theclass && g_object_class_find_property (theclass, "gst-sme-prop-key-hivpipe-decin-duration")){
                GST_DEBUG("get gst-sme-prop-key-hivpipe-decin-duration,name=%s",
                  gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)));
                g_object_get(element, "gst-sme-prop-key-hivpipe-decin-duration",
                            &decin_duration, NULL);
                break;
              }else{
                GST_DEBUG("not get gst-sme-prop-key-hivpipe-decin-duration,name=%s,theclass=%p",
                  gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)), theclass);
              }
            }else{
              GST_DEBUG("not a vdec,name:%s",
                fac?gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)):"not a fac");
            }
          }else{
            GST_DEBUG("not an element");
          }
        }
      }
        ...
        for (walk = dgstbin->children; walk; walk = g_slist_next (walk)) {
          ...
          if(element && GST_IS_ELEMENT(element)){
            ...
            if(fac && gst_element_factory_list_is_type (fac,
              GST_ELEMENT_FACTORY_TYPE_DECODER | GST_ELEMENT_FACTORY_TYPE_MEDIA_VIDEO)){
              GObjectClass *theclass = g_type_class_peek(gst_element_factory_get_element_type(fac));
              if(theclass && g_object_class_find_property (theclass, "gst-sme-prop-key-hivpipe-dec-interval")){
                GST_DEBUG("get gst-sme-prop-key-hivpipe-dec-interval,name=%s",
                  gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)));
                g_object_get(element, "gst-sme-prop-key-hivpipe-dec-interval",
                            &u_cur_interval, NULL);
                break;
              }else{
                GST_DEBUG("not get gst-sme-prop-key-hivpipe-dec-interval,name=%s,theclass=%p",
                  gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac)), theclass);
              }
            }else{
              GST_DEBUG("not a vdec,name:%s",
...
static void manifest_parsing_complete_cb (GstElement * element,
    gpointer input,
    gpointer result,
    GstDecodeChain * chain);
    ...
    GstCaps * caps, GstDecodeChain * chain)
{
  ...
  if (is_parser_converter) {
    ...
    GST_INFO_OBJECT (dbin, "add:capsfilter to decbin");
...
#else
    chain->endpadscaps = g_list_append (chain->endpadscaps, gst_caps_ref(caps));
#endif
    ...
    GstDecodeChain * chain)
{
  ...
  while (factories->n_values > 0) {
    ...
    GST_INFO_OBJECT (dbin, "add:%s to decbin",
        gst_plugin_feature_get_name (GST_PLUGIN_FEATURE (factory)));
...
//#ifndef __LINUX__
    ...
    if (gst_element_factory_list_is_type (factory,
            GST_ELEMENT_FACTORY_TYPE_DECODER |
            GST_ELEMENT_FACTORY_TYPE_MEDIA_VIDEO)) {
        GST_INFO("video dec element");
        GstElement* vsink = NULL;
        GstObject* playbin = NULL;
        g_object_set(element,
                     "gst-sme-prop-key-vdisp-surface",
                     (gulong)dbin->vdisp_surface,
                     NULL);
        if(GST_OBJECT_PARENT(dbin)){
          playbin = GST_OBJECT_PARENT(GST_OBJECT_PARENT(dbin));
          if(playbin){
            g_object_get(playbin, "video-sink", &vsink, NULL);
            if(vsink){
              gpointer mpihdl = NULL;
              g_object_get(vsink, "gst-sme-prop-key-hivpipe-hdl", &mpihdl, NULL);
              GST_INFO("set mpihld:%p", mpihdl);
              g_object_set(element, "gst-sme-prop-key-hivpipe-hdl", mpihdl, NULL);
              gst_object_unref(vsink);
            }else{
              GST_WARNING("vsink is null");
            }
          }
        }
    }
    ...
    update_buffering_av_multiqueue (dbin, chain, factory);
...
#if defined (SME_AUDIO_BITSTREAM)
    chain->totalendpads =  get_element_max_src_pads_num (element);
    GST_WARNING_OBJECT(dbin, "add an element with %d potential endpads",
                       chain->totalendpads);
#endif
    CHAIN_MUTEX_UNLOCK (chain);
    ...
    if (chain->demuxer == TRUE) {
      ...
      if (IS_ADAPTIVE_DEMUX(element)) {
        g_object_set (element, "timeout",(guint)16, NULL);
        g_object_set (element, "user-agent",dbin->user_agent, NULL);
        if (dbin->cookies)
          g_object_set (element, "cookies",dbin->cookies, NULL);
        g_object_set (element, "referer",dbin->referer, NULL);
        g_signal_connect (element, "manifest-parse-complete",
            G_CALLBACK (manifest_parsing_complete_cb),
            chain);
      }
    }
    ...
    GstPad * pad, GstCaps * caps, GstDecodeChain * chain)
{
  ...
  chain->endcaps = gst_caps_ref (caps);
#else
  chain->endpads = g_list_append (chain->endpads, gst_object_ref(dpad));
  chain->endpadscaps = g_list_append (chain->endpadscaps, gst_caps_ref(caps));
#endif
    ...
    GstCaps * caps, GstDecodeBin * decode_bin)
{
  ...
  /* BEGIN: Added for DTS2015092309761 by liurongliang(l00180035), 2015/9/28 */
  GST_INFO("type_found, decode_bin->is_subtile=%d", decode_bin->is_subtile);
  if(decode_bin->is_subtile){
    gchar* str_caps = NULL;

    str_caps = gst_caps_to_string(caps);
    GST_INFO("type_found, str_caps=%s", str_caps);
    if(strncmp(str_caps, "text/", 5)
      && strncmp(str_caps, "application/x-subtitle", strlen("application/x-subtitle"))
      && strncmp(str_caps, "application/x-subtitle-tmplayer", strlen("application/x-subtitle-tmplayer"))
      && strncmp(str_caps, "application/x-subtitle-sami", strlen("application/x-subtitle-sami"))
      && strncmp(str_caps, "application/x-subtitle-mpl2", strlen("application/x-subtitle-mpl2"))
      && strncmp(str_caps, "application/x-subtitle-dks", strlen("application/x-subtitle-dks"))
      && strncmp(str_caps, "application/x-subtitle-qttext", strlen("application/x-subtitle-qttext"))){
      GST_ELEMENT_ERROR (decode_bin, STREAM, SUBTILE,
        ("subtile but typed for:%s", str_caps), (NULL));
      g_free(str_caps);
...
/*
struct _GstBitrateInfo
{
  gint* video_bitrate_list;
  gint video_bitrates_num;
  gint* audio_bitrate_list;
  gint audio_bitrate_num;
};
typedef struct _GstBitrateInfo GstBitrateInfo;
*/
static void manifest_parsing_complete_cb (GstElement * element,
  gpointer input,
  gpointer result,
...
{
  GstDecodeBin *dbin = chain->dbin;
  GST_WARNING_OBJECT(dbin, "in");
/*
   GstBitrateInfo* pbitrate_info = (GstBitrateInfo*)input;
   GST_WARNING_OBJECT(dbin, "in,v num:%d,a num:%d",
   pbitrate_info->video_bitrates_num,pbitrate_info->audio_bitrate_num);


   for (int i = 0; i < pbitrate_info->video_bitrates_num;i++)
   {
       GST_WARNING_OBJECT(dbin,"Video[%d] bitrate:[%d]",i,pbitrate_info->video_bitrate_list[i]);
   }
   for (int i = 0; i < pbitrate_info->audio_bitrate_num;i++)
   {
       GST_WARNING_OBJECT(dbin,"Audio[%d] bitrate:[%d]",i,pbitrate_info->audio_bitrate_list[i]);
   }
*/
  g_signal_emit(dbin,gst_decode_bin_signals[SIGNAL_MANIFEST_PARSE_COMPLETE],0,input,result);
  GST_WARNING_OBJECT(dbin, "out");
}
...
#if defined (SME_AUDIO_BITSTREAM)
static guint
get_element_max_src_pads_num (GstElement * srcelement)
{
  GstElementFactory *srcfactory;
  ...
  templates = gst_element_factory_get_static_pad_templates (srcfactory);
...
#else
  for (l = chain->endpads; l;) {
    GstDecodePad *endpad = (GstDecodePad *)l->data;
    GList *next;
    ...
    decode_pad_set_target (endpad, NULL);
    endpad->exposed = FALSE;
    if (!hide) {
      gst_object_unref (endpad);
      next = l->next;
      chain->endpads = g_list_delete_link (chain->endpads, l);
      l = next;
    }
  }
#endif
...
#else
  for (l = chain->endpadscaps; l;) {
    GstCaps *endcaps = (GstCaps *)l->data;
    GList *next;

    if (endcaps) {
      gst_caps_unref (endcaps);
      next = l->next;
      chain->endpadscaps = g_list_delete_link(chain->endpadscaps, l);
      l = next;
    }
  }
#endif
    ...
    gboolean preroll, gboolean seekable)
{
  ...
  GST_INFO_OBJECT (multiqueue, "use buffering %d", use_buffering);
...
#else
    if(dbin->use_buffering){
        // for hls/dash the mq after hls/dashdemux
        max_bytes = AUTO_PLAY_SIZE_BYTES;
        max_buffers = 0;
        //AUTO_PLAY_SIZE_BUFFERS;
        max_time = AUTO_PLAY_SIZE_TIME;
        GST_INFO("for hls/dashdemux multi-queue");
    }
    else{
        //for local play
        max_bytes = AUTO_PLAY_SIZE_BYTES * 5;
        max_buffers = 0;
        //AUTO_PLAY_SIZE_BUFFERS;
        max_time = AUTO_PLAY_SIZE_TIME * 5;
        GST_INFO("for localplay multi-queue");
    }
#endif
  }

  //max_time = 2 * GST_SECOND;
  GST_INFO_OBJECT (multiqueue, "setting limits: buffering : %d,  %u bytes, %u buffers, "
      "%" G_GUINT64_FORMAT, dbin->use_buffering, max_bytes, max_buffers, max_time);
...
gst_decode_group_new (GstDecodeBin * dbin, GstDecodeChain * parent)
{
  ...
  GST_INFO_OBJECT (dbin, "Creating new group %p with parent chain %p,use_buffering=%d", group,
      parent,dbin->use_buffering);
  ...
  GST_INFO_OBJECT (dbin, "add:multiqueue to decbin");
...
gst_decode_group_control_demuxer_pad (GstDecodeGroup * group, GstPad * pad)
{
  ...
  {
      GstElement* ele = GST_OBJECT_PARENT(pad);
      if(ele && GST_IS_ELEMENT(ele)){
        GstElementFactory *fac = gst_element_get_factory(ele);
        if(fac){
            //GType ele_type = gst_element_factory_get_element_type(fac);
            const gchar* ele_name = gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(fac));
            if(ele_name
              && (!strcasecmp(ele_name, "hlsdemux")
                || !strcasecmp(ele_name, "dashdemux")
                || !strcasecmp(ele_name, "mssdemux"))){
              GST_INFO("multiqueue,src peer is:%s,disable buffering,use later", ele_name);
              g_object_set (group->multiqueue, "use-buffering", FALSE, NULL);
              g_object_set (group->multiqueue,
                    "max-size-bytes", AUTO_PLAY_SIZE_BYTES,
                    "max-size-time", AUTO_PLAY_SIZE_TIME,
                    "mq-is-after-demux", 0, NULL);
              g_object_set (ele, "timeout",(guint)16, NULL);
            }else{
              GST_WARNING("mq peer parent name is %s", ele_name ? ele_name:"null element");
            }
        }else{
          GST_WARNING("mq peer parent fac is %p,not element", fac);
        }
      }else{
        GST_WARNING("mq peer parent is %p,not element", ele);
      }
  }
#endif
  if ((gst_pad_link (pad, sinkpad) != GST_PAD_LINK_OK)) {
...
gst_decode_group_is_complete (GstDecodeGroup * group)
{
  ...
  /*
     we donot wait for all sub chain complete, just if one is complete than we return complete.
  */
...
gst_decode_chain_is_complete (GstDecodeChain * chain)
{
   ...
   */
  if (chain->endpads) {
    guint complete_pad_num = 0;

    for (list = chain->endpads; list; list = list->next) {
      GstDecodePad *endpad = (GstDecodePad *)list->data;

      if (endpad && (endpad->blocked || endpad->exposed)) {
          complete_pad_num++;
      }
    }
    if (complete_pad_num == chain->totalendpads || chain->no_more_pads) {
      complete = TRUE;
      goto out;
    }
  }
    ...
    gboolean * last_group, gboolean * drained, gboolean * switched)
{
  ...
  if (chain->endpads) {
    ...
    for (list = chain->endpads; list; list = list->next) {
      GstDecodePad *endpad = (GstDecodePad *)list->data;
      drained &= endpad->drained;
    }

    chain->drained = drained;
    goto beach;
  }
#endif
...
#else
  if (G_UNLIKELY ((chain->endpads || chain->deadend)
          && (chain->endpadscaps == NULL))) {
    GST_WARNING ("End chain without valid caps !");
    return NULL;
  }
#endif
...
#else
  if ((chain->elements || !chain->active_group) &&
      (chain->endpads || chain->deadend)) {
    s = gst_structure_new_id_empty (topology_structure_name);

    /**FIXME:
     * Need to figure out the order of each field in structure.
     */
    for (l = chain->endpadscaps; l; l = l->next) {
      caps = (GstCaps *)l->data;
      gst_structure_id_set (u, topology_caps, GST_TYPE_CAPS, caps,
          NULL);
    }

    for (l = chain->endpads; l; l = l->next) {
      GstDecodePad *endpad = (GstDecodePad *)l->data;

      gst_structure_id_set (u, topology_pad, GST_TYPE_PAD, endpad, NULL);
      gst_structure_id_set (u, topology_element_srcpad, GST_TYPE_PAD,
          endpad, NULL);
    }
    gst_structure_id_set (s, topology_next, GST_TYPE_STRUCTURE, u, NULL);
...
gst_decode_bin_expose (GstDecodeBin * dbin)
{
  ...
  for (tmp = endpads; tmp; tmp = tmp->next) {
    ...
    /* 0. skip the already exposed pad */
    if (dpad->exposed) {
      continue;
    }
    ...
    gboolean * missing_plugin)
{
  ...
  GList *l;
  GstDecodeBin *dbin;
  gboolean res_expose_chain = FALSE;
  ...
  if (chain->endpads) {
    for (l = chain->endpads; l; l = l->next) {
      ...
      if (!endpad->blocked && !endpad->exposed)
        continue;
      *endpads = g_list_prepend (*endpads, gst_object_ref (endpad));
    }
    if (*endpads)
      return TRUE;
    else
  ...
  /* BEGIN: Modified by liurongliang(l00180035), 2015/8/7 */
  /*
     we donot wait for all sub chain expose, expose what can expose.
  */
  /* END:   Modified by liurongliang(l00180035), 2015/8/7 */
  if(!group->children){
    res_expose_chain = TRUE;
  }
    ...
    gboolean * missing_plugin)
{
  ...
  /* only expose the chain when all the pads are exposed */
  if (chain->endpads) {
    guint exposed_pads_num = 0;
    GstDecodePad *endpad = NULL;

    for (l = chain->endpads; l; l = l->next) {
      endpad = (GstDecodePad *)l->data;
      if (!endpad->blocked && !endpad->exposed)
        continue;
      exposed_pads_num++;
    }

    if (exposed_pads_num == chain->totalendpads || chain->no_more_pads) {
      for (l = chain->endpads; l; l = l->next) {
        endpad = (GstDecodePad *)l->data;
        *endpads = g_list_prepend (*endpads, gst_object_ref (endpad));
      }
...
source_pad_blocked_cb (GstPad * pad, GstPadProbeInfo * info, gpointer user_data)
{
  ...
  if (GST_PAD_PROBE_INFO_TYPE (info) & GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM) {
    ...
    GST_INFO_OBJECT (pad, "Seeing event '%s'", GST_EVENT_TYPE_NAME (event));
...
#if defined (SME_AUDIO_BITSTREAM)
    //if EOS comes, we should set no_more_pads to TRUE & let it bypass to downstream.
    else if(dpad && dpad->chain && !dpad->chain->no_more_pads
      && GST_EVENT_EOS == GST_EVENT_TYPE (event))
    {
      dpad->chain->no_more_pads = TRUE;
      ret = GST_PAD_PROBE_OK;
      GST_INFO_OBJECT (pad, "eos");
    }
#endif
...
gst_decode_bin_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      GST_DEBUG_OBJECT (dbin, "gst_decode_bin_change_state GST_STATE_CHANGE_NULL_TO_READY");
      if (dbin->typefind == NULL)
        goto missing_typefind;
      break;
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      /* Make sure we've cleared all existing chains */
      GST_DEBUG_OBJECT (dbin, "gst_decode_bin_change_state GST_STATE_CHANGE_READY_TO_PAUSED");
      EXPOSE_LOCK (dbin);
    ...
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      GST_DEBUG_OBJECT (dbin, "gst_decode_bin_change_state GST_STATE_CHANGE_PAUSED_TO_READY");
shutdown:
      if (dbin->have_type_id)
        g_signal_handler_disconnect (dbin->typefind, dbin->have_type_id);
      dbin->have_type_id = 0;
      DYN_LOCK (dbin);
      GST_DEBUG_OBJECT (dbin, "setting shutdown flag");
      dbin->shutdown = TRUE;
      unblock_pads (dbin);
      DYN_UNLOCK (dbin);
      break;
    case GST_STATE_CHANGE_READY_TO_NULL:
      ...
      if (!dbin->shutdown) {
        shutdown_flag = TRUE;
        DYN_UNLOCK (dbin);
        GST_DEBUG_OBJECT (dbin, "gst_decode_bin_change_state GST_STATE_CHANGE_READY_TO_NULL, run GST_STATE_CHANGE_PAUSED_TO_READY first");
        goto shutdown;
      }
      DYN_UNLOCK (dbin);
    default:
  ...
  if (shutdown_flag) {
        GST_ELEMENT_CLASS (parent_class)->change_state (element, GST_STATE_CHANGE_PAUSED_TO_READY);
  }
...
shutdown2:
      ...
      EXPOSE_UNLOCK (dbin);
      GST_OBJECT_LOCK (dbin);
      /* free multiqueue list */
      if (dbin->buffering_av_multiqueue) {
        g_list_foreach (dbin->buffering_av_multiqueue,
            (GFunc) gst_object_unref, NULL);
        g_list_free (dbin->buffering_av_multiqueue);
        dbin->buffering_av_multiqueue = NULL;
      }
      GST_OBJECT_UNLOCK (dbin);
      break;
    case GST_STATE_CHANGE_READY_TO_NULL:
      GST_DEBUG_OBJECT (dbin, "gst_decode_bin_change_state GST_STATE_CHANGE_READY_TO_NULL");
      dbin->is_subtile = 0;
      if (shutdown_flag) {
        GST_DEBUG_OBJECT (dbin, "gst_decode_bin_change_state GST_STATE_CHANGE_READY_TO_NULL, run GST_STATE_CHANGE_PAUSED_TO_READY first");
        goto shutdown2;
...
gst_decode_bin_handle_message (GstBin * bin, GstMessage * msg)
{
  ...
  if (GST_MESSAGE_TYPE (msg) == GST_MESSAGE_BUFFERING) {
      update_overall_buffering (bin, msg);
  }
...
/** BEGIN: Added for DTS2015092509721 by baihaili(b00311320), 2015/10/12 */

/* In the cases where several multiqueues exist in the pipeline, the buffering
 * percent needs to be the mininal one among multiqueues. Otherwise, pipeline
 * may enter to pause too late or playing too early if buffering messages
 * are sending independently.
 */
static gboolean
update_overall_buffering (GstDecodeBin *dbin, GstMessage *message)
{
  guint min_percent, percent;
  GstElement *msg_src, *oq;
  GList *tmp;

  /* if there is only one multiqueue within decode bin, do nothing */
  if (g_list_length (dbin->buffering_av_multiqueue) <= 1) {
    GST_LOG_OBJECT (dbin, "forward the original buffering percent");
    return TRUE;
  }

  /* get the percent from message and treat it as the minimal one */
  gst_message_parse_buffering (message, &min_percent);
  msg_src = (GstElement *)GST_MESSAGE_SRC (message);
  GST_DEBUG_OBJECT (dbin, "message src %s, percent %d",
      GST_ELEMENT_NAME (msg_src), min_percent);

  for (tmp = dbin->buffering_av_multiqueue; tmp; tmp = g_list_next (tmp)) {
    oq = (GstElement *) tmp->data;
    if (msg_src == oq) {
      continue;
    }
    g_object_get (oq, "current-percent", &percent, NULL);
    GST_DEBUG_OBJECT (dbin, "multiqueue %s percent %d",
        GST_ELEMENT_NAME (oq), percent);
  ...
  GST_DEBUG_OBJECT (dbin, "overall percent %d", min_percent);
  gst_message_set_buffering (message, min_percent);
...
static void
update_buffering_av_multiqueue (GstDecodeBin *dbin,
    GstDecodeChain *chain,
    GstElementFactory *factory)
{
  gboolean isvideodec, isaudiodec;
  gboolean isbuffering;
  ...
  isaudiodec = gst_element_factory_list_is_type (factory,
      GST_ELEMENT_FACTORY_TYPE_DECODER |
      GST_ELEMENT_FACTORY_TYPE_MEDIA_AUDIO);
  ...
  if (isvideodec || isaudiodec) {
    multiqueue = chain->parent->multiqueue;
    g_object_get (multiqueue, "use-buffering", &isbuffering, NULL);

    if (isbuffering) {
      /* check whether the multiqueue is already in the list */
      for (tmp = dbin->buffering_av_multiqueue; tmp; tmp = g_list_next (tmp)) {
        GstElement *oq = (GstElement *) tmp->data;
        if (multiqueue == oq) {
          goto done;
        }
      }

      /* add multiqueue to the list */
      dbin->buffering_av_multiqueue = g_list_prepend (
          dbin->buffering_av_multiqueue,
          g_object_ref (multiqueue));

      GST_INFO_OBJECT (dbin, "add multiqueue %s to buffering_av_multiqueue",
          GST_ELEMENT_NAME (multiqueue));
    }
  }
...
/** END: Added for DTS2015092509721 by baihaili(b00311320), 2015/10/12 */
