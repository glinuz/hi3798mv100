	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		...
		for (path = 0; path < RF_PATH_MAX; path++) {
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				for (tx_num = RF_1TX; tx_num < hal_data->nss_num; tx_num++) {

					if ((tx_num == RF_1TX && (!IS_1T_RATE_SECTION(rs) || hal_data->nss_num < 1))
						|| (tx_num == RF_2TX && (!IS_2T_RATE_SECTION(rs) || hal_data->nss_num < 2))
						|| (tx_num == RF_3TX && (!IS_3T_RATE_SECTION(rs) || hal_data->nss_num < 3))
						|| (tx_num == RF_4TX && (!IS_4T_RATE_SECTION(rs) || hal_data->nss_num < 4)))
						continue;
...
void dump_tx_power_by_rate_offset(void *sel, _adapter *adapter)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				for (tx_num = RF_1TX; tx_num < hal_data->nss_num; tx_num++) {
					...
					if (IS_HARDWARE_TYPE_JAGUAR_AND_JAGUAR2(adapter))
						...
						max_rate_num = 8;
					rate_num = rates_by_sections[rs].num;
		...
		for (bw = 0; bw < MAX_5G_BANDWIDTH_NUM; bw++) {
			...
			if (band == BAND_ON_2_4G) {
				...
				if (bw == CHANNEL_WIDTH_20)
					ch_num = CENTER_CH_5G_20M_NUM;
				else if (bw == CHANNEL_WIDTH_40)
					ch_num = CENTER_CH_5G_40M_NUM;
				else
					ch_num = CENTER_CH_5G_80M_NUM;
			}
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				if ((IS_2T_RATE_SECTION(rs) && hal_data->nss_num <= 1)
					|| (IS_3T_RATE_SECTION(rs) && hal_data->nss_num <= 2)
					|| (IS_4T_RATE_SECTION(rs) && hal_data->nss_num <= 3))
						continue;
				...
				for (n = 0; n < ch_num; n++) {
					...
					if (band == BAND_ON_2_4G) {
						...
						if (bw == CHANNEL_WIDTH_20)
							ch = center_ch_5g_20m[n];
						else if (bw == CHANNEL_WIDTH_40)
							ch = center_ch_5g_40m[n];
						else
							ch = center_ch_5g_80m[n];
					}
					...
					if (IS_1T_RATE_SECTION(rs))
						base = PHY_GetTxPowerByRateBase(adapter, band, RF_PATH_A, RF_1TX, rs);
					else if (IS_2T_RATE_SECTION(rs))
						base = PHY_GetTxPowerByRateBase(adapter, band, RF_PATH_A, RF_2TX, rs);
					else if (IS_3T_RATE_SECTION(rs))
						base = PHY_GetTxPowerByRateBase(adapter, band, RF_PATH_A, RF_3TX, rs);
					else if (IS_4T_RATE_SECTION(rs))
						base = PHY_GetTxPowerByRateBase(adapter, band, RF_PATH_A, RF_4TX, rs);
					else {
						base = 0;
						rtw_warn_on(1);
					}
					...
					for (path = 0; path < RF_PATH_MAX; path++) {
						...
						if (IS_1T_RATE_SECTION(rs))
							base = PHY_GetTxPowerByRateBase(adapter, band, path, RF_1TX, rs);
						else if (IS_2T_RATE_SECTION(rs))
							base = PHY_GetTxPowerByRateBase(adapter, band, path, RF_2TX, rs);
						else if (IS_3T_RATE_SECTION(rs))
							base = PHY_GetTxPowerByRateBase(adapter, band, path, RF_3TX, rs);
						else if (IS_4T_RATE_SECTION(rs))
							base = PHY_GetTxPowerByRateBase(adapter, band, path, RF_4TX, rs);
						else {
							base = 0;
							rtw_warn_on(1);
						}
