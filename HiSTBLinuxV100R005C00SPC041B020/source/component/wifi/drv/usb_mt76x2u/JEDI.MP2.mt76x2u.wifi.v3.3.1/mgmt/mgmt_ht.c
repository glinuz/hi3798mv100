...
VOID RTMPSetHT(IN RTMP_ADAPTER *pAd, IN OID_SET_HT_PHYMODE * pHTPhyMode)
{
	...
	if (pHTPhyMode->BW == BW_40) {
		...
		/* Sta can turn on BBP 40MHz after connection with 40MHz AP. */
		/* Sta only broadcast 40MHz capability before connection. */
		    ...
		    OUT HT_CAPABILITY_IE *pHtCapability, OUT ADD_HT_INFO_IE *pAddHtInfo)
{
	...
	/* rt2860 only support MCS max=32, no need to copy all 16 uchar. */
	RTMPMoveMemory(pAddHtInfo->MCSSet, /*pRtHt->MCSSet */ pMcsSet, 4);
