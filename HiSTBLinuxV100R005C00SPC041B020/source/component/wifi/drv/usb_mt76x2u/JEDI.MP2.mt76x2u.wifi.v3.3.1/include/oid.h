...
#define BAND_WIDTH_10		4	/* 802.11j has 10MHz. This definition is for internal usage.
...
#define	RT_HOSTAPD_OID_SET_WPS_PROBE_RESP_IE (OID_GET_SET_TOGGLE |    \
...
typedef struct GNU_PACKED _NDIS_WLAN_BSSID_EX {
	...
	UCHAR WpsAP;		/* 0x00: not support WPS, 0x01: support normal WPS,
				* 0x02: support Ralink auto WPS, 0x04: support Samsung WAC
				*/
	CHAR MinSNR;
	NDIS_802_11_SSID Ssid;	/* SSID */
...
#ifdef DBG
/* When use private ioctl oid get/set the configuration,
* we can use following flags to provide specific rules when handle the cmd
 */
#define RTPRIV_IOCTL_FLAG_UI	      0x0001	/* Notidy this private cmd send by UI. */
#define RTPRIV_IOCTL_FLAG_NODUMPMSG   0x0002    /* Notify driver cannot dump msg to stdio/stdout
						* when run this private ioctl cmd
						*/
#define RTPRIV_IOCTL_FLAG_NOSPACE     0x0004	/* Notify driver didn't need copy msg to caller due
						* to the caller didn't reserve space for this cmd
						*/
#endif /* DBG */
...
typedef struct _OID_BACAP_STRUC {
	UCHAR RxBAWinLimit;
	UCHAR TxBAWinLimit;
	UCHAR Policy;		/* 0: DELAY_BA 1:IMMED_BA  (//BA Policy subfiled value in
				* ADDBA frame) 2: BA-not use. other value invalid
				*/
	UCHAR MpduDensity;	/* 0: DELAY_BA 1:IMMED_BA  (//BA Policy subfiled value in
				*  ADDBA frame) 2: BA-not use. other value invalid
				*/
...
#define OID_VENDOR_IE_ENABLE 0x0900
#define OID_SET_VENDOR_IE 0x0901
