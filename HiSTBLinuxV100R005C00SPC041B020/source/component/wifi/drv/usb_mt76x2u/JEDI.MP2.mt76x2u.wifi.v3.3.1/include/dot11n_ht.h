...
/* 7.3.2.60 */
/* in TU. min amount of time that the
* STA continuously scans each channel when performing an active OBSS scan.
*/
#define dot11OBSSScanPassiveDwell		   20
/* in TU.min amount of time that the STA continuously
* scans each channel when performing an passive OBSS scan.
*/
#define dot11OBSSScanActiveDwell	           10
/* in sec. max interval between scan operations to
* be performed to detect BSS channel width trigger events.
*/
#define dot11BSSWidthTriggerScanInterval           300
/* in TU. min total amount of time that
* the STA scans each channel when performing a passive OBSS scan.
*/
#define dot11OBSSScanPassiveTotalPerChannel	   200
/* in TU. min total amount of time that the STA scans each channel
* when performing a active OBSS scan
*/
#define dot11OBSSScanActiveTotalPerChannel	   20
/* min ratio between the delay time in performing a switch from 20MHz BSS to
* 20/40 BSS operation and the maximum interval between overlapping BSS scan operations.
*/
#define dot11BSSWidthChannelTransactionDelayFactor 5
/* in %%, max total time that a STA may be active on the medium during a period of
*  (dot11BSSWidthChannelTransactionDelayFactor * dot11BSSWidthTriggerScanInterval) seconds
*  without being obligated to perform OBSS Scan operations. default is 25(== 0.25%)
*/
...
#else
	UCHAR InfoReq:1;
	UCHAR Intolerant40:1; /* Inter-BSS. set 1 when prohibits a receiving BSS from operating
				* as a 20/40 Mhz BSS.
				*/
	UCHAR BSS20WidthReq:1; /* Intra-BSS set 1 when prohibits a receiving AP from operating
				* its BSS as a 20/40MHz BSS.
				*/
	UCHAR ObssScanExempReq:1;
	UCHAR ObssScanExempGrant:1;
	UCHAR rsv:3;
