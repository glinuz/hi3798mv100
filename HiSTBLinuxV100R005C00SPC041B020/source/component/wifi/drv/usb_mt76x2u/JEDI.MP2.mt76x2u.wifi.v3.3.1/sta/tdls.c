	...
	IN	BOOLEAN bWaitACK)
{
	...
	if (pAd->NullContext[1].InUse == FALSE)
	{
		...
		RTMPWriteTxWI(pAd, pTxWI,  FALSE, FALSE, FALSE, FALSE, TRUE, FALSE, 0, pEntry->Aid, Length,
			0, 0, (UCHAR)pAd->CommonCfg.MlmeTransmit.field.MCS, IFS_HTTXOP, FALSE, &Transmit);
		if (bWaitACK)
			pTxWI->TXRPT = 1;
		else
			pTxWI->TXRPT = 0;
