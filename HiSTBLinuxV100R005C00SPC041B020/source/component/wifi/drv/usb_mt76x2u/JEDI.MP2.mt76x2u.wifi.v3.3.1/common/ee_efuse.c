			 ...
			 IN USHORT Offset, IN USHORT Length, OUT USHORT * pData)
{
	...
	/* Step1. Write EFSROM_MODE (0x580, bit7:bit6) to 0. Read in logical
	 * view.
	 */
	...
	if (eFuseCtrlStruc.field.EFSROM_AOUT == 0x3f) {
		...
		/*              D       C       B       A       |       D       C       B       A */
		/*Return 2-bytes */
		/*The return byte statrs from S. Therefore, the little-endian will return BA, the Big-endian
		 *will return DC. */
				...
				IN USHORT Offset, IN USHORT Length, OUT USHORT *pData)
{
	...
	/* Step0. Write 10-bit of address to EFSROM_AIN (0x580, bit25:bit16).
	 *  The address must be 16-byte alignment.
	 */
	eFuseCtrlStruc.field.EFSROM_AIN = Offset & 0xfff0;

	/* Step1. Write EFSROM_MODE (0x580, bit7:bit6) to 1. Read in physical
	 * view.
	 */
	eFuseCtrlStruc.field.EFSROM_MODE = 1;

	/* Step2. Write EFSROM_KICK (0x580, bit30) to 1 to kick-off physical
	 * read procedure.
	 */
	eFuseCtrlStruc.field.EFSROM_KICK = 1;
				    ...
				    IN USHORT Offset, IN USHORT Length, IN USHORT *pData)
{
	...
	do {
		...
		/* convert the address from 10 to 8 bit ( bit7, 6 = parity and bit5 ~ 0 = bit9~4),
		 * and write to logical map entry */
		tmpOffset = Offset;
		tmpOffset >>= 4;
		tmpOffset |=
		    ((~
		...
		/* Step 5. Compare data if not the same, invalidate the mapping entry,
		 * then re-write the data until E-fuse is exhausted */
		bWriteSuccess = TRUE;
		for (i = 0; i < 8; i++) {
			addr = BlkNum * 0x10;
...
INT set_eFuseLoadFromBin_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	INT TotalByte = 0;
	USHORT *PDATA;
	UCHAR all_ff[16] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF };
	UCHAR *ptr;
	UCHAR index;
	USHORT offset = 0;
...
VOID eFuseGetFreeBlockCount(IN PRTMP_ADAPTER pAd, PUINT EfuseFreeBlock)
{
	...
	*EfuseFreeBlock = 0;
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s LastFreeBlock= 0x%x\n", __func__,
				  LastFreeBlock));

	/* if not find last free block, return free block number = 0, this
	 * should not happen since we have checked first free block number
	 * previously.
	 */
	...
	/* return total free block number, last free block number must >= first
	 * free block number
	 */
...
INT eFuse_init(RTMP_ADAPTER *pAd)
{
	...
	/* If the used block of efuse is less than 5, we assume the default
	 * value of this efuse is empty and change to the buffer mode in odrder
	 * to bring up interfaces successfully. */
	if (EfuseFreeBlock >= pAd->chipCap.EFUSE_RESERVED_SIZE) {
		DBGPRINT(RT_DEBUG_OFF, ("NVM is efuse but the info is too less to bring up the interface\n"));
		DBGPRINT(RT_DEBUG_OFF, ("Load EEPROM buffer from BIN, and force to use BIN buffer mode\n"));

		pAd->bUseEfuse = FALSE;
		result = rtmp_ee_load_from_bin(pAd);

		/* Force to use BIN eeprom buffer mode */
...
INT efuse_probe(RTMP_ADAPTER *pAd)
{
	...
	/* [GeK]might be un-initialized, init before return */
	pAd->bUseEfuse = FALSE;
	...
	pAd->bUseEfuse = (eFuseCtrl & 0x80000000) ? TRUE : FALSE;
