...
VOID HandleCounterMeasure(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	/* record which entry causes this MIC error, if this entry sends disauth/disassoc,
	...
	if (pAd->ApCfg.CMTimerRunning == TRUE) {
		...
		for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
			/* happened twice within 60 sec,  AP SENDS disaccociate all associated STAs.
...
VOID ApCliRTMPReportMicError(IN PRTMP_ADAPTER pAd, IN PCIPHER_KEY pWpaKey, IN INT ifIndex)
{
	...
	if (pAd->ApCfg.ApCliTab[ifIndex].MicErrCnt == 0) {
		...
		if ((pAd->ApCfg.ApCliTab[ifIndex].LastMicErrorTime + (60 * OS_HZ)) < Now) {
			...
			/* RTMPSendWirelessEvent(pAd, IW_COUNTER_MEASURES_EVENT_FLAG, pAd->
			MacTab.Content[BSSID_WCID].Addr, BSS0, 0); */
			...
			/*
			   ...
			   No necessary to clean all Tx ring, on RTMPHardTransmit will stop sending
			   non-802.1X EAPOL packets
