...
HI_S32  MJPEG_DecodeFrame(HI_HANDLE hInst, HI_CODEC_STREAM_S * pstIn, HI_CODEC_FRAME_S * pstOut)
{
    ...
    Surface.SurfaceInfo.MbSurface.YVirtAddr    = (HI_VOID*)pstOut->stOutputAddr.pu8Vir;
    ...
    Surface.SurfaceInfo.MbSurface.CbCrVirtAddr = (HI_VOID*)(pstOut->stOutputAddr.pu8Vir + MemInfo.YMemSize);
...
HI_S32 MJPEG_DecodeFrame(HI_HANDLE hInst, HI_CODEC_STREAM_S * pstIn, HI_CODEC_FRAME_S * pstOut)
{
    ...
    HI_JPEG_SetStreamPhyMem(&s_stMjpegParam[hInst].stInfo, pstIn->u32PhyAddr);
    ...
    s32Ret = HI_JPEG_GetJpegInfo(&s_stMjpegParam[hInst].stInfo, &stJpegInfo);
    if (HI_SUCCESS != s32Ret)
	{
    ...
    switch (stJpegInfo.enFmt)
    {
        case JPEG_FMT_YUV400:
            s_stMjpegParam[hInst].stInfo.out_color_space = JCS_YUV400_SP;
            pstOut->unInfo.stVideo.enColorFormat = HI_CODEC_COLOR_FORMAT_YUV_SEMIPLANAR_400;
            break;
        case JPEG_FMT_YUV420:
            s_stMjpegParam[hInst].stInfo.out_color_space = JCS_YUV420_SP;
            pstOut->unInfo.stVideo.enColorFormat = HI_CODEC_COLOR_FORMAT_YUV_SEMIPLANAR_420;
            break;
        case JPEG_FMT_YUV444:
            s_stMjpegParam[hInst].stInfo.out_color_space = JCS_YUV444_SP;
            pstOut->unInfo.stVideo.enColorFormat = HI_CODEC_COLOR_FORMAT_YUV_SEMIPLANAR_444;
            break;
        case JPEG_FMT_YUV422_12:
            s_stMjpegParam[hInst].stInfo.out_color_space = JCS_YUV422_SP_12;
            pstOut->unInfo.stVideo.enColorFormat = HI_CODEC_COLOR_FORMAT_YUV_SEMIPLANAR_422_1X2;
            break;
        case JPEG_FMT_YUV422_21:
            s_stMjpegParam[hInst].stInfo.out_color_space = JCS_YUV422_SP_21;
            pstOut->unInfo.stVideo.enColorFormat = HI_CODEC_COLOR_FORMAT_YUV_SEMIPLANAR_422_2X1;
    ...
    if ((stJpegInfo.enFmt == JPEG_FMT_YUV444) || (stJpegInfo.enFmt == JPEG_FMT_YUV422_12))
    {
        s32Ret = HI_JPEG_SetYCbCr420spFlag(&s_stMjpegParam[hInst].stInfo,HI_TRUE);
    ...
    s32Ret = HI_JPEG_SetOutDesc(&s_stMjpegParam[hInst].stInfo, &stSurfaceDesc);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 StopDecodeFrame(HI_HANDLE hInst)
{
    ...
    HI_JPEG_SetLowDelayInf(&s_stMjpegParam[hInst].stInfo, g_lowDelayLinePhyAddr[hInst], (HI_CHAR*)(HI_ULONG)g_lowDelayLineVirAddr[hInst]);
