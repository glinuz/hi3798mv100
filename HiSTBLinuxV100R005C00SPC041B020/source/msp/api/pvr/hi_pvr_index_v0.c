...
static ssize_t PVRIndexReadIndexFromCache0(PVR_INDEX_HANDLE hIndex, HI_VOID *pBuf, HI_U32 u32ReadFrm)
{
    ...
    //检查是否在文件中
    if (!PVRIndexCheckReadFrmIsInFile0(u32ReadFrm, &hIndex->stFileCycInfo))
    {
...
//对readcache进行边界检查，保证头部和尾部不在无效区间内
    if (PVRIndexCheckReadCacheHeadValid0(hIndex->stIdxReadCache.u32StartNum, u32ReadFrm, &hIndex->stFileCycInfo) &&
        PVRIndexCheckReadCacheTailValid0(hIndex->stIdxReadCache.u32StartNum + hIndex->stIdxReadCache.u32UsedNum - 1, u32ReadFrm, &hIndex->stFileCycInfo))
    {
...
//如果被覆盖或者没cache  中，重新读取cache

    {
        ...
        if (!PVRIndexCheckReadCacheHeadValid0((HI_U32)s32Start, u32ReadFrm, &hIndex->stFileCycInfo))
        {
            s32Start = hIndex->stFileCycInfo.u32StartFrame;
        }
        ...
        if (!PVRIndexCheckReadCacheTailValid0(u32CacheEndNum, u32ReadFrm, &hIndex->stFileCycInfo))
        {
...
static ssize_t PVRIndexCacheReadIdx0(PVR_INDEX_HANDLE handle, PVR_FILE fd, HI_VOID* pData, size_t size, 
                               HI_U32 u32Offset, HI_U32 u32DirectFlag)
{
    ...
    {
        HI_ERR_PVR("NULL handle->stIdxReadCache.pu8Addr!\n");
...
#endif
#endif
size_t PVRIndex_ReadIndex0(PVR_INDEX_ENTRY_NEW_S *pstEntryBuf, HI_S32 s32Num, PVR_FILE fd, HI_U64 u64Offset, PVR_INDEX_HANDLE handle)
{
    size_t readSz = 0;
#ifndef HI_PVR_EXTRA_BUF_SUPPORT
    HI_U32 u32Size = 0;
...
#ifndef HI_PVR_EXTRA_BUF_SUPPORT
    u32Size = s32Num*sizeof(PVR_INDEX_ENTRY_S);
    readSz = PVRIndexCacheReadIdx0(handle, fd, &(pstEntryBuf->stEntry1), u32Size, (off_t)u64Offset,0);
#else
...
{
    ssize_t readNum ;
    readNum = PVRIndex_ReadIndex0(pEntry,1,handle->s32ReadFd,(u32FrameNum * sizeof(PVR_INDEX_ENTRY_S)),handle);
    if (readNum != (ssize_t)sizeof(PVR_INDEX_ENTRY_S))
    {
        if (-1 == readNum)
        {
            HI_ERR_PVR("read failed in PVRIndexGetEntryByNum");
            return HI_ERR_PVR_FILE_CANT_READ;
        }
        else
        {
            HI_ERR_PVR("read to end, S:%d, E:%d, L:%d, C:%d G:%d\n", handle->stCycMgr.u32StartFrame, handle->stCycMgr.u32EndFrame,
