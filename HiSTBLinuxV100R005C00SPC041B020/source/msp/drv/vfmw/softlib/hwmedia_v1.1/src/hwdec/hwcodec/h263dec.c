...
BVpQe37EcBHeQzdD2JsEBseJgo1oFnGgErkhdhHfeKTlkvjkcRTfwbgqgaSeyV8yalGxc6St
iK0YP6TObqXXG+sEYJyuGSAJXKBbkmjRIih3BTbUlFafRyuVyLCuCLFYy0BwpQ==#*/
...
static int decode_slice(MpegEncContext *s)
{
    ...
    for (; s->mb_y < s->mb_height; s->mb_y++)
    {
        ...
        for (; s->mb_x < s->mb_width; s->mb_x++)
        {
            ...
            if (ret < 0)
            {
                const int xy __attribute__((unused)) = s->mb_x + s->mb_y * s->mb_stride;
