    ...
    switch(pstCmd->enDbgType)
    {
        ...
        case DBG_W_YUV_FILE:
            switch(pstCmd->hDbgPart)
            {
                case DEF_DBG_SRC_ID:
                    ...
                    pstInstDbg->unInfo.bits.start_index = (pstCmd->hDbgValue >> 16) & 0xFFFF;
                ...
                case DEF_DBG_PORT2_ID:
                    ...
                    pstPortDbg->unInfo.bits.start_index = (pstCmd->hDbgValue >> 16) & 0xFFFF;
                    pstPortDbg->unInfo.bits.end_index = pstPortDbg->unInfo.bits.start_index + 
                                    (((pstCmd->hDbgValue & 0xFFFF) > 16) ? 16 : (pstCmd->hDbgValue & 0xFFFF));
        ...
        case DBG_REVISE_SRC_WH:
            ...
            VPSS_FATAL("Revise src image width*height %d*%d \n", 
                        (pstCmd->hDbgValue>>16)&0xFFFF, pstCmd->hDbgValue&0xFFFF);
            break;
        default:
            ...
            switch (u32DbgPart)
            {
                ...
                case DEF_DBG_PORT2_ID:
                        ...
                        || ((enCmd == DBG_W_YUV_IDX) && pstPortDbg->unInfo.bits.writestreamIdx))
                    {
                        ...
                        {
                            //VPSS_FATAL("index %d is not in [%d, %d]\n", pstFrm->u32FrameIndex, u16StartIdx, u16EndIdx);
                        }
                    }
					...
					if (pstPortDbg->unInfo.bits.writestreamIdx)
					{
