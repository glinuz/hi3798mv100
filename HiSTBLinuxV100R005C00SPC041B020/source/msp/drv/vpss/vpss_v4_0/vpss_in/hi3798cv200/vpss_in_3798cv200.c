...
HI_S32 VPSS_IN_CorrectProgInfo(VPSS_IN_ENTITY_S *pstEntity,HI_DRV_VIDEO_FRAME_S *pImage)
{
	...
	return	HI_SUCCESS;
#endif
	//el,hdr10..need detile,but no alg hdr type, frame fix to prog
	if (VPSS_HDR_TYPE_NEED_DETILE_NOALG(pImage->enSrcFrameType))
	{
		pImage->bProgressive = HI_TRUE;
		return	HI_SUCCESS;
	}
...
HI_S32 VPSS_IN_Refresh_V3(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    */
    ...
    if (0)
    {
        ...
        HI_PRINT("Image Info:Index %d Type %d Format %d W %d H %d Prog %d topfirst %d FieldMode %d enBufValidMode %d PTS %d Rate %d LastFlag %#x Delta %d CodeType %d,SourceType %d,BitWidth %d\n"
