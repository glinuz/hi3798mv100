...
L0_Context  Si2144_rawI2C_context;
...
char *traceElapsedTime       (void) {
    ...
    timeElapsed = Si2144_system_time();
    ms=timeElapsed%1000;
    ...
    snprintf(trace_elapsed_time,"%02d:%02d:%02d.%03d ",hours,min,sec,ms);
...
char *SiTraceConfiguration   (const char *config) {
    ...
    if (!strncmp(trace_message,"traces",6)) {
        ...
        for(loop=0;loop<nbArgs;loop++) {
            if(!Si2144_strcmp_nocase_n(array[loop],"help",4)) {
                /*print the help menu*/
...
#if       SiTRACES_FEATURES == SiTRACES_FULL
            else if(!Si2144_strcmp_nocase_n(array[loop],"erase"   ,5) ) {
                file = fopen(trace_file_name,"w");
                if(!file) {
                  trace_output_type=TRACE_MEMORY;
            ...
            else if(!Si2144_strcmp_nocase_n(array[loop],"save"    ,4) ) {
            ...
            else if(!Si2144_strcmp_nocase_n(array[loop],"show"    ,4) ) {
            ...
            else if(!Si2144_strcmp_nocase_n(array[loop],"-name"   ,5) ) {
                trace_configuration_message=1;
                if(!array[loop+1]) {
...
#endif /* SiTRACES_FEATURES == SiTRACES_FULL */
            else if(!Si2144_strcmp_nocase_n(array[loop],"-verbose",8) ) {
                trace_configuration_message=1;
                if((!Si2144_strcmp_nocase_n(array[loop+1],"on" ,2)) || (!strncmp(array[loop+1],"1",1))) trace_config_verbose=1;
                if((!Si2144_strcmp_nocase_n(array[loop+1],"off",3)) || (!strncmp(array[loop+1],"0",1))) trace_config_verbose=0;
            }
            else if(!Si2144_strcmp_nocase_n(array[loop],"count"   ,5) ) {
                sprintf(trace_stockInt,"%d\n",trace_count);
                trace_count=0;
                return trace_stockInt;
            }
            else if(!Si2144_strcmp_nocase_n(array[loop],"suspend" ,7) ) {
                trace_suspend = 1;
                sprintf(trace_stockInt,"%d\n",trace_suspend);
                return trace_stockInt;
            }
            else if(!Si2144_strcmp_nocase_n(array[loop],"resume"  ,6) ) {
                trace_suspend=0;
                sprintf(trace_stockInt,"%d\n",trace_suspend);
                return trace_stockInt;
            }
            else if(!Si2144_strcmp_nocase_n(array[loop],"lost"    ,4) ) {
                sprintf(trace_stockInt,"%d\n",trace_lost_traces);
                return trace_stockInt;
            }
            else if(!Si2144_strcmp_nocase_n(array[loop],"flush"   ,5) ) {
                trace_configuration_message=1;
              /* empty the trace buffer */
                if(strlen(trace_buffer)) {traceFlushBuffer(trace_buffer);}
                return (char *)"traces memory flushed\n";
              }
            else if(!Si2144_strcmp_nocase_n(array[loop],"get"     ,3) ) {
            ...
            else if(!Si2144_strcmp_nocase_n(array[loop],"status"  ,6) ) {
                switch(trace_output_type) {
            ...
            else if(!Si2144_strcmp_nocase_n(array[loop],"-output"  ,7) ) {
                trace_configuration_message=1;
                if (!Si2144_strcmp_nocase_n(array[loop+1],"none"  ,4)) trace_output_type=TRACE_NONE;
                if (!Si2144_strcmp_nocase_n(array[loop+1],"memory",6)) trace_output_type=TRACE_MEMORY;
                if (!Si2144_strcmp_nocase_n(array[loop+1],"stdout",6)) trace_output_type=TRACE_STDOUT;
#if       SiTRACES_FEATURES == SiTRACES_FULL
                if (!Si2144_strcmp_nocase_n(array[loop+1],"file"  ,4)) trace_output_type=TRACE_EXTERN_FILE;
#endif /* SiTRACES_FEATURES == SiTRACES_FULL */
            }
            else if(!Si2144_strcmp_nocase_n(array[loop],"-file"    ,5) ) {
                trace_configuration_message=1;
                if((!Si2144_strcmp_nocase_n(array[loop+1],"on" ,2)) || (!strncmp(array[loop+1],"1",1))) trace_config_files=1;
                if((!Si2144_strcmp_nocase_n(array[loop+1],"off",3)) || (!strncmp(array[loop+1],"0",1))) trace_config_files=0;
            }
            else if(!Si2144_strcmp_nocase_n(array[loop],"-line"    ,5) ) {
                trace_configuration_message=1;
                if((!Si2144_strcmp_nocase_n(array[loop+1],"on" ,2)) || (!strncmp(array[loop+1],"1",1))) trace_config_lines=1;
                if((!Si2144_strcmp_nocase_n(array[loop+1],"off",3)) || (!strncmp(array[loop+1],"0",1))) trace_config_lines=0;
            }
            else if(!Si2144_strcmp_nocase_n(array[loop],"-function",9) ) {
                trace_configuration_message=1;
                if((!Si2144_strcmp_nocase_n(array[loop+1],"on" ,2)) || (!strncmp(array[loop+1],"1",1))) trace_config_functions=1;
                if((!Si2144_strcmp_nocase_n(array[loop+1],"off",3)) || (!strncmp(array[loop+1],"0",1))) trace_config_functions=0;
            }
            else if(!Si2144_strcmp_nocase_n(array[loop],"-time"    ,5) ) {
                trace_configuration_message=1;
                if((!Si2144_strcmp_nocase_n(array[loop+1],"on" ,2)) || (!strncmp(array[loop+1],"1",1))) trace_config_time=1;
                if((!Si2144_strcmp_nocase_n(array[loop+1],"off",3)) || (!strncmp(array[loop+1],"0",1))) trace_config_time=0;
            }
        }
    }
...
************************************************************************************************************************/
int  Si2144_strcmp_nocase   (const char* str1, const char* str2) {
    size_t       l;
    unsigned int i;
    int       diff;
    l = strlen(str1);
...
/************************************************************************************************************************
  ...
  Returns:    0 if identical
************************************************************************************************************************/
int  Si2144_strcmp_nocase_n (const char* str1, const char* str2, unsigned int n) {
    size_t       l;
    size_t       l1;
    size_t       l2;
    unsigned int i;
...
int  Si2144_L0_StoreError   (const char *message) {
  int length;
  int length_new_msg;
...
int  Si2144_L0_ErrorMessage (void) {
  int length;
  length = sprintf(Si2144_L0_error_message, "%s", stored_error_message);
...
************************************************************************************************************************/
unsigned int     Si2144_system_time          (void)        {
  /* <porting> Replace 'clock' by whatever is necessary to return the system time in milliseconds */
  //return (int)clock()*1000/CLOCKS_PER_SEC;
  struct timeval stSi2144Time;
  HI_U32  u32SystemTimeInMs = 0;

  do_gettimeofday(&stSi2144Time);
  u32SystemTimeInMs = stSi2144Time.tv_sec*1000 + stSi2144Time.tv_usec/1000;
...
************************************************************************************************************************/
void    Si2144_L0_Init              (unsigned int u32TunerId,L0_Context *i2c,unsigned int u32I2cChannel) {
  int i;
  #ifdef    USB_Capability
  ...
  rawI2C = &Si2144_rawI2C_context;
  rawI2C->indexSize = 0;
  stored_error_message = L0_error_buffer_0;
  Si2144_L0_error_message     = L0_error_buffer_1;
  sprintf(stored_error_message, "%s", "");
  #ifdef    USB_Capability
  if (!cypress_checks_done) {
    Si2144_L0_Connect (i2c, CUSTOMER);
...
************************************************************************************************************************/
char*   Si2144_L0_InterfaceType     (L0_Context* i2c) {
...
************************************************************************************************************************/
int     Si2144_L0_SetAddress        (L0_Context* i2c, unsigned int add, int addSize) {
  i2c->address   = add;
  i2c->indexSize = addSize;
  switch (i2c->connectionType) {
  #ifdef USB_Capability
    case USB:
      Si2144_L0_Connect (i2c, i2c->connectionType);
      return 1;
      break;
  #endif /* USB_Capability */
    case SIMU:
...
************************************************************************************************************************/
int     Si2144_L0_Connect           (L0_Context *i2c, CONNECTION_TYPE connType) {
  #ifdef    USB_Capability
...
/************************************************************************************************************************
  ...
  Returns:    void
************************************************************************************************************************/
void    Si2144_L0_TrackRead         (L0_Context* i2c, unsigned int track) {
  i2c->trackRead  = track;
}
/************************************************************************************************************************
  ...
  Returns:    void
************************************************************************************************************************/
void    Si2144_L0_TrackWrite        (L0_Context* i2c, unsigned int track) {
...
************************************************************************************************************************/
int     Si2144_L0_ReadBytes         (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer) {
  ...
  switch (i2c->connectionType) {
    ...
    case CUSTOMER:
      ...
      */
      //HI_INFO_TUNER("TunerId:%d,I2cChannel:%d,Addr:0x%x\n",i2c->u32TunerId,i2c->u32I2cChannel,i2c->address);
      for(i = 0;i < 1;i++)
      {
	      hi3137_config_i2c_out(i2c->u32TunerId);
	      //s32Ret = HI_DRV_I2C_Read_SiLabs(i2c->u32I2cChannel,i2c->address,(unsigned char)iI2CIndex,1,pucDataBuffer,iNbBytes);
	      s32Ret = HI_DRV_I2C_Read(i2c->u32I2cChannel,i2c->address,0xFE,1,pucDataBuffer,iNbBytes);
		  if(HI_SUCCESS != s32Ret)
		  {
				HI_ERR_TUNER("HI_DRV_I2C_Read_SiLabs err.\n");
		  }
      }
	  nbReadBytes = iNbBytes;
      break;
  ...
  trace_skip_info = 1;
    ...
    sprintf(trace_L0_message, "%s <%s", trace_L0_message, Si2144_L0_InterfaceType(i2c));
...
************************************************************************************************************************/
int     Si2144_L0_WriteBytes        (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer) {
	HI_S32 s32Ret = HI_SUCCESS;
    HI_U8 pArray[256];
  int r, i=0, nbWrittenBytes, write_error;
  nbWrittenBytes = 0;
  write_error    = 0;
  for (i=0; i <i2c->indexSize;i++) {
    r = 8*(i2c->indexSize -1 -i);
...
/************************************************************************************************************************
  ...
  Returns:    the number of bytes read.
************************************************************************************************************************/
int     Si2144_L0_ReadCommandBytes  (L0_Context* i2c, int iNbBytes, unsigned char *pucDataBuffer) {
  return Si2144_L0_ReadBytes (i2c, 0x00, iNbBytes, pucDataBuffer);}
/************************************************************************************************************************
...
************************************************************************************************************************/
int     Si2144_L0_WriteCommandBytes (L0_Context* i2c, int iNbBytes, unsigned char *pucDataBuffer) {
  return Si2144_L0_WriteBytes(i2c, 0x00, iNbBytes, pucDataBuffer);}
/************************************************************************************************************************
  ...
  Returns:    the number of bytes read.
************************************************************************************************************************/
int     Si2144_L0_ReadRawBytes      (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pbtDataBuffer) {
  int i,r,ret;
  ...
  if (i2c->indexSize >0) {
    ...
    ret = Si2144_L0_WriteBytes(rawI2C, 0, i2c->indexSize, pbtIndexBuffer);
    if (ret == 0) return 0; /* There has been a communication error, do not attempt to read */
  }
  /* Now read the data bytes */
  ret = Si2144_L0_ReadBytes (rawI2C, 0, iNbBytes, pbtDataBuffer);
  return ret;
}
/************************************************************************************************************************
  ...
  Returns:    the number of bytes written.
************************************************************************************************************************/
int     Si2144_L0_WriteRawBytes     (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pbtDataBuffer) {
  int i,r,ret;
  ...
  rawI2C->trackWrite     = i2c->trackWrite;
  rawI2C->connectionType = i2c->connectionType;
  //pbtBuffer = (unsigned char*) kmalloc((iNbBytes + i2c->indexSize)*sizeof(unsigned char),GFP_KERNEL);
  pbtBuffer = HI_KMALLOC(HI_ID_TUNER, (iNbBytes + i2c->indexSize)*sizeof(unsigned char), GFP_KERNEL);
  ...
  ret = Si2144_L0_WriteBytes (rawI2C, 0, iNbBytes + i2c->indexSize, pbtBuffer) - i2c->indexSize;
  HI_KFREE(HI_ID_TUNER,pbtBuffer);
  return ret;
}
/************************************************************************************************************************
...
************************************************************************************************************************/
long    Si2144_L0_ReadRegister      (L0_Context *i2c, unsigned int iI2CIndex, unsigned char offset, unsigned char nbbit, unsigned int isSigned) {
  ...
  if (i2c->mustReadWithoutStop==0) {
    nbReadBytes = Si2144_L0_ReadRawBytes(i2c    , iI2CIndex, iNbBytes, registerDataBuffer);
  } else {
    nbReadBytes = Si2144_L0_ReadBytes   (i2c    , iI2CIndex, iNbBytes, registerDataBuffer);
  }
...
************************************************************************************************************************/
long    Si2144_L0_WriteRegister     (L0_Context *i2c, unsigned int iI2CIndex, unsigned char offset, unsigned char nbbit, unsigned char alone, long Value) {
  ...
  nbWrittenBytes = Si2144_L0_WriteBytes(i2c, iI2CIndex, iNbBytes, registerDataBuffer);
...
/************************************************************************************************************************
  ...
  Returns: the value written. O if error during the write
************************************************************************************************************************/
long    Si2144_L0_ReadRegisterTrace (L0_Context *i2c, char* name, unsigned int iI2CIndex, unsigned char offset, unsigned char nbbit, unsigned int isSigned) {
  long res = Si2144_L0_ReadRegister (i2c, iI2CIndex, offset, nbbit, isSigned);
...
/************************************************************************************************************************
  ...
  Returns: the value written. O if error during the write
************************************************************************************************************************/
long    Si2144_L0_WriteRegisterTrace(L0_Context *i2c, char* name, char* valtxt, unsigned int iI2CIndex, unsigned char offset, unsigned char nbbit, unsigned char alone, long Value) {
  unsigned int iNbBytes;
  long res;
  iNbBytes = (nbbit + offset + 7)/8;
#ifdef    SiTRACES
...
#endif /* SiTRACES */
  res = Si2144_L0_WriteRegister (i2c, iI2CIndex, offset, nbbit, alone, Value);
  return res;
}
#if 0
/************************************************************************************************************************
...
/*    SiTRACE("L0_ReadString address 0x%02x, index 0x%02x (on %d bytes), %d bytes\n", address, iI2CIndex, indexSize, iNbBytes); */
    ...
    readBytes = Si2144_L0_ReadBytes (i2c, iI2CIndex, iNbBytes, pbtDataBuffer);
...
int     L0_WriteString       (L0_Context* i2c, char *writeString) {
    ...
    L0_SetAddress   (i2c, address, indexSize);
    writeBytes = Si2144_L0_WriteBytes (i2c, iI2CIndex, iNbBytes, pbtDataBuffer);
    return writeBytes;
}
#endif
