                               ...
                               struct snd_pcm_hw_params* params, struct snd_pcm_runtime* runtime)
{
    if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
    {
        ...
        stAII2sAlsaAttr.pSubstream              = (void*)substream;
        stAII2sAlsaAttr.stBuf.u32BufPhyAddr     = runtime->dma_addr;    // for dma buffer
        stAII2sAlsaAttr.stBuf.pu8BufVirAddr     = (HI_U8*)runtime->dma_area;
    ...
    else
    {
        ...
        stAOI2sAlsaAttr.pSubstream              = (void*)substream;
        stAOI2sAlsaAttr.stBuf.u32BufPhyAddr     = runtime->dma_addr;    // for dma buffer
        stAOI2sAlsaAttr.stBuf.pu8BufVirAddr     = (HI_U8*)runtime->dma_area;
                              ...
                              struct snd_pcm_hw_params* params)
{
    ...
    if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
    {
        ...
        ret = hi_ai_alsa_open(&stAiParam, (HI_VOID*)(&stAII2sAlsaAttr), &had->cfile);
    ...
    else
    {
        if (had->open_cnt == 0)
        {
            ...
            ret = ao_dma_open(&stAoParam, (void*)&stAOI2sAlsaAttr, &had->file);
...
#else
    ret = hiaudio_ao_hw_proc_init(soc_platform->component.card->snd_card, ALSA_PROC_AO_NAME, had);
    ret = hiaudio_ai_hw_proc_init(soc_platform->component.card->snd_card, ALSA_PROC_AI_NAME, had);
#endif
#endif
    return ret;
}
...
static int __init soc_hw_snd_platform_probe(struct platform_device* pdev)
{
    ...
    had = HI_KZALLOC(HI_ID_AO, sizeof(struct hii2saudio_data), GFP_KERNEL);
    if (had == NULL)
    {
        return -ENOMEM;
    }
