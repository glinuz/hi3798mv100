                     ...
                     int32 *work_buff)
{
    ...
    {
        int32 band;
        for (band = 0; band < FILTERBANK_BANDS; band += 2)
        {
    ...
    else
    {
        ...
        int32 band;
        for (band = 0; band < FILTERBANK_BANDS; band += 3)
        {
            int32 *inData = &circ_buffer[544 - (band<<5)];
...
void pvmp3_equalizer_mp2(int32 *circ_buffer,
                     e_equalization equalizerType,
                     int32 *work_buff,
                     int16 filterbands)
{
    ...
    {
        int32 band;
        for (band = 0; band < filterbands; band += 2)
        {

            int32 *pt_work_buff = &work_buff[band];
            int32 *inData = &circ_buffer[((filterbands-1)*SUBBANDS_NUMBER) - (band<<5)];

            int32 i;
            for (i = 0; i < SUBBANDS_NUMBER*filterbands; i += filterbands << 2)
            {
                int32 temp1 = (pt_work_buff[ i ]);
                int32 temp2 = (pt_work_buff[ i +   filterbands ]);
                int32 temp3 = (pt_work_buff[ i + 2*filterbands ]);
                int32 temp4 = (pt_work_buff[ i + 3*filterbands ]);
                *(inData++) = temp1;
                *(inData++) = temp2;
                *(inData++) = temp3;
                *(inData++) = temp4;
