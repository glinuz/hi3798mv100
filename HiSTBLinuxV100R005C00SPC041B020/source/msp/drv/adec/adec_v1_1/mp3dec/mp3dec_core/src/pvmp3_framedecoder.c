                              ...
                              void              *pMem)
{
    ...
    outputFrameSize = (info->version_x == MPEG_1) ?
...
#if defined (MPEG_LAYER12_SUPPORT)
    if ((info->layer_description == 2) || (info->layer_description == 1))
    {
		int *mdctBuf[2];
        int32 ch;
        ...
        if (frmBytes > pExt->inputBufferCurrentLength)
        {
            pExt->outputFrameSize = 0;
            pExt->inputBufferUsedLength = 0;
            return NO_ENOUGH_MAIN_DATA_ERROR;
        }
		mdctBuf[0] = pChVars[0]->work_buf_int32;
		mdctBuf[1] = pChVars[1]->work_buf_int32;

        if(info->layer_description == 2)
        {
        	Ret = LAYER12_DecodeLayer2(&pVars->inputStream, info->mode, info->version_x, info->layer_description,
                                        info->bitrate_index, info->sampling_frequency, info->mode_ext, mdctBuf);
        	}	
		else
		{
        	Ret = LAYER12_DecodeLayer1(&pVars->inputStream, info->mode, info->version_x, info->layer_description,
                                        info->bitrate_index, info->sampling_frequency, info->mode_ext, mdctBuf);
		}
        ...
        {
            pExt->outputFrameSize = 0;
            pExt->inputBufferUsedLength = 1;   // discard 1 bytes
        ...
        else if (info->layer_description == 1)
        {
            filterbands = 12;
        }
        for (ch = 0; ch < pVars->num_channels; ch++)
        {
            ...
            pvmp3_poly_phase_synthesis_mp2(pChVars[ch],
                                           pVars->num_channels,
                                           pExt->equalizerType,
                                           &ptrOutBuffer[ch],
                                           filterbands);
        } /* end ch loop */
        samples = filterbands * SUBBANDS_NUMBER;

        pExt->outputFrameSize = samples * pExt->num_channels;
        pExt->inputBufferUsedLength  = (pVars->inputStream.usedBits + 0x7) >> 3;
    ...
    if (info->layer_description == 3)
    {
        ...
        if (errorCode != NO_DECODING_ERROR)
        {
			...
			{
				// consume data,avoid dead-loop
				if(pVars->inputStream.usedBits)
				{
					pExt->inputBufferUsedLength  = (pVars->inputStream.usedBits + 0x7) >> 3; 
				}
				else
        ...
        temp = pvmp3_get_main_data_size(info, pVars);
        ...
        if (bytes_to_discard < 0 || crc_error_count)
        {
            ...
            pv_memset((void*)&pChVars[ LEFT]->circ_buffer[0],
                      0,
                      (1152+480) * sizeof(pChVars[ LEFT]->circ_buffer[0]));
            pv_memset((void*)&pChVars[RIGHT]->circ_buffer[0],
                      0,
                      (1152+480) * sizeof(pChVars[RIGHT]->circ_buffer[0]));
        ...
        for (gr = 0; gr < (1 + !(info->version_x)); gr++)
        {
            ...
            for (ch = 0; ch < pVars->num_channels; ch++)
            {
                 ...
                 */
                mixedBlocksLongBlocks = 0; /*  0 = long or short, 2=mixed, 4=mixed 2.5@8000 */
...
void fillMainDataBuf(void  *pMem, int32 temp)
{
    ...
    if ((offset + temp) < BUFSIZE)
    {
        ...
        else
        {
            int32 tmp1 = *(ptr++);
            int32 nBytes;
            for (nBytes = temp >> 1; nBytes != 0; nBytes--)  /* read main data. */
            {
                int32 tmp2 = *(ptr++);
