...
HI_S32 WinHalSetRect_MPW(HI_U32 u32LayerId, WIN_HAL_PARA_S *pstPara, HI_S32 s32exl)
{
	...
	else if (u32LayerId == VDP_LAYER_VID3)
	{
		...
		if (pstPara->pstFrame->enFieldMode == HI_DRV_FIELD_ALL)
		{
			eFmode  = VDP_RMODE_INTERFACE;
		}
		else if (pstPara->pstFrame->enFieldMode == HI_DRV_FIELD_TOP)
		{
			eFmode  = VDP_RMODE_TOP;
		}
		else if (pstPara->pstFrame->enFieldMode == HI_DRV_FIELD_BOTTOM)
		{
			eFmode  = VDP_RMODE_BOTTOM;
		}
