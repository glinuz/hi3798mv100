...
#define VIDEO_ZOOM_IN_VERTICAL_MAX   7/4
#else
...
typedef struct tagWIN_HAL_PARA_S
{
	...
	/*for v0 which has zme, field read or frame read.*/
	HI_BOOL  bFieldRead;
	HI_BOOL  bFieldOut;
...
typedef struct tagVIDEO_LAYER_FUNCTIONG_S
{
	...
	/*since, zme has a scale capability, when out of capability, we should drop line.
	  *so we should give a func to get the  extract line number.
	  */
