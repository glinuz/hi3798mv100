...
HI_VOID  VDP_DRIVER_Initial()
{
    ...
    /*4、in hifone, cmb2 is deleted.*/
    VDP_WBC_SetConnection(VDP_LAYER_WBC_HD0, 0);
...
HI_VOID DISP_ResetCRG(HI_VOID)
{
    ...
    unTmpValue.bits.vo_ppc_cken  = 1;
    unTmpValue.bits.sd_ppc_cken  = 1;
    unTmpValue.bits.hd_ppc_cken  = 1;
...
=============================================================================*/
	...
	unTmpValue.bits.vo_hd_sub_cken	   = 1;
...
HI_VOID VDP_CloseClkResetModule(HI_VOID)
{
    ...
    //U_PERI_CRG126 unTmpValue2;  /*hifone  no used*/
    //U_PERI_CRG127 unTmpValue127;

    /*to be reviewed, datasheet: vdac bg分频选择：S5V100不需要用*/
   ...
   // unTmpValue2.bits.vdac_b_srst_req = 1;
    ...
    unTmpValue1.bits.vo_ppc_cken  = 0;
    unTmpValue1.bits.sd_ppc_cken  = 0;
    unTmpValue1.bits.hd_ppc_cken  = 0;
    unTmpValue1.bits.vdp_cfg_cken = 0;
    unTmpValue1.bits.vdp_clk_sel  = 0;
    g_pstRegCrg->PERI_CRG54.u32 = unTmpValue1.u32;
...
HI_VOID VDP_SelectClk(HI_U32 u32VDPClkMode)
{
    ...
    PERI_CRG54Tmp.u32 = g_pstRegCrg->PERI_CRG54.u32;
    PERI_CRG54Tmp.bits.vdp_clk_sel = u32VDPClkMode;/*300M*/
    g_pstRegCrg->PERI_CRG54.u32 = PERI_CRG54Tmp.u32;
#endif
}
...
HI_VOID VDP_VDAC_SetEnable(HI_U32 uVdac, HI_U32 enable,HI_U32 u32DelayMs)
{
    ...
    if (enable)
    {
		...
		VO_DAC_CTRL0.u32 = VDP_RegRead((HI_U32)&(pVdpReg->VO_DAC_CTRL0.u32));
        VO_DAC_CTRL0.u32 |=  1<<22;
        VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_CTRL0.u32)), VO_DAC_CTRL0.u32);
   }
...
{
	//HI_PRINT("------4kP60--Set---%d\n",enVdpIntfMode);
	
	if(VDP_INTF_MODE_NON_4K60 == enVdpIntfMode)
	{
	  VDP_SetTwoChnEnable(HI_FALSE);
	  VDP_SetTwoChnMode(HI_FALSE);
	  VDP_SetTwoChnDbgEnable(HI_FALSE);
	}
	else if(VDP_INTF_MODE_4K60_ALL == enVdpIntfMode)
	{
	  VDP_SetTwoChnEnable(HI_TRUE);
	  VDP_SetTwoChnMode(HI_TRUE);
	  VDP_SetTwoChnDbgEnable(HI_TRUE);
	}
	else if(VDP_INTF_MODE_4K60_HDMI420 == enVdpIntfMode)
	{
	  VDP_SetTwoChnEnable(HI_TRUE);
	  VDP_SetTwoChnMode(HI_TRUE);
	  VDP_SetTwoChnDbgEnable(HI_FALSE);
...
{
    volatile U_V0_SRORESO V0_SRORESO;
   ...
   V0_SRORESO.u32 = VDP_RegRead((HI_U32)(&(pVdpReg->V0_SRORESO.u32) + u32Data * VID_OFFSET));
   V0_SRORESO.bits.ow = stRect.s32Width - 1;
   V0_SRORESO.bits.oh = stRect.s32Height - 1;
   VDP_RegWrite((HI_U32)(&(pVdpReg->V0_SRORESO.u32) + u32Data * VID_OFFSET), V0_SRORESO.u32);
