 ...
 { {0,   0,   0,   240,  18,  4,   720-16, 119+8, 19+8,     240,  19,  4,    124, 3,  0, 0, 0,  0},/* 480I(NTSC) */
  ...
  //考虑一般用户不会使用HDMI输出480I，所以不支持HDMI_480I输出，选择hmid=0
    ...
    {   {1,   1,   2,   900,  23,  3,  1440, 112, 48,     1,   1,  1,    32, 6,   1, 0,  0,  0}, /* 1440x900@60Hz@88.75MHz */
        DISP_CLOCK_SOURCE_HD0,
        {0x22000000, 0x2006163}, /* 1440x900@60Hz_RB@88.75MHz */
    ...
    {   {1,   1,   2,   900,  23,  3,  1600, 112, 48,    1,   1,  1,    32, 5,   1,  0,  0,  0}, /* 1600x900@60Hz_RB@97.750 MHz  */
            DISP_CLOCK_SOURCE_HD0,
            {0x22000000, 0x2006187}, /* 1600x900@60Hz_RB@97.750 MHz  */
...
HI_S32 Chip_Specific_WinHalSetAddr(HI_U32 u32LayerId, WIN_HAL_PARA_S *pstPara, HI_S32 s32exl)
{
    ...
    /*in hifone, compress and non-compress are the same.
     *Vertical crop is calculated by software. horizal crop is done by ip.
     */
    ...
    /*horizical crop is done by ip, v0/v3 is same, v1/v4 has multi-area mode.
      the whole layer face the final output.*/
...
{
    DISP_MEMSET((void*)pstVideoLayerStatus, 0, sizeof(VIDEO_LAYER_STATUS_S) * DEF_VIDEO_LAYER_MAX_NUMBER);

    pstVideoLayerStatus[0].u32WinNum  = 0;
    pstVideoLayerStatus[0].bMultiMode = HI_FALSE;

    pstVideoLayerStatus[1].u32WinNum  = 0;
    pstVideoLayerStatus[1].bMultiMode = HI_TRUE;

    pstVideoLayerStatus[3].u32WinNum  = 0;
    pstVideoLayerStatus[3].bMultiMode = HI_FALSE;

    pstVideoLayerStatus[4].u32WinNum  = 0;
    pstVideoLayerStatus[4].bMultiMode = HI_TRUE;
...
HI_S32 Chip_Specific_SetLayerCapability(VIDEO_LAYER_CAPABILITY_S *pstVideoLayerCap)
{
    ...
    pstVideoLayerCap[eId].u32BitWidthExternalBus = 10;
    ...
    pstVideoLayerCap[eId].u32BitWidth = 10;
    /*in hifone, v3/v4 does not support 10bit in bus interface.*/
    pstVideoLayerCap[eId].u32BitWidthExternalBus = 8;
