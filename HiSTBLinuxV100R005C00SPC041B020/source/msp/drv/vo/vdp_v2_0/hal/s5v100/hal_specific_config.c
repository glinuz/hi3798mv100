...
/* Synm Iop  Itf  Vact Vbb Vfb   Hact  Hbb Hfb      Bvact Bvbb Bvfb  Hpw Vpw Hmid bIdv bIhs bIvs */
 ...
 { {1,	 1,   2,  2160,  82,  8,  3840, 384, 1056,		1,	 1,  1,    88, 10, 1, 0,  0,  0}, 
   DISP_CLOCK_SOURCE_HD0,
   {0x12000000, 0x02002063}, /*  3840x2160@50Hz@594.000MHz  */
 ...
 { {1,	 1,   2,  2160,  82,  8,  3840, 384, 176,		1,	 1,  1,    88, 10, 1, 0,  0,  0}, 
   DISP_CLOCK_SOURCE_HD0,
   {0x12000000, 0x02002063}, /*  3840x2160@60Hz@594.000MHz  */
...
/* Synm Iop  Itf  Vact Vbb Vfb   Hact  Hbb Hfb      Bvact Bvbb Bvfb  Hpw Vpw Hmid bIdv bIhs bIvs */
 ...
 { {1,	 1,   2,  2160,  82,  8,  4096, 384, 1020,		 1,   1,  1,	88, 10, 1, 0,  0,  0},
   DISP_CLOCK_SOURCE_HD0,
   {0x12000000, 0x02002063}, /*  4096x2160@24Hz@297.000MHz	*/
 ...
 { {1,	 1,   2,  2160,  82,  8,  4096, 216, 968,		 1,   1,  1,	88, 10, 1, 0,  0,  0},
   DISP_CLOCK_SOURCE_HD0,
   {0x12000000, 0x02002063}, /*  4096x2160@25Hz@297.000MHz	*/
...
{
   // HI_PRINT("---layer:%d, bFirlterEnable:%d!\n", u32LayerId, bFirlterEnable);
...
HI_S32 Chip_Specific_SetMixerBgColor(HI_DRV_DISPLAY_E eChn, VDP_BKG_S stBkg)
{
    if (HI_DRV_DISPLAY_1 == eChn)
    {
        ...
        /*set cmb2 bgcolor, may fall into hal specific layer.*/
...
HI_S32 Chip_Specific_WinDoFurtherZmeProcess(HI_U32 u32LayerId, WIN_HAL_PARA_S *pstPara)
{
    ...
    /* 98c bind to vp, when 98m, fixed to V0, no choice.
     * so this function should fall into chip specific layer.
     */
    VDP_SetSrConnection(VDP_SR_CONN_VP0);
    if ((pstPara->bSecondHorZmeEnable) || (pstPara->bSecondVerZmeEnable))
    {
        if (pstPara->bSecondHorZmeEnable)
            DispRect.u32IWth = pstPara->pstDispInfo->stFmtResolution.s32Width /  2;
        else
            DispRect.u32IWth = pstPara->pstDispInfo->stFmtResolution.s32Width;

        if (pstPara->bSecondVerZmeEnable)
            DispRect.u32IHgt = pstPara->pstDispInfo->stFmtResolution.s32Height / 2;
        else
            DispRect.u32IHgt = pstPara->pstDispInfo->stFmtResolution.s32Height;

        VDP_VP_SetLayer_InReso(VDP_LAYER_VP0, DispRect);
        u32Width = DispRect.u32IWth;
        u32Height = DispRect.u32IHgt;


        DISP_MEMSET(&DispRect, 0, sizeof(VDP_DISP_RECT_S));
        DispRect.u32DXL  = pstPara->pstDispInfo->stFmtResolution.s32Width;
        DispRect.u32DYL  = pstPara->pstDispInfo->stFmtResolution.s32Height;
        DispRect.u32OWth = pstPara->pstDispInfo->stFmtResolution.s32Width;
        DispRect.u32OHgt = pstPara->pstDispInfo->stFmtResolution.s32Height;
        VDP_VP_SetLayer_OutReso(VDP_LAYER_VP0,DispRect);
        VDP_DISP_SetSrEnable(0, HI_TRUE);
        ...
        */
        VDP_DISP_SetSrServeMode (0, VDP_SR_1IN_1OUT);

        stZmeI.u32ZmeFrmWIn = u32Width;
        stZmeI.u32ZmeFrmHIn = u32Height;

        stZmeI.u32ZmeFrmWOut = pstPara->pstDispInfo->stFmtResolution.s32Width;
        stZmeI.u32ZmeFrmHOut = pstPara->pstDispInfo->stFmtResolution.s32Height;
    ...
    else
    {
        ...
        DispRect.u32VXL  = pstPara->pstDispInfo->stFmtResolution.s32Width;
        DispRect.u32VYL  = pstPara->pstDispInfo->stFmtResolution.s32Height;

        u32Width = DispRect.u32IWth;
        u32Height = DispRect.u32IHgt;

        VDP_DISP_SetSrEnable(0, HI_FALSE);

        /*only layer 0 and 1 has VP,so only layer 3/4  cant operate VP.*/
        if (u32LayerId == VDP_LAYER_VID0)
...
HI_S32 Chip_Specific_SetLayerCapability(VIDEO_LAYER_CAPABILITY_S *pstVideoLayerCap)
{
    ...
    pstVideoLayerCap[eId].u32LayerWidthMax   = 4096;
    pstVideoLayerCap[eId].u32LayerHeightMax  = 2160;

    /*add sr dci information.*/
    pstVideoLayerCap[eId].stLayerProcInfo.eSrLocation = ALG_LOCATION_IN_VP0;
    pstVideoLayerCap[eId].stLayerProcInfo.eDciLocation = ALG_LOCATION_IN_VP0;
    pstVideoLayerCap[eId].stLayerProcInfo.bSrBehindDci = HI_TRUE;
...
HI_VOID  Chip_Specific_DispSetPll(DISP_PLL_SOURCE_E enPllIndex,HI_U32 u32PllxReg0,HI_U32 u32PllxReg1)
{
    if (DISP_CLOCK_SOURCE_HD0 == enPllIndex)
    {
        ...
        //msleep(5);
        //g_pstRegCrg->PERI_CRG116.u32 = 0x101;
        // msleep(5);
        //g_pstRegCrg->PERI_CRG122.u32 = 0;

#else

        g_pstRegCrg->PERI_CRG_PLL11.u32 = 0x3002031;
        msleep(5);
        g_pstRegCrg->PERI_CRG_PLL10.u32 = 0x24800000;
        msleep(5);
        g_pstRegCrg->PERI_CRG_PLL11.u32 = 0x2002031;
        msleep(5);
        g_pstRegCrg->PERI_CRG116.u32 = 0x1;
         msleep(5);
        g_pstRegCrg->PERI_CRG122.u32 = 0x8;
#endif
    }
    else  if (DISP_CLOCK_SOURCE_HD1 == enPllIndex)
    {
        U_PERI_CRG_PLL16 PERI_CRG16Tmp;
        U_PERI_CRG_PLL17 PERI_CRG17Tmp;

        PERI_CRG16Tmp.u32 = g_pstRegCrg->PERI_CRG_PLL16.u32;
        PERI_CRG16Tmp.bits.qpll_ctrl0 = u32PllxReg0;
        g_pstRegCrg->PERI_CRG_PLL16.u32 = PERI_CRG16Tmp.u32;

        PERI_CRG17Tmp.u32 = g_pstRegCrg->PERI_CRG_PLL17.u32;
        PERI_CRG17Tmp.bits.qpll_ctrl1 = u32PllxReg1;
        g_pstRegCrg->PERI_CRG_PLL17.u32 = PERI_CRG17Tmp.u32;
    }
...
HI_VOID Chip_Specific_CBM_GetMixvPrio(VDP_CBM_MIX_E enMixer, HI_U32 u32prio, HI_U32 *pu32layer_id)
{
    *pu32layer_id = VDP_LAYER_VID_BUTT;
    ...
    {
        *pu32layer_id = VDP_LAYER_VID_BUTT;
        HI_PRINT("Error, %s: Set mixer  select wrong layer ID\n", __func__);
    }
