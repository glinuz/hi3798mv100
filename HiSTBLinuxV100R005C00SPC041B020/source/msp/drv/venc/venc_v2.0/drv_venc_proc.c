	    ...
	    if (pEncPara->WithoutVPSS)  
	    {
	         if (!g_stVencChn[u32ChnID].bEnable)  /*if the channel not start! OK*/
	         {
				  ...
				  if (g_stVencChn[u32ChnID].hPort[0] == HI_INVALID_HANDLE)
				  {
				        ...
				        stPort0Cfg.s32OutputWidth            = pEncPara->stRc.PicWidth;
				        stPort0Cfg.s32OutputHeight           = pEncPara->stRc.PicHeight;
				        ...
				        stPort0Cfg.stBufListCfg.u32BufSize   = 2*(pEncPara->stRc.PicWidth * pEncPara->stRc.PicHeight);
				        stPort0Cfg.stBufListCfg.u32BufStride = (pEncPara->stRc.PicWidth + (15)) & ~(15);           //16对齐   
