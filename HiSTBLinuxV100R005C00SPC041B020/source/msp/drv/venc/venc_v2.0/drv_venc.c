...
#if (defined(__VENC_S40V200_CONFIG__) || defined(__VENC_S5V100_CONFIG__) || defined(__VENC_98CV200_CONFIG__) || defined(__VENC_98MV200_CONFIG__) )    //s5v100 & s40v200 & 98cv200 have no vedu_chip_id limit!
    g_vedu_chip_id = 2;
                              ...
                              VENC_BUF_OFFSET_S* pstBufOffSet)
{
    ...
    else
    {
        ...
        {
            pstStream->enDataType.enHEVCEType = (stVeduPacket.NaluType == H265_NALU_TYPE_IDR) ? 19 : stVeduPacket.NaluType;
        }
        else
        {
...
HI_S32 VENC_DRV_ReleaseStream(VeduEfl_EncPara_S* hVencChn, VENC_INFO_ACQUIRE_STREAM_S* pstAcqStrm)
{
    ...
    else
    {
        ...
        if (pstEncChnPara->Protocol == VEDU_H265)
        {
            stVeduPacket.NaluType = pstStream.enDataType.enHEVCEType;
        }
        else
        {
...
static HI_VOID VENC_DRV_SetAttr_Error(HI_UNF_VENC_CHN_ATTR_S *pstChnUserCfg, HI_UNF_VENC_CHN_ATTR_S* pstAttr, HI_BOOL Enable)
{
    ...
    {
        HI_ERR_VENC("98mv200 VENC not support this active change encode type:(%d -> %d)!\n",
...
HI_S32 HI_DRV_VENC_SetSrcInfo(HI_HANDLE hVenc, HI_DRV_VENC_SRC_HANDLE_S *pstSrcInfo)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32ChnID;
    VeduEfl_EncPara_S* hVencChn;
