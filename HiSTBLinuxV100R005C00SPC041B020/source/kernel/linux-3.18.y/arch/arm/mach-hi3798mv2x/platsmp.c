...
static int __cpuinit hi3798mv2x_boot_secondary(unsigned int cpu,
					struct task_struct *idle)
{
	unsigned long timeout;

	set_scu_boot_addr(0xFFFF0000,
		(unsigned int)virt_to_phys(hi3798mv2x_secondary_startup));
...
static void __cpuinit hi3798mv2x_secondary_init(unsigned int cpu)
{
	/*
...
static void __init hi3798mv2x_smp_init_cpus(void)
{
...
void slave_cores_power_up(int cpu)
{
	...
	regval = readl(cpu_rst);
	regval &= ~(1 << (cpu + 12));
	writel(regval, cpu_rst);
...
struct smp_operations hi3798mv2x_smp_ops __initdata = {
	.smp_init_cpus = hi3798mv2x_smp_init_cpus,
	.smp_secondary_init = hi3798mv2x_secondary_init,
	.smp_boot_secondary = hi3798mv2x_boot_secondary,
#ifdef CONFIG_HOTPLUG_CPU
	.cpu_die = hi3798mv2x_cpu_die,
#endif
};
