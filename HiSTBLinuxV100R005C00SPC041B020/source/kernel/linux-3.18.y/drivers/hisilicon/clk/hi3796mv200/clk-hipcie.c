...
static int hiclk_enable_pcie(struct clk_hw *hw)
{
	...
	writel(0x10,IO_ADDRESS(0xf8b22400));//Set GPIO2_3 output
	udelay(20);
	writel(0x10,IO_ADDRESS(0xf8b22020));//GPIO2_3 output High
	udelay(20);
#endif
	...
	/* SSC on */
//	nano_register_write(clk, PERI_COMBPHY0_CFG, 0x2, 0x8);

	/* seperate_rate 1'b1 */
	nano_register_write(clk, PERI_COMBPHY0_CFG, 0x1, 0x8);

	/* SPLIT_CP_DIS */
	nano_register_write(clk, PERI_COMBPHY0_CFG, 0xc, 0x9);

	/* PLL 3rd filter bypass */
	nano_register_write(clk, PERI_COMBPHY0_CFG, 0x1a, 0x4);
