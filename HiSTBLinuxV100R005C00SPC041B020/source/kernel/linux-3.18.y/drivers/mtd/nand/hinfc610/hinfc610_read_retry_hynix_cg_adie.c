...
static int hinfc610_hynix_cg_adie_get_rr_param(void *host)
{
	char *otp;
	...
	writel(0x40, hinfc_host->chip->IO_ADDR_R); /* data: 0x00 */
	hinfc_write(hinfc_host, 0xFF, HINFC610_ADDRL);/* address: 0xAE */
	hinfc_write(hinfc_host, 0x36, HINFC610_CMD);  /* cmd: 0x36 */
	/*
	 * no need to config HINFC610_OP_WAIT_READY_EN, here not config this bit.
	 */
	hinfc_write(hinfc_host, HINFC610_WRITE_1CMD_1ADD_DATA(hinfc_host), 
		    HINFC610_OP);
	...
	hinfc_write(hinfc_host, 0xCC, HINFC610_ADDRL);/* address: 0xB0 */
	/*
	 * no need to config HINFC610_OP_WAIT_READY_EN, here not config this bit.
	 ...
	 */
	hinfc_write(hinfc_host, HINFC610_READ_2CMD_5ADD(hinfc_host), HINFC610_OP);
	WAIT_CONTROLLER_FINISH(hinfc_host);

	/*step6 save otp read retry table to mem*/
	otp = hinfc610_hynix_cg_adie_otp_check(hinfc_host->chip->IO_ADDR_R + 2);
	if (!otp) {
	 ...
	 * need to config HINFC610_OP_WAIT_READY_EN, here config this bit.
	 */
	hinfc_write(hinfc_host, HINFC610_WRITE_1CMD_0ADD_NODATA_WAIT_READY(hinfc_host), 
		    HINFC610_OP);
	WAIT_CONTROLLER_FINISH(hinfc_host);
...
static int hinfc610_hynix_cg_adie_set_rr_param(void *host, int param)
{
	unsigned char *rr;
	struct hinfc_host *hinfc_host = (struct hinfc_host *)host;
	...
	return hinfc610_hynix_cg_adie_set_rr_reg(hinfc_host, (char *)rr);
}
/*****************************************************************************/

static int hinfc610_hynix_cg_adie_reset_rr_param(void *host)
{
