...
HI_S32 HI_TEE_CIPHER_Init(HI_VOID)
{
    ...
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_INIT, HI_NULL);
}

HI_S32 HI_TEE_CIPHER_CreateHandle(HI_HANDLE* phCipher, HI_TEE_CIPHER_ATTS_S *pstCipherAttr)
{
    CIPHER_HANDLE_S stCIHandle;

    stCIHandle.pstCipherAtts = pstCipherAttr;
    stCIHandle.phCIHandle = phCipher;
    
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_CREATEHANDLE, &stCIHandle);
}

HI_S32 HI_TEE_CIPHER_DestroyHandle(HI_HANDLE hCipher)
{
    return  __TEE_CIPHER_Ioctl(CMD_CIPHER_DESTROYHANDLE, &hCipher);
}
...
HI_S32 HI_TEE_CIPHER_ConfigHandle(HI_HANDLE hCipher, HI_TEE_CIPHER_CTRL_S* pstCtrl)
{
    ...
    stCIConfig.CIHandle = hCipher;
    stCIConfig.pstCIpstCtrl = pstCtrl; 
    
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_CONFIGHANDLE, &stCIConfig);
}

HI_S32 HI_TEE_CIPHER_GetHandleConfig(HI_HANDLE hCipher, HI_TEE_CIPHER_CTRL_S* pstCtrl)
{
    CIPHER_Config_CTRL stCIData;
    
    stCIData.CIHandle = hCipher;
    stCIData.pstCIpstCtrl = pstCtrl;
    
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_GETHANDLECONFIG, &stCIData);
}
...
HI_S32 HI_TEE_CIPHER_Encrypt(HI_HANDLE hCipher, HI_U32 u32SrcPhyAddr, HI_U32 u32DestPhyAddr, HI_U32 u32ByteLength)
{
    ...
    while(u32ByteLength > 0)
    {
        ...
        stCIData.ScrPhyAddr=u32SrcPhyAddr+u32Offset;
        stCIData.DestPhyAddr=u32DestPhyAddr+u32Offset;
        stCIData.u32DataLength=u32TempLen;
        ret = __TEE_CIPHER_Ioctl(CMD_CIPHER_ENCRYPT, &stCIData);
        if(ret != HI_SUCCESS)
        {
...
HI_S32 HI_TEE_CIPHER_Decrypt(HI_HANDLE hCipher, HI_U32 u32SrcPhyAddr, HI_U32 u32DestPhyAddr, HI_U32 u32ByteLength)
{
    ...
    while(u32ByteLength > 0)
    {
        ...
        stCIData.DestPhyAddr=u32DestPhyAddr+u32Offset;
        stCIData.u32DataLength=u32TempLen;
        ret = __TEE_CIPHER_Ioctl(CMD_CIPHER_DECRYPT, &stCIData);
        if(ret != HI_SUCCESS)
        {
...
HI_S32 HI_TEE_CIPHER_EncryptUnblock(HI_HANDLE hCipher, HI_U32 u32SrcPhyAddr, HI_U32 u32DestPhyAddr, HI_U32 u32ByteLength)
{
    HI_S32 ret = HI_SUCCESS;
    HI_U32 u32Offset = 0;
    HI_U32 u32TempLen = 0;
    ...
    while(u32ByteLength > 0)
    {
        ...
        stCIData.DestPhyAddr=u32DestPhyAddr+u32Offset;
        stCIData.u32DataLength=u32TempLen;
        ret = __TEE_CIPHER_Ioctl(CMD_CIPHER_ENCRYPT_UNBLOCK, &stCIData);
        if(ret != HI_SUCCESS)
        {
            HI_ERR_CIPHER("CIPHER_ENCRYPT_UNBLOCK failure, ret=0x%x\n", ret);
            return HI_FAILURE;
        }
...
HI_S32 HI_TEE_CIPHER_DecryptUnblock(HI_HANDLE hCipher, HI_U32 u32SrcPhyAddr, HI_U32 u32DestPhyAddr, HI_U32 u32ByteLength)
{
    HI_S32 ret = HI_FAILURE;
    HI_U32 u32Offset = 0;
    HI_U32 u32TempLen = 0;
    ...
    while(u32ByteLength > 0)
    {
        ...
        stCIData.DestPhyAddr=u32DestPhyAddr+u32Offset;
        stCIData.u32DataLength=u32TempLen;
        ret = __TEE_CIPHER_Ioctl(CMD_CIPHER_DECRYPT_UNBLOCK, &stCIData);
        if(ret != HI_SUCCESS)
        {
            HI_ERR_CIPHER("CIPHER_DECRYPT_UNBLOCK failure, ret=0x%x\n", ret);
            return HI_FAILURE;
        }
...
HI_S32 HI_TEE_CIPHER_GetInfo(HI_HANDLE hCipher, HI_TEE_CIPHER_INFO_S *pstInfo)
{
    CIPHER_INFO_S stInfo;

    stInfo.CIHandle = hCipher;
    stInfo.pstInfo = pstInfo;
    
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_GET_INFO, &stInfo);
}
...
HI_S32 HI_TEE_CIPHER_EncryptMulti(HI_HANDLE hCipher, HI_TEE_CIPHER_DATA_S *pstDataPkg, HI_U32 u32DataPkgNum)
{
    ...
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_ENCRYPTMULTI, &stPkg);
}
...
HI_S32 HI_TEE_CIPHER_DecryptMulti(HI_HANDLE hCipher, HI_TEE_CIPHER_DATA_S *pstDataPkg, HI_U32 u32DataPkgNum)
{
    ...
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_DECRYPTMULTI, &stPkg);
}

HI_S32 HI_TEE_CIPHER_GetRandomNumber(HI_U32 *pu32Rnd)
{
    CIPHER_RNG_S stRNG;

    stRNG.u32TimeOutUs = -1;
    stRNG.pu32RNG = pu32Rnd;
    
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_GETRANDOMNUMBER, &stRNG);
}
...
HI_S32 HI_TEE_CIPHER_DeInit(HI_VOID)
{
    ...
    __TEE_CIPHER_Ioctl(CMD_CIPHER_DEINIT, HI_NULL);
    s_CipherDevFd = -1;
...
HI_S32 HI_TEE_CIPHER_HashInit(HI_TEE_CIPHER_HASH_ATTS_S *pstHashAttr, HI_HANDLE *pHashHandle)
{
    ...
    stHashInit.pstHashAttr = pstHashAttr;
    stHashInit.pHashHandle = pHashHandle;
    
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_CALCHASHINIT, &stHashInit);
}
...
HI_S32 HI_TEE_CIPHER_HashUpdate(HI_HANDLE hHashHandle, HI_U8 *pu8InputData, HI_U32 u32InputDataLen)
{
    ...
    stHashUpdate.HashHandle = hHashHandle;
    stHashUpdate.pu8InputData = pu8InputData;
    stHashUpdate.u32InputDataLen = u32InputDataLen;
    
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_CALCHASHUPDATE, &stHashUpdate);
}
...
HI_S32 HI_TEE_CIPHER_HashFinal(HI_HANDLE hHashHandle, HI_U8 *pu8OutputHash)
{
    ...
    stHashFinish.HashHandle = hHashHandle;
    stHashFinish.pu8OutputHash = pu8OutputHash;
    
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_CALCHASHFINAL, &stHashFinish);
}

HI_S32 HI_TEE_CIPHER_Test(HI_U32 u32Cmd, HI_VOID *pu8Param, HI_U32 u32ParamSize)
{
     CIPHER_TEST_DATA_S stCipherTest;

     stCipherTest.u32Cmd = u32Cmd;
     stCipherTest.pu8Param = pu8Param;
     stCipherTest.u32ParamSize = u32ParamSize;
     
     return __TEE_CIPHER_Ioctl(CMD_CIPHER_TEST, &stCipherTest);
}

HI_S32 HI_TEE_CIPHER_PBKDF2(HI_TEE_CIPHER_PBKDF2_S *pstInfo, HI_U8 *pu8Output)
{
    CIPHER_PBKDF2_DATA_S stCipherPbdf2;

    stCipherPbdf2.pstInfo = pstInfo;
    stCipherPbdf2.pu8Output = pu8Output;
    
    return __TEE_CIPHER_Ioctl(CMD_CIPHER_PBKDF2, &stCipherPbdf2);
}
...
static HI_S32 RSA_GetAttr(HI_U32 u32SchEme, 
                          ...
                          HI_U32 *pu32HLen,
                          HI_U32 *pu32KeyLen,
                          HI_TEE_CIPHER_HASH_TYPE_E *pstenHashType)
{
    HI_S32 ret = HI_SUCCESS;
    ...
    *pu32HLen = 0;
    *pstenHashType = HI_TEE_CIPHER_HASH_TYPE_BUTT;
    
    switch(u32SchEme)
    {
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_NO_PADDING:
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_0:
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_1:         
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_2:
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_RSAES_PKCS1_V1_5:
        *pu32HLen = 0;
        *pstenHashType = HI_TEE_CIPHER_HASH_TYPE_BUTT;
        break;
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_RSAES_OAEP_SHA1:     
    case HI_TEE_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_V15_SHA1:
    case HI_TEE_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_PSS_SHA1:
        *pu32HLen = 20;
        *pstenHashType = HI_TEE_CIPHER_HASH_TYPE_SHA1;
        break;
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_RSAES_OAEP_SHA256:
    case HI_TEE_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_V15_SHA256:
    case HI_TEE_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_PSS_SHA256:
        *pu32HLen = 32;
        *pstenHashType = HI_TEE_CIPHER_HASH_TYPE_SHA256;
        break;
    default:
...
static HI_S32 RSA_PKCS1_MGF1(HI_TEE_CIPHER_HASH_TYPE_E enHashType,
                             HI_U8  *pu8Seed,  
                             HI_U32 u32Seedlen, 
                             HI_U8 *pu8Mask, HI_U32 u32MaskLen)
{
    ...
    if( enHashType == HI_TEE_CIPHER_HASH_TYPE_SHA1)
    {
        u32MdLen = 20;
    }
    else if( enHashType == HI_TEE_CIPHER_HASH_TYPE_SHA256)
    {
        u32MdLen = 32;
    }
    else
    ...
    for (i = 0; u32Outlen < u32MaskLen; i++) 
    {
        ...
        Ret = HI_TEE_CIPHER_HashInit(&stHashAttr, &HashHandle);
        if (Ret != HI_SUCCESS)
        {
    		return Ret;
        }
        Ret = HI_TEE_CIPHER_HashUpdate(HashHandle, pu8Seed, u32Seedlen);
        if (Ret != HI_SUCCESS)
        {
    		return Ret;
        }
        Ret = HI_TEE_CIPHER_HashUpdate(HashHandle, u8Cnt, 4);
        if (Ret != HI_SUCCESS)
        {
    		return Ret;
        }
        Ret = HI_TEE_CIPHER_HashFinal(HashHandle, u8Md);
        if (Ret != HI_SUCCESS)
        {
    		return Ret;
        }
...
static HI_S32 RSA_GetRandomNumber(HI_U8 *pu8Rand, HI_U32 u32Size)
{
    ...
    {
        ret = HI_TEE_CIPHER_GetRandomNumber(&u32Rand);
        if (ret != HI_SUCCESS)
        {
...
static HI_U32 HI_TEE_CIPHER_GetBitNum(HI_U8 *pu8BigNum, HI_U32 u32NumLen)
{
...
so: mlen < k - 2hlen - 2 
*************************************************************/
static HI_S32 RSA_padding_add_PKCS1_OAEP(HI_TEE_CIPHER_HASH_TYPE_E enHashType,
                                  HI_U32 u32HLen,
                                  HI_U32 u32KeyLen,
                                  HI_U8 *pu8Input,
                                  HI_U32 u32InLen,
                                  ...
                                  HI_U32 *pu32OutLen)
{
    ...
    if( enHashType == HI_TEE_CIPHER_HASH_TYPE_SHA256)
    {   
...
**********************************************************/
static HI_S32 RSA_padding_add_EMSA_PKCS1_v15(HI_TEE_CIPHER_HASH_TYPE_E enHashType,
                                        HI_U32 u32HLen,
                                        HI_U32 u32KeyLen,
                                        HI_U8 *pu8Input,
                                        HI_U32 u32InLen,
                                        ...
                                        HI_U32 *pu32OutLen)
{
    ...
    *p++ = RSA_SIGN;
    switch(enHashType)
    {
    case HI_TEE_CIPHER_HASH_TYPE_SHA1:
        ...
        *p++ = 0;
        ...
        break;
    case HI_TEE_CIPHER_HASH_TYPE_SHA256:
...
/******************************************************************
	...
	EM =    |    maskedDB       |  maskedseed |bc|
			+-------------------+-------------+--+
 ******************************************************************/
static HI_S32 RSA_padding_add_PKCS1_PSS(HI_TEE_CIPHER_HASH_TYPE_E enHashType,
                                        HI_U32 u32HLen,
                                        HI_U32 u32EmBit, 
							            HI_U8 *pu8Input,
							            HI_U32 u32InLen,
                                        ...
                                        HI_U32 *pu32OutLen)
{
    ...
    *pu32OutLen = u32KeyLen;
    ...
    ret = HI_TEE_CIPHER_HashUpdate(HashHandle, pu8M, u32Index);
    if (ret != HI_SUCCESS)
    {
    ...
    ret = HI_TEE_CIPHER_HashFinal(HashHandle, pu8Maskedseed);
    if (ret != HI_SUCCESS)
    {
								  ...
								  HI_U8 *pu8Output, HI_U32 *pu32OutLen)
{
    ...
    HI_U32 u32KeyLen;
    HI_U8  u8BT;
    HI_TEE_CIPHER_HASH_TYPE_E enHashType;
    ...
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_1:
        ...
        break;
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_2:
        u8BT = (HI_U8)(pstRsaEnc->enScheme - HI_TEE_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_0);
    ...
    ret = __TEE_CIPHER_Ioctl(CMD_CIPHER_CALCRSA, &stRsaData);

    return ret;
}

static HI_S32 RSA_padding_check_PKCS1_OAEP(HI_TEE_CIPHER_HASH_TYPE_E enHashType,
                                  HI_U32 u32HLen,
                                  HI_U32 u32KeyLen,
                                  HI_U8 *pu8Input, 
                                  HI_U32 u32InLen,
...
static HI_S32 RSA_padding_check_EMSA_PKCS1_v15(HI_TEE_CIPHER_HASH_TYPE_E enHashType,
                                        HI_U32 u32HLen,
                                        HI_U32 u32KeyLen,
                                        HI_U8 *pu8Input,
                                        HI_U32 u32InLen,
...
static HI_U32 RSA_padding_check_PKCS1_PSS(HI_TEE_CIPHER_HASH_TYPE_E enHashType,
                                          HI_U32 u32HLen,
                                          HI_U32 u32EmBit, 
							              HI_U8 *pu8Input,
							              HI_U32 u32InLen,
							              HI_U8 *pu8MHash)
{
    ...
    ret = HI_TEE_CIPHER_HashFinal(HashHandle, u8H);
    if (ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("HashFinal failed\n");
        free(pu8M);
								   ...
								   HI_U8 *pu8Output, HI_U32 *pu32OutLen)
{
    ...
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_1:
        ...
        break;
    case HI_TEE_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_2:
        u8BT = (HI_U8)(pstRsaDec->enScheme - HI_TEE_CIPHER_RSA_ENC_SCHEME_BLOCK_TYPE_0);
							 ...
							 HI_U8 *pu8OutSign, HI_U32 *pu32OutSignLen)
{
    ...
    if (pu8HashData == HI_NULL)
    {
        ...
        if (ret != HI_SUCCESS)
        {
            HI_ERR_CIPHER("HI_TEE_CIPHER_HashInit error\n");
            return ret;
        }
        ret = HI_TEE_CIPHER_HashUpdate(HashHandle, pu8InData, u32InDataLen);
        if (ret != HI_SUCCESS)
        {
            HI_ERR_CIPHER("HI_TEE_CIPHER_HashUpdate error\n");
            return ret;
        }
        ret = HI_TEE_CIPHER_HashFinal(HashHandle, u8Hash);
        if (ret != HI_SUCCESS)
        {
            HI_ERR_CIPHER("HI_TEE_CIPHER_HashUpdate error\n");
            return ret;
    ...
    case HI_TEE_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_PSS_SHA1:
    case HI_TEE_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_PSS_SHA256:
        u32EmBit = HI_TEE_CIPHER_GetBitNum(pstRsaSign->stPriKey.pu8N, u32KeyLen);
							   ...
							   HI_U8 *pu8InSign, HI_U32 u32InSignLen)
{
    ...
    case HI_TEE_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_PSS_SHA1:
    case HI_TEE_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_PSS_SHA256:
        u32EmBit = HI_TEE_CIPHER_GetBitNum(pstRsaVerify->stPubKey.pu8N, u32KeyLen);
