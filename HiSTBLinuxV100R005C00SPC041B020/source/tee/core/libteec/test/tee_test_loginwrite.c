...
int login_write_01()
{
	...
	fd1 = TEEC_FOpen("/data/01.txt", TEE_DATA_FLAG_ACCESS_WRITE);
	if (fd1 != -1) {
		TEEC_Error("%s: TEEC_FOpen failed, errno=0x%x\n", __FUNCTION__, TEEC_GetFErr());
		res = 1;
		//goto error;
	}
	TEEC_Error("test: fd1=%d\n", fd1);
	...
	//test for share_flag
	fd = TEEC_FOpen("/data/01.txt", TEE_DATA_FLAG_ACCESS_WRITE | TEE_DATA_FLAG_CREATE | TEE_DATA_FLAG_SHARE_WRITE);
	if (fd == -1) {
		TEEC_Error("%s: TEEC_FOpen failed, errno=0x%x\n", __FUNCTION__, TEEC_GetFErr());
		res = 1;
		goto error;
	...
	fd1 = TEEC_FOpen("/data/01.txt", TEE_DATA_FLAG_ACCESS_WRITE | TEE_DATA_FLAG_SHARE_WRITE);
	if (fd1 == -1) {
		TEEC_Error("%s: TEEC_FOpen failed, errno=0x%x\n", __FUNCTION__, TEEC_GetFErr());
		res = 1;
		goto error;
	...
	ret = TEEC_FClose(fd);
	if (ret == -1) {
		TEEC_Error("%s: TEEC_FClose0 failed\n");
		res = 1;
		goto error;
	}
	...
	ret = TEEC_FClose(fd1);
	if (ret == -1) {
		TEEC_Error("%s: TEEC_FClose1 failed\n");
		res = 1;
		goto error;
	}
...
int main()
{
	...
	if (test_write_read_05_times() != 0) {
		printf("test test_write_read_05_times failed\n");
		while (1);
	} else {
	...
	if (tee_not_close() != 0) {
		printf("test tee_not_close failed\n");
		while (1);
	} else {
		printf("test tee_not_close success\n");
	}
#endif
