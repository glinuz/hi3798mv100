...
typedef struct hiDRV_MEM_S
{
    ...
    MEM_RECORD_S  stVdmHalMem_Sec[MAX_VDH_NUM];      /* VDM HAL Sec 内存 */	
...
typedef struct hiVFMW_CHAN_MEM_S
{
    ...
    MEM_RECORD_S   stChanMem_BPD_SegID;    /* 该通道的 bpd segid 存储资源 */ 
...
//    SINT32         s32CreatWithMem;     /* 1:该通道内存资源由外部申请、外部释放 */
    ...
    MEM_RECORD_S   stChanMem_BPD_SegID;    /* 该通道的 bdp segid 存储资源 */ 	
