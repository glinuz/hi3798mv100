/******************************************************************************

Copyright (C), 2005-2014, Hisilicon Tech. Co., Ltd.
******************************************************************************
File Name     : cx_cfg.c
Version       : Initial
Author        : Hisilicon hisecurity group
Created       : 2013-08-28
Last Modified :
Description   :  
Function List :
History       :
******************************************************************************/
/*this file should be configured by customer*/
#include "hi_type.h"
#ifdef HI_MINIBOOT_SUPPORT
#include "app.h"
#else
#include "exports.h"
#endif
#include "hi_unf_cipher.h"
#include "cx_verify_def.h"
#include "cx_adp.h"
#include "cx_debug.h"

/*nand and SPI max flash block size*/
HI_U32 g_MaxBlockSize =  0x00100000;//(1M)

#define KEY_LEN 16

/*Address of bootenv parameters in flash*/

#ifdef HI_MINIBOOT_SUPPORT
#ifdef CONFIG_ENV_ADDR     //it's defined in source/boot/Makefile.miniboot , value comes from  CFG_HI_BOOT_ENV_STARTADDR defined in cfg.mak
HI_U32 g_EnvFlashAddr = CONFIG_ENV_ADDR;
#else
HI_U32 g_EnvFlashAddr = 0x000A0000;      //640KB
#endif

#else

#ifdef CFG_ENV_ADDR     //it's defined in source/boot/Makefile.fastboot , value comes from  CFG_HI_BOOT_ENV_STARTADDR defined in cfg.mak
HI_U32 g_EnvFlashAddr = CFG_ENV_ADDR;    
#else
HI_U32 g_EnvFlashAddr = 0x000A0000;      //640KB
#endif

#endif

/*Size of bootenv parameters in flash*/

#ifdef HI_MINIBOOT_SUPPORT

#ifdef CONFIG_ENV_RANGE    //it's it's defined in source/boot/Makefile.miniboot, value comes from  CFG_HI_BOOT_ENV_STARTADDR defined in cfg.mak
HI_U32 g_EnvFlashPartionSize =  CONFIG_ENV_RANGE;
#else
HI_U32 g_EnvFlashPartionSize =  0x00020000; //128K
#endif

#else

#ifdef CFG_ENV_RANGE    //it's defined in source/boot/Makefile.fastboot , value comes from  CFG_HI_BOOT_ENV_RANGE defined in cfg.mak
HI_U32 g_EnvFlashPartionSize =  CFG_ENV_RANGE;   
#else
HI_U32 g_EnvFlashPartionSize =  0x00020000; //128K
#endif

#endif



/*Address of bak bootenv parameters in flash*/
HI_U32 g_EnvBakFlashAddr = 0x00200000;

/*At preset, the value is configured to the fixed size:256M. This is not good. We must optimize the issue later*/
HI_U32 g_DDRSize =  0x10000000;					//256M

/*configure for img encrypt and decrypt*/


static HI_U8 s_cx_customer_rsa_public_key_N[256]={
            , 
	};

static HI_U8 s_cx_customer_rsa_public_key_E_3[256] = {
            
            };

static HI_U8 G1_CIPHERKEY[16]      = {0x5b,0x45,0x14,0x37,0x6d,0xed,0x1d,0x08,0x64,0x97,0xbc,0xba,0xe6,0x7f,0x90,0x78};
static HI_U8 G2_CIPHERKEY[16]      = {0xd6,0xbc,0x9e,0xe2,0x3a,0x61,0x50,0x10,0xb4,0x64,0x2e,0xaa,0xe8,0xf0,0x53,0xa6};
static HI_U8 G1_KEYLADDER_KEY1[16] = {0x1c,0x21,0x42,0xa2,0x9e,0x31,0x57,0x8d,0xff,0xa4,0xe1,0xdf,0x2e,0x36,0xa0,0x6e};
static HI_U8 G1_KEYLADDER_KEY2[16] = {0x7c,0x76,0x1a,0x38,0xc1,0x46,0x4e,0x53,0x8c,0xa3,0xd7,0xae,0x56,0xee,0x27,0xb7};
static HI_U8 G2_KEYLADDER_KEY1[16] = {0x41,0x98,0x30,0x77,0x14,0xed,0x9d,0xc3,0x60,0x55,0x49,0xcf,0x5e,0x75,0x13,0xa2};
static HI_U8 G2_KEYLADDER_KEY2[16] = {0x5c,0x98,0xbe,0x7c,0x52,0x24,0x35,0xb8,0x30,0xa5,0x10,0x54,0x33,0xc3,0x21,0xf8};


/*BLPK for encrypting bootloader*/
static HI_U8  s_cx_customer_blpk[16] = {0x67,0xb1,0xf8,0x28,0xdd,0x93,0x41,0xbc,0xca,0xed,0x73,0xd4,0x8d,0xa8,0xe9,0x1f};



HI_S32 HI_CX_GetKeyladderKey(HI_CA_KEY_GROUP_E enKeyGroup, HI_U8 u8KeyladderKey1[16], HI_U8 u8KeyladderKey2[16], HI_U8 u8CipherKey[16])
{
    HI_U8 *pu8KeyLadderkey1, *pu8KeyLadderkey2, *pu8CipherKey;
    
    if (enKeyGroup == HI_CA_KEY_GROUP_1)
    {
       pu8KeyLadderkey1 = G1_KEYLADDER_KEY1;
       pu8KeyLadderkey2 = G1_KEYLADDER_KEY2;
       pu8CipherKey = G1_CIPHERKEY;
    }
    else if (enKeyGroup == HI_CA_KEY_GROUP_2)
    {
       pu8KeyLadderkey1 = G2_KEYLADDER_KEY1;
       pu8KeyLadderkey2 = G2_KEYLADDER_KEY2;
       pu8CipherKey = G2_CIPHERKEY;
    }
    else
    {
       return HI_FAILURE;
    }
    
    if (u8KeyladderKey1 != HI_NULL)
    {
        memcpy(u8KeyladderKey1, pu8KeyLadderkey1, KEY_LEN);
    }
    
    if (u8KeyladderKey2 != HI_NULL)
    {
        memcpy(u8KeyladderKey2, pu8KeyLadderkey2, KEY_LEN);
    }
    
    if (u8CipherKey != HI_NULL)
    {
        memcpy(u8CipherKey, pu8CipherKey, KEY_LEN);
    }    

    return HI_SUCCESS;
}

HI_S32 HI_CX_GetBLPKInfor(HI_U8 u8BLPK[16])
{
    if (u8BLPK == HI_NULL)
    {
       return HI_FAILURE;
    }    

    memcpy(u8BLPK, s_cx_customer_blpk, KEY_LEN); 

    return HI_SUCCESS;
}

HI_S32 HI_CX_GetRSAKeyInfor(AUTHALG_E eAuthType, HI_UNF_CIPHER_RSA_PUB_KEY_S *pstPubKey)
{
    if (pstPubKey == HI_NULL)
    {
       return HI_FAILURE;
    }

     pstPubKey->pu8N = s_cx_customer_rsa_public_key_N;
     pstPubKey->u16NLen = sizeof(s_cx_customer_rsa_public_key_N);
     pstPubKey->pu8E = s_cx_customer_rsa_public_key_E_3;
     pstPubKey->u16ELen = sizeof(s_cx_customer_rsa_public_key_E_3);    

    return HI_SUCCESS;
}
    
