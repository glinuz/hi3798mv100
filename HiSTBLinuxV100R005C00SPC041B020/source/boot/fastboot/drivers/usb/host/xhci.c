...
#  include "hiusb-xhci-hi3796mv2x.c"
#endif
		...
		u16 wIndex, volatile uint32_t *addr, u32 port_status)
{
	...
	debug("clear port change, actual port %d status  = 0x%x\n",
			 wIndex, port_status);
}
					...
					struct xhci_hcor **ret_hcor)
{
	...
	if (usb_xhci_cpu_init(index, &addr))
		return -1;

	*ret_hccr = (struct xhci_hccr *)(addr);
	*ret_hcor = (struct xhci_hcor *)((uint32_t) *ret_hccr
 ...
 */
int usb_lowlevel_init_ex(int index, void **controller)
{
	uint32_t val;
	uint32_t val2;
	uint32_t reg;
	...
	xhcic[index].xhci = CONFIG_XHCI_CONTROLLER_FLAG;
 ...
 * @return none
 */
int usb_lowlevel_stop_ex(int index)
{
