...
/*****************************************************************************/
static void hisfc400_spi_search_rw(struct spi_nand_chip_info_t *spiinfo,
...
/*****************************************************************************/
static void hisfc400_spi_get_erase(struct spi_nand_chip_info_t *spiinfo,
...
static void hisfc400_flash_init(struct hisfc_host *host, unsigned char mfr)
{
	...
	if (protect & protect_mask) {
		...
		if (regval & protect_mask)
			printf("Write protection disable fail, "
	...
	if (feature & (FEATURE_ECC_ENABLE | FEATURE_OTP_ENABLE)
		...
		if (regval & (FEATURE_ECC_ENABLE | FEATURE_OTP_ENABLE))
			printf("Internal ECC/OTP disable fail, "
				"Status Register: 0x%x.\n", regval);

		if ((mfr == NAND_MFR_WINBOND) && (!(regval & FEATURE_BUF_ENABLE)))
			printf("enable Buffer Read Mode failed, "
				"Status Register: 0x%x.\n", regval);
	}
}

/*****************************************************************************/
void hisfc400_spi_probe(struct hisfc_host *host, 
		       struct spi_nand_chip_info_t *info)

{
	struct hisfc_operation *spi = host->spi;
	unsigned char mfr = info->id[0];

	spi->host   = host;
	spi->driver = info->driver;

	hisfc400_spi_search_rw(info, spi->read, HISFC400_SUPPORT_READ,
			HISFC400_SUPPORT_MAX_DUMMY, SPI_NAND_READ);

	hisfc400_spi_search_rw(info, spi->write, HISFC400_SUPPORT_WRITE,
			HISFC400_SUPPORT_MAX_DUMMY, SPI_NAND_WRITE);

	hisfc400_spi_get_erase(info, spi->erase);
