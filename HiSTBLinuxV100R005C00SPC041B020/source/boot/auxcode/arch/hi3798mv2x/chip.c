...
static void get_lsadc_volt_region(struct lsadc_volt_region *region)
{
	...
	regval = readl(REG_BASE_SCTL + REG_SC_LSADC_CTRL);
	regval |= REG_SC_LSADC_CTRL_EN_A;
	writel(regval, REG_BASE_SCTL + REG_SC_LSADC_CTRL);

	delay(12000);

	lsadc_val = (u16)(readl(REG_BASE_SCTL + REG_SC_LSADC_DATA)
				& REG_SC_LSADC_DATA_A_MASK);
...
void set_core_voltage(void)
{
	...
	core_volt_reg_val  = readl(PMC_CORE_PWM);
	cpu_volt_reg_val = readl(PMC_CPU_PWM);
	...
	writel(core_volt_reg_val, PMC_CORE_PWM);
	writel(cpu_volt_reg_val, PMC_CPU_PWM);

	/* add for core&gpu controlled by one power */
	writel(core_volt_reg_val, PMC_GPU_PWM);
...
{
	writel(0x130, 0xf8a21060);
	writel(0x130, 0xf8a21064);
	writel(0x130, 0xf8a21068);
	writel(0x130, 0xf8a2106c);
	writel(0x130, 0xf8a21070);
	writel(0x130, 0xf8a210ac);
}
#endif
...
unsigned int boottype2modtype(unsigned int boot_type)
{
	...
	for (i = 0; i < size; i++) {
		if (boot_type == BOOT_MODE_NAND) {
			regval = readl(REG_FMC_BASE + REG_FMC_CFG);
			regval &= REG_FMC_NFMODE_MASK;
			if (regval == 0) {
